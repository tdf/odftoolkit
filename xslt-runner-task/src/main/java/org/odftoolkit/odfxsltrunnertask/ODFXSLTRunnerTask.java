...
public class ODFXSLTRunnerTask extends Task {
  ...
  /**
   * Set output file. The source of the transformation is stored under this name as plain (XML)
   * file.
   *
   * @param aOutFile
   */
  ...
  /**
   * Sets template mode. In template mode, the output package must exist, and the specified path
   * within the output file is replaced with the result of the transformation
   *
  ...
  /**
   * Set force mode. In this mode, the style sheet is applied even if the output file or package is
   * newer than the source and the style sheet.
   *
   * @param bForce
   */
  ...
  public Param createParam() {
    ...
    if (m_aParams == null) m_aParams = new Vector<XSLTParameter>();
  ...
  public @Override void execute() throws BuildException {
    ...
    if (!m_aStyleSheet.exists())
      throw new BuildException(
          "style sheet " + m_aStyleSheet.getAbsolutePath() + " does not exist", getLocation());
    ...
    if (m_aInput == null && m_aInputFile == null)
      ...
      throw new BuildException(
          "in and infile attributes must not be set simultaneously", getLocation());
    ...
    if (!m_aInput.exists())
      throw new BuildException(
          "input file " + m_aInput.getAbsolutePath() + " does not exist", getLocation());
    ...
    if (m_aOutput != null && m_aOutputFile != null)
      throw new BuildException(
          "out and outfile attribute must not be set simultaneously", getLocation());
    if (m_bTemplate && m_aOutput == null)
      throw new BuildException(
          "out attribute must be set if template attribute is set", getLocation());
    ...
    if (m_aOutputFile != null) {
      ...
      if (!m_aOutput.exists())
        throw new BuildException(
            "output file " + m_aStyleSheet.getAbsolutePath() + " does not exist", getLocation());
    }
    ...
    if (!m_bForce
        ...
        && m_aOutput.lastModified() > m_aStyleSheet.lastModified()) return;
    ...
    if (bError) throw new BuildException("transformation failed", getLocation());
  }
}
