...
public class OdfTable {
  ...
  private OdfTable(TableTableElement table) {
    ...
    if (mDocument instanceof OdfSpreadsheetDocument) mIsSpreadsheet = true;
  ...
  public long getWidth() {
    if (!mIsSpreadsheet) {
      ...
      if (sWidth == null) {
        ...
        return tableWidth;
      } else return PositiveLength.parseLong(sWidth, Unit.MILLIMETER);
    } else {
  ...
  private OdfTableColumn getHeaderColumnByIndex(
      TableTableHeaderColumnsElement headers, int nIndex) {
    int result = 0;
    OdfTableColumn col = null;
    for (Node n : new DomNodeList(headers.getChildNodes())) {
      if (n instanceof TableTableColumnElement) {
