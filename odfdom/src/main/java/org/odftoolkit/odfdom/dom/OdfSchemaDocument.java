...
public abstract class OdfSchemaDocument extends OdfPackageDocument {
  ...
  /** @return the office:styles element from the styles dom or null if there is no such element. */
  public OdfOfficeStyles getDocumentStyles() {
    if (mDocumentStyles == null) {
      try {
        ...
        if (stylesDom != null) {
          mDocumentStyles =
              OdfElement.findFirstChildNode(OdfOfficeStyles.class, stylesDom.getFirstChild());
        } else {
          return null;
        }
      } catch (Exception ex) {
