...
public class NonNegativeLength extends Length {
  ...
  private static final Pattern nonNegativeLengthPattern =
      Pattern.compile("^?([0-9]+(\\.[0-9]*)?|\\.[0-9]+)((cm)|(mm)|(in)|(pt)|(pc)|(px))$");
  /**
   * Construct NonNegativeLength by the parsing the given string
   *
   * @param length The String to be parsed into NonNegativeLength
  ...
  /**
   * check if the specified String instance is a valid {@odf.datatype nonNegativeLength} data type
   *
   * @param stringValue the value to be tested
   * @return true if the value of argument is valid for {@odf.datatype nonNegativeLength} data type
   *     false otherwise
   */
