...
public class NamespacedToken implements OdfDataType {
  ...
  private static final Pattern namespacedTokenPattern =
      Pattern.compile("^[0-9a-zA-Z_]+:[0-9a-zA-Z._\\-]+$");

  /**
   * Construct NamespacedToken by the parsing the given string
   *
   * @param namespacedToken The String to be parsed into NamespacedToken
   ...
   *
   * @return return a string representation of the value of this NamespacedToken object
   */
  @Override
  ...
  /**
   * check if the specified String instance is a valid {@odf.datatype namespacedToken} data type
   *
   * @param stringValue the value to be tested
   * @return true if the value of argument is valid for {@odf.datatype namespacedToken} data type
   *     false otherwise
   */
