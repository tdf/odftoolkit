...
public class CellRangeAddress implements OdfDataType {
  ...
  private static final Pattern cellRangeAddressPattern1 =
      Pattern.compile(
          "^(\\$?([^\\. ']+|'([^']|'')+'))?\\.\\$?[A-Z]+\\$?[0-9]+(:(\\$?([^\\. ']+|'([^']|'')+'))?\\.\\$?[A-Z]+\\$?[0-9]+)?$");
  private static final Pattern cellRangeAddressPattern2 =
      Pattern.compile(
          "^(\\$?([^\\. ']+|'([^']|'')+'))?\\.\\$?[0-9]+:(\\$?([^\\. ']+|'([^']|'')+'))?\\.\\$?[0-9]+$");
  private static final Pattern cellRangeAddressPattern3 =
      Pattern.compile(
          "^(\\$?([^\\. ']+|'([^']|'')+'))?\\.\\$?[A-Z]+:(\\$?([^\\. ']+|'([^']|'')+'))?\\.\\$?[A-Z]+$");
   ...
   *
   * @param cellRangeAddress The String to be parsed into CellRangeAddress
   ...
   *
   * @return return a string representation of the value of this CellRangeAddress object
   */
  @Override
   ...
   * @return true if the value of argument is valid for {@odf.datatype cellRangeAddress} data type
   *     false otherwise
   */
  public static boolean isValid(String stringValue) {
    if ((stringValue == null)
        || (!(cellRangeAddressPattern1.matcher(stringValue).matches()
            || cellRangeAddressPattern2.matcher(stringValue).matches()
            || cellRangeAddressPattern3.matcher(stringValue).matches()))) {
      return false;
    } else {
      return true;
    }
