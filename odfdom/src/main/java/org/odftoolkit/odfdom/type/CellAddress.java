...
public class CellAddress implements OdfDataType {
  ...
  private static final Pattern cellAddressPattern =
      Pattern.compile("^(\\$?([^\\. ']+|'([^']|'')+'))?\\.\\$?[A-Z]+\\$?[0-9]+$");
  /**
   * Construct CellAddress by the parsing the given string
   *
   * @param cellAddress The String to be parsed into CellAddress
   ...
   *
   * @return return a string representation of the value of this CellAddress object
   */
  @Override
   ...
   * @return true if the value of argument is valid for {@odf.datatype cellAddress} data type false
   *     otherwise
   */
