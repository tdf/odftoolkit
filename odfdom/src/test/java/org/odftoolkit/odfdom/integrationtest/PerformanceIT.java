...
public class PerformanceIT {
  ...
  public PerformanceIT() {
    try {
      TEST_FILE_FOLDER =
          PerformanceIT.class.getClassLoader().getResource("").toURI().getPath()
  ...
  private String getOutputPath() {
    ...
    try {
      File rootpath =
          new File(PerformanceIT.class.getClassLoader().getResource("").toURI().getPath());
  ...
  private void writeToLog() throws Exception {
    ...
    try {
      ...
      OfficeSpreadsheetElement spreadsheet =
          (OfficeSpreadsheetElement)
              timedoc
                  .getContentDom()
                  .getElementsByTagNameNS(OdfDocumentNamespace.OFFICE.getUri(), "spreadsheet")
                  .item(0);
    ...
    String[] summaryName =
        new String[] {"Load All Documents", "Parse All Documents", "Save All Documents"};
  ...
  private void updateTableCells(
      OdfDocument odfdoc, String tablename, double[] values, String[] labels) {
    int i = 0, j = 0;
    ...
    try {
      ...
      spreadsheet =
          (OfficeSpreadsheetElement)
              dom.getElementsByTagNameNS(OdfDocumentNamespace.OFFICE.getUri(), "spreadsheet")
                  .item(0);
      ...
      for (i = 1; i < values.length + 1; i++) {
        ...
        cell.setOfficeValueAttribute(values[j]);
        p = dom.newOdfElement(OdfTextParagraph.class);
