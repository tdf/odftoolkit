...
public class EmbeddedDocumentTest {
  ...
  private static final String TEST_OUTPUT_FOLDER =
      ResourceUtilities.getTestOutputFolder()
          + File.separatorChar
          + "EmbeddedDocumentTest"
          + File.separatorChar;
  ...
  private static final String TEST_FILE_EMBEDDED_SIDEBYSIDE_SAVE_OUT =
      "SaveEmbeddedDocSideBySide.odt";
  ...
  private static final String TEST_FILE_MODIFIED_EMBEDDED_SAVE_STANDALONE =
      "SaveModifiedEmbeddedDocAlone.odt";
  ...
  public static void setUpBeforeClass() throws Exception {
    ...
    File outputDir = new File(TEST_OUTPUT_FOLDER);
    outputDir.mkdir();
  }

  /**
   * The document A contains the embedded document E1, this test case is used to embed E1 to another
   * document B
   */
  @Test
  public void testEmbedEmbeddedDocument() {
    try {
      ...
      paths =
          saveDoc.getPackage().getDocumentPaths("application/vnd.oasis.opendocument.presentation");
      int presentationDocCount = paths.size();
      Assert.assertTrue(docCount > presentationDocCount);
      saveDoc.save(TEST_OUTPUT_FOLDER + TEST_FILE_EMBEDDED_SAVE_OUT);
      saveDoc.close();
  ...
  /**
   * The document B is embedded to document A and the directory path of A and B are absolute from
   * the package DOCA/ and DOCB/
   */
  @Test
  public void testembeddedDocumentsLocatedSideBySide() {
    try {
      ...
      docA.newImage(ResourceUtilities.getTestInputURI(TEST_PIC));
      ...
      TextPElement lastPara =
          (TextPElement) xpath.evaluate("//text:p[last()]", contentA, XPathConstants.NODE);
      ...
      // save
      docA.save(TEST_OUTPUT_FOLDER + TEST_FILE_EMBEDDED_SIDEBYSIDE_SAVE_OUT);
      OdfDocument testLoad =
          OdfDocument.loadDocument(TEST_OUTPUT_FOLDER + TEST_FILE_EMBEDDED_SIDEBYSIDE_SAVE_OUT);
      OdfFileEntry imageEntry =
          testLoad
              .getPackage()
              .getFileEntry(OdfPackage.OdfFile.IMAGE_DIRECTORY.getPath() + SLASH + TEST_PIC);
            ...
            doc1.getPackage()
                .getFileEntry(
                    doc1.getDocumentPath()
                        + OdfPackage.OdfFile.IMAGE_DIRECTORY.getPath()
                        + SLASH
                        + TEST_PIC_ANOTHER);
  ...
  /**
   * 1) A new sub document text document DOCA/ is inserted into a new text document 2) A picture is
   * being added to the subdocument (ie. /DOCA/Pictures/testA.jpg) 3) A new sub document spreadsheet
   * document is inserted into the first (ie. /DOCA/DOCB) 4) In the last paragraph of /DOCA a frame
   * with a reference to the subdocument DOCB is added 5) A picture is being added to the second
   * subdocument (ie. /DOCA/DOCB/Pictures/testB.jpg) 6) The spreadsheetname of DOCB is set to
   * "NewTable" 7) DOCA/ is saved in a document for its own
   */
  @Test
  public void testembeddedDocumentWithSubPath() {
    try {
      ...
      docA.save(TEST_OUTPUT_FOLDER + TEST_FILE_EMBEDDED_INCLUDED_SAVE_OUT);
      OdfDocument testLoad =
          OdfDocument.loadDocument(TEST_OUTPUT_FOLDER + TEST_FILE_EMBEDDED_INCLUDED_SAVE_OUT);
      OdfFileEntry imageEntry =
          testLoad
              .getPackage()
              .getFileEntry(OdfPackage.OdfFile.IMAGE_DIRECTORY.getPath() + SLASH + TEST_PIC);
  ...
  /**
   * The document A contains the embedded document E1, This test case is used to show how to save
   * the E1 to a stand alone document.
   */
  @Test
  public void testSaveEmbeddedDocument() {
    try {
      ...
      for (String childDocPath : embeddedDocs.keySet()) {
        ...
        childDoc.save(TEST_OUTPUT_FOLDER + embedFileName);
        LOG.log(Level.INFO, "Save file : {0}", TEST_OUTPUT_FOLDER + embedFileName);
        OdfDocument embeddedDoc = OdfDocument.loadDocument(TEST_OUTPUT_FOLDER + embedFileName);
  ...
  /**
   * There are two document, one is Presentation1_INVALID.odp another is a new text document
   * TestModifiedEmbeddedDoc.odt Presentation1_INVALID.odp contains an embed document named "Object
   * 1/", add one paragraph to Object 1 then embed "Object 1" to the new text document, and save
   * this text document reload TestModifiedEmbeddedDoc.odt, then get and modify embed document
   * "DocA" and save it to a standalone document load the saved standalone document, and check the
   * content of it
   */
  @Test
  public void testEmbedModifiedEmbeddedDocument() {
    try {
      ...
      TextHElement header =
          (TextHElement) xpath.evaluate("//text:h[1]", embedContentDom, XPathConstants.NODE);
      ...
      embeddedDoc.newImage(ResourceUtilities.getTestInputURI(TEST_PIC));
      ...
      saveDoc.save(TEST_OUTPUT_FOLDER + TEST_FILE_MODIFIED_EMBEDDED);
      saveDoc.close();
      // reload TestModifiedEmbeddedDoc.odt
      saveDoc = OdfDocument.loadDocument(TEST_OUTPUT_FOLDER + TEST_FILE_MODIFIED_EMBEDDED);
      ...
      Assert.assertNotNull(imageEntry);
      embeddedDoc.newImage(ResourceUtilities.getTestInputURI(TEST_PIC_ANOTHER));
      // save the "DocA" as the standalone document
      embeddedDoc.save(TEST_OUTPUT_FOLDER + TEST_FILE_MODIFIED_EMBEDDED_SAVE_STANDALONE);
      // load the standalone document and check the content
      OdfDocument standaloneDoc =
          OdfDocument.loadDocument(
              TEST_OUTPUT_FOLDER + TEST_FILE_MODIFIED_EMBEDDED_SAVE_STANDALONE);
      ...
      imageEntry =
          standaloneDoc
              ...
              .getFileEntry(OdfPackage.OdfFile.IMAGE_DIRECTORY.getPath() + SLASH + TEST_PIC);
      Assert.assertNotNull(imageEntry);
      OdfFileEntry anotherImageEntry =
          standaloneDoc
              .getPackage()
              .getFileEntry(
                  OdfPackage.OdfFile.IMAGE_DIRECTORY.getPath() + SLASH + TEST_PIC_ANOTHER);
  ...
  public void testRemoveEmbeddedDocument() {
    try {
      ...
      doc.save(TEST_OUTPUT_FOLDER + TEST_FILE_REMOVE_EMBEDDED_SAVE_OUT);
      doc.close();
      // check manifest entry for the embed document
      // the sub entry of the embed document such as the pictures of the document should also be
      // removed
      doc = OdfDocument.loadDocument(TEST_OUTPUT_FOLDER + TEST_FILE_REMOVE_EMBEDDED_SAVE_OUT);
  ...
  private void updateFrameForEmbeddedDoc(OdfFileDom dom, String originPath, String newPath)
      throws Exception {
