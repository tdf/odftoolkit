...
public class PackageTest {
  ...
  public void testNotCompressImages() throws Exception {
    ...
    ZipInputStream zinput =
        new ZipInputStream(ResourceUtilities.getTestResourceAsStream(IMAGE_PRESENTATION));
  ...
  public void loadPackage() {
    try {
      ...
      OdfPackage formulaPackage =
          OdfPackage.loadPackage(ResourceUtilities.getAbsolutePath(ODF_FORMULAR_TEST_FILE));
      ...
      try {
        ...
        OdfPackage.loadPackage(
            new File(ResourceUtilities.getAbsolutePath(IMAGE_TEST_FILE)),
            null,
            new DefaultHandler());
        Assert.fail();
  ...
  public void validationTest() {
    ...
    try {
      ...
      try {
        OdfPackage.loadPackage(
            new File(ResourceUtilities.getAbsolutePath("testA.jpg")), null, handler4);
        Assert.fail();
      } catch (Exception e) {
        String errorMsg = OdfPackageConstraint.PACKAGE_IS_NO_ZIP.getMessage();
        if (!e.getMessage().endsWith(errorMsg.substring(errorMsg.indexOf("%1$s") + 4))) {
    ...
    try {
      OdfDocument doc =
          OdfDocument.loadDocument(
              ResourceUtilities.getAbsolutePath(TEST_STYLE_STYLE_ATTRIBUTE_ODT));
