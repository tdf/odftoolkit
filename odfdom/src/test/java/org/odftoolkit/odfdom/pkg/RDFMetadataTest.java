...
public class RDFMetadataTest extends ModelTestBase {
  ...
  public void testGetRDFMetaFromGRDDLXSLT() throws Exception {
    OdfTextDocument odt =
        (OdfTextDocument) OdfDocument.loadDocument(ResourceUtilities.getAbsolutePath(SIMPLE_ODT));
  ...
  public void testGetInContentMetaFromCache() throws Exception {
    ...
    TextMetaElement tm =
        (TextMetaElement)
            xpath.evaluate("//text:p/text:meta[last()]", contentDom, XPathConstants.NODE);
    ...
    TestCase.assertEquals(
        "http://dbpedia.org/page/J._K._Rowling",
        m1.listStatements().nextStatement().getSubject().getURI());

    tm.setTextContent("Joanne Kathleen Rowling");
    m1 = odt.getInContentMetadataFromCache();
    TestCase.assertEquals(
        "Joanne Kathleen Rowling", m1.listStatements().nextStatement().getObject().toString());

    tm.setXhtmlPropertyAttribute("dc:name");
    m1 = odt.getInContentMetadataFromCache();
    TestCase.assertEquals(
        "http://purl.org/dc/elements/1.1/name",
        m1.listStatements().nextStatement().getPredicate().getURI());
    ...
    TestCase.assertEquals(
        ...
        m1.listStatements().nextStatement().getObject().toString());
    ...
    TableCoveredTableCellElement tctce =
        contentDom.newOdfElement(TableCoveredTableCellElement.class);
  ...
  public void testGetBookmarkRDFMetadata() throws Exception {
    ...
    OdfContentDom contentDom = odt.getContentDom();
    XPath xpath = contentDom.getXPath();
    TextBookmarkStartElement tm =
        (TextBookmarkStartElement)
            xpath.evaluate("//text:bookmark-start[last()]", contentDom, XPathConstants.NODE);
    BookmarkRDFMetadataExtractor extractor =
        BookmarkRDFMetadataExtractor.newBookmarkTextExtractor();
