...
/**
 * Test for Presentation Slide API, including get/move/insert/delete slide in one presentation and
 * the slides operation of two presentations.
 */
public class SlideTest {
  ...
  /**
   * Test case for get presentation slide, including get the slide count, get the slide at the
   * specified position or with the specified name, and get the collection of slide.
   *
   * <p><b>Precondition</b> Load Presentation File at test\resources\performance\Presentation1.odp
   * <b>Method</b> 1)By using OdfPresentationDocument.getSlideCount() to Get the number of the
   * slides in this presentation. 2)According to the slide count, get a slide at the specific
   * position of the loaded presentation document using OdfPresentationDocument.getSlideByIndex(int
   * index), and the param "index" should be limited with the slide count. Use
   * OdfDrawPage.getSlideIndex() method to verify the returned slide.
   * 3)OdfPresentationDocument.getSlideByName(String name) 4)Using
   * dfPresentationDocument.getSlides() to get the slide iterator <b>Covered Element</b>
   * &lt;office:presentation&gt;,&lt;draw:page&gt; <b>Note</b>
   * OdfPresentationDocument.getContentRoot override OdfDocument.getContentRoot method will return
   * OfficePresentationElement, rather than OdfOfficePresentation because of the inheritance
   * relationship broken up between dom and doc element And because OdfOfficePresentation is
   * useless, so I delete this class <b>Code Coverage Result</b> 1)can not catch Exception because
   * it is only used to catch the exception thrown by getContentRoot()
   */
  @Test
  public void testGetSlide() {
    try {
      doc =
          OdfPresentationDocument.loadDocument(
              ResourceUtilities.getAbsoluteInputPath(TEST_PRESENTATION_FILE_MAIN));
  ...
  /**
   * Test case for get/set slide name to make sure the name is unique in the whole presentation
   * document. If the loaded presentation document is not valid that contain the same slide name, we
   * should modified the duplicate slide name to make them unique.
   *
   * <p><b>Precondition</b> Load Presentation File at test\resources\performance\Presentation1.odp
   * Using the dom method to make slide index 4 and 8 have the same name, and delete the "draw:name"
   * attribute of slide 7. <b>Method</b> 1)By accessing any slide API to trigger the duplicate slide
   * name check method. After this operation, slide 8 will change the slide name. 2)Trigger
   * OdfDrawPage.getSlideName() to make the slide 7 has the unique slide name. 3)Using
  ...
  public void testSlideName() {
    try {
      ...
      note7.addText(
          "This is slide at index" + slide7.getSlideIndex() + " named " + slide7.getSlideName());
      ...
      OdfSlide slide1 = doc.getSlideByIndex(1);
      Assert.assertTrue(
          "the given name page1 is duplicate with the previous slide",
          slide1.getSlideName().equals("haha"));
    } catch (Exception e) {
      LOG.log(Level.SEVERE, e.getMessage(), e);
      Assert.fail("Failed with " + e.getClass().getName() + ": '" + e.getMessage() + "'");
    }
    ...
    try {
      doc.save(
          ResourceUtilities.getTestOutputFile(TEST_PRESENTATION_FILE_OUT_PREFIX + "SlideName.odp"));
    } catch (Exception e) {
      LOG.log(Level.SEVERE, e.getMessage(), e);
      Assert.fail("Failed with " + e.getClass().getName() + ": '" + e.getMessage() + "'");
    }
  ...
  /**
   * Test case for delete presentation slide, including delete slide at the specified position or
   * with the specified name.
   *
   * <p><b>Precondition</b> Load Presentation File at test\resources\performance\Presentation1.odp
   * <b>Method</b> 1)By using OdfPresentationDocument.deleteSlideByIndex(int index) to delete the
   * slide at the specified position which is present by param "index", then index of the slide
   * which after the delete slide will change. 2)Delete slide with the specified name by using
   * OdfPresentationDocument.deleteSlideByName(String name) <b>PostCondition</b> Save the modified
   * document at SlideResultDelete.odp <b>Covered Element</b> 1)
   * &lt;office:presentation&gt;,&lt;draw:page&gt; 2)All the element that contains "xlink:href"
   * attribute, such as &lt;draw:object&gt;, &lt;draw:image&gt;, etc. 3)All the style name
   * definition element, include &lt;style:style&gt;, &lt;text:list-style&gt;,
   * &lt;number:time-style&gt;, &lt;number:date-style&gt;, &lt;number:boolean-style&gt;,
   * &lt;number:number-style&gt;, &lt;number:currency-style&gt;, &lt;number:percentage-style&gt;,
   * &lt;number:text-style&gt; 4)All the <code>OdfStylableElement</code> which have the style name
   * reference, such as &lt;draw:frame&gt;, &lt;text:p&gt; etc. <b>Note</b> When the specific slide
   * is delete, if it referred images, ole or styles are not used by other slide they will all be
  ...
  public void testDeleteSlide() {
    try {
      ...
      // this slide contains an embed document, remove this slide will also remove the embed
      // document
      ...
      // slide at index 9 contains two images and one embed document, the embed document aslo have
      // the object replacement image
      ...
      doc2 =
          OdfPresentationDocument.loadDocument(
              ResourceUtilities.getAbsoluteInputPath(TEST_PRESENTATION_FILE_ANOTHER));
    ...
    try {
      doc.save(
          ResourceUtilities.getTestOutputFile(TEST_PRESENTATION_FILE_OUT_PREFIX + "Delete.odp"));
      doc2.save(
          ResourceUtilities.getTestOutputFile(TEST_PRESENTATION_FILE_OUT_PREFIX + "Delete2.odp"));
    } catch (Exception e) {
      LOG.log(Level.SEVERE, e.getMessage(), e);
      Assert.fail("Failed with " + e.getClass().getName() + ": '" + e.getMessage() + "'");
    }
  ...
  /**
   * Test case for create new slide at the specified position, with the specified name and slide
   * template.
   *
   * <p><b>Precondition</b> Load Presentation File at test\resources\performance\Presentation1.odp
   * <b>Method</b> OdfPresentationDocument.newSlide(int index, String name, OdfDrawPage.SlideLayout
   * slideLayout) is used to new a slide at the specified position with the specified name, and use
   * the specified slide template. <b>Postcondition</b> Save the modified document at
   * SlideResultNew.odp <b>Covered Element</b> 1) &lt;style:presentation-page-layout&gt; referred by
   * "presentation:presentation-page-layout-name" attribute of &lt;draw:page&gt; 2)
   * &lt;style:master-page&gt; referred by "draw:master-page-name" attribute of &lt;draw:page&gt; 3)
   * The placeholder element of &lt;draw:page&gt;, such as title, outline placeholder <b>Note</b>
   * The new slide will use the same master page style of the previous slide, and add the
   * placeholder on the slide according to the slide template parameter.
   */
  @Test
  public void testNewSlide() {
    try {
      ...
      doc.newSlide(
          3,
          OdfSlide.SlideLayout.TITLE_PLUS_TEXT.toString(),
          OdfSlide.SlideLayout.enumValueOf("title_text"));
    ...
    try {
      doc.save(ResourceUtilities.getTestOutputFile(TEST_PRESENTATION_FILE_OUT_PREFIX + "New.odp"));
    } catch (Exception e) {
      LOG.log(Level.SEVERE, e.getMessage(), e);
      Assert.fail("Failed with " + e.getClass().getName() + ": '" + e.getMessage() + "'");
    }
  ...
  /**
   * Test case for move the slide from one position to another, or copy a specified slide and insert
   * it to another specified position.
   *
   * <p><b>Precondition</b> Load Presentation File at test\resources\performance\Presentation1.odp
   * test\resources\performance\Presentation1.odp <b>Method</b> 1)Using
   * OdfPresentationDocument.moveSlide(int current, int destination) to move a slide at the current
   * position to the destination position. 2)OdfPresentationDocument.copySlide(int source, int dest,
   * String newName) is used to make a copy of the slide at a specified position to another position
   * in this presentation. <b>Postcondition</b> Save the modified document at
   * SlideResultMoveAndCopy.odp <b>Covered Element</b> 1)
   * &lt;office:presentation&gt;,&lt;draw:page&gt; 2) &lt;presentation:notes&gt; of each slide.
   * <b>Note</b> Each slide has its own notes page to show its notes view, while the notes page has
   * the "draw:page-number" attribute to show the notes view of which slide so when the slide is
   * moved or copied, the index of this slide will be change, the notes page have to change the
   * "draw:page-number" value.
   */
  @Test
  public void testMoveAndCopySlide() {
    ...
    try {
      doc.save(
          ResourceUtilities.getTestOutputFile(
              TEST_PRESENTATION_FILE_OUT_PREFIX + "MoveAndCopy.odp"));
    } catch (Exception e) {
      LOG.log(Level.SEVERE, e.getMessage(), e);
      Assert.fail("Failed with " + e.getClass().getName() + ": '" + e.getMessage() + "'");
    }
  ...
  /**
   * Test case for copy a slide which comes from the other presentation to the specified position of
   * the current presentation.
   *
   * <p><b>Precondition</b> Load Presentation File at test\resources\performance\Presentation1.odp &
   * test\resources\performance\Presentation2.odp <b>Method</b> Using
   * OdfPresentationDocument.copyForeignSlide(int destIndex,OdfPresentationDocument srcDoc, int
   * srcIndex) to copy a slide which locates at "srcIndex" position of the "srcDoc" presentation
   * document and insert it to the current presentation document at "destIndex" position.
   * <b>Postcondition</b> Save the modified document at SlideResultCopyForeignSlide.odp <b>Covered
   * Element</b> 1) &lt;office:presentation&gt;,&lt;draw:page&gt; 2) All the element that contains
   * "xlink:href" attribute, such as &lt;draw:object&gt;, &lt;draw:image&gt;, etc. 3) Each style
   * definition element of &lt;draw:page&gt; and its all child elements such as layout style, master
   * page style, object style and text style the style definition element must contain "style:name"
   * or "draw:name" attribute, include
   * &lt;style:mater-page&gt;,&lt;style:page-layout&gt;,&lt;style:presentation-page-layout&gt;,
   * &lt;style:style&gt;, &lt;text:list-style&gt;, &lt;number:time-style&gt;,
   * &lt;number:date-style&gt;, &lt;number:boolean-style&gt;, &lt;number:number-style&gt;,
   * &lt;number:currency-style&gt;, &lt;number:percentage-style&gt;, &lt;number:text-style&gt;,
   * &lt;text:outline-style&gt;,&lt;style:font-face&gt;, 4) the element which refer the style name,
   * that is &lt;draw:page&gt; and all the child elements. <b>Note</b> If the copied style/image/ole
   * name is duplicated with destination presentation document, the name definition element and
  ...
  public void testCopyForeignSlide() {
    ...
    try {
      doc2.save(
          ResourceUtilities.getTestOutputFile(
              TEST_PRESENTATION_FILE_OUT_PREFIX + "CopyForeignSlide2.odp"));
    } catch (Exception e) {
      LOG.log(Level.SEVERE, e.getMessage(), e);
      Assert.fail("Failed with " + e.getClass().getName() + ": '" + e.getMessage() + "'");
    }
  ...
  /**
   * Test case for appending all the slide of one presentation to the end of another presentation.
   *
   * <p><b>Precondition</b> Load Presentation File at test\resources\performance\Presentation1.odp &
   * test\resources\performance\Presentation2.odp <b>Method</b> Using
   * OdfPresentationDocument.appendPresentation(OdfPresentationDocument aDoc) to append the
   * specified presentation to the current presentation. <b>Postcondition</b> Save the modified
   * document at SlideResultMerge.odp <b>Covered Element</b> Same with <code>CopyForeignSlide</code>
   * method, except that it covers all the slide element of the document,rather than specific slide.
   * <b>Note</b> 1)You'd better check the generated document to look at if there are any
   * object/style is lost. 2) The generated file size will be larger than we append by using
   * OpenOffice/Symphony, that is because the styles and images from the different document might
   * have the same content, but I still append them rather than use one copy, because compare the
   * odf element content and the image stream might cost much time.
   */
  @Test
  public void testAppendPresentation() {
    try {
      ...
      NodeList objectList =
          slideEle.getElementsByTagNameNS(OdfDocumentNamespace.DRAW.getUri(), "object");
    ...
    try {
      doc2.save(
          ResourceUtilities.getTestOutputFile(TEST_PRESENTATION_FILE_OUT_PREFIX + "Merge2.odp"));
    } catch (Exception e) {
      LOG.log(Level.SEVERE, e.getMessage(), e);
      Assert.fail("Failed with " + e.getClass().getName() + ": '" + e.getMessage() + "'");
    }
  ...
  public void testAnotherMergeDoc() {
    try {
      ...
      doc.save(
          ResourceUtilities.getTestOutputFile(TEST_PRESENTATION_FILE_OUT_PREFIX + "Merge1.odp"));
    } catch (Exception e) {
      LOG.log(Level.SEVERE, e.getMessage(), e);
      Assert.fail("Failed with " + e.getClass().getName() + ": '" + e.getMessage() + "'");
    }
   ...
   *
   * <p><b>Precondition</b> Load Presentation File at test\resources\performance\SlideTest1.odp,
   ...
   * <b>Postcondition</b> Save the modified document at SlideResultCopyThreeDoc.odp <b>Notice</b>
   * This test case is used to show that the style name can be renamed if they define the different
   * style.
   */
  @Test
  public void testCopyThreeDoc() {
    try {
      ...
      OdfPresentationDocument testdoc1 =
          OdfPresentationDocument.loadDocument(
              ResourceUtilities.getAbsoluteInputPath(TEST_PRESENTATION_DOCUMENT1));
      // testdoc1 contain "dp1" for draw page style
      OdfPresentationDocument testdoc2 =
          OdfPresentationDocument.loadDocument(
              ResourceUtilities.getAbsoluteInputPath(TEST_PRESENTATION_DOCUMENT2));
      // testdoc1 contain "dp1" for draw page style
      OdfPresentationDocument testdoc3 =
          OdfPresentationDocument.loadDocument(
              ResourceUtilities.getAbsoluteInputPath(TEST_PRESENTATION_DOCUMENT3));
      ...
      testdoc1.save(
          ResourceUtilities.getTestOutputFile(
              TEST_PRESENTATION_FILE_OUT_PREFIX + "CopyThreeDoc.odp"));
