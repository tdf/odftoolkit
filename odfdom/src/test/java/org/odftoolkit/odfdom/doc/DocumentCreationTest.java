...
public class DocumentCreationTest {
  ...
  // Changed leading space against character as leading space have to be <text:s/> element in ODF,
  // see
  ...
  private static final String TEST_FILE_ACCESS_EMBEDDED =
      TEST_INPUT_FOLDER + "TestAccess_EmbeddedDocument.odt";
  private static final String TEST_FILE_EMBEDDED_EMBEDDED =
      TEST_INPUT_FOLDER + "TestAccess_EmbeddedinEmbedded.odt";
  private static final String TEST_FILE_SAVE_EMBEDDED =
      TEST_INPUT_FOLDER + "testSaveEmbeddedDoc.odt";
  private static final String TEST_FILE_SAVE_EMBEDDED_OUT =
      TEST_OUTPUT_FOLDER + "TestSaveEmbeddedDoc_newName.odt";
  private static final String TEST_FILE_SAVE_EMBEDDED_OUT2 =
      TEST_OUTPUT_FOLDER + "TestSaveEmbeddedDoc2.odt";
  private static final String TEST_FILE_SAVE_QUEER_PATH =
      TEST_OUTPUT_FOLDER + "TestSaveQueerEmbeddedPathDoc1.odt";
  private static final String CORRUPTED_MIMETYPE_DOC =
      TEST_INPUT_FOLDER + "CorruptedMimetypeDoc.odt";
  private static final String CORRUPTED_MIMETYPE_DOC_OUT =
      TEST_INPUT_FOLDER + "TestSaveCorruptedMimetypeDoc.odt";
  private static final String CORRUPTED_MIMETYPE_CHART =
      TEST_INPUT_FOLDER + "CorruptedMimetypeChart.odc";
  private static final String CORRUPTED_MIMETYPE_CHART_OUT =
      TEST_INPUT_FOLDER + "TestSaveCorruptedMimetypeChart.odc";
    ...
    try {
      ...
      OdfPackage pkg =
          OdfPackage.loadPackage(
              ResourceUtilities.getTestOutputAsStream("TestEmpty_OdfTextDocument.odt"));

      // loads the images from the URLs and inserts the image in the package, adapting the manifest
      pkg.insert(
          ResourceUtilities.getURI("test-input" + File.separatorChar + TEST_PIC),
          "Pictures/" + TEST_PIC,
          null);
      ...
      OdfDocument odfDoc =
          OdfDocument.loadDocument(
              ResourceUtilities.getAbsoluteOutputPath("TestEmpty_OdfTextDocument.odt"));

      // get the ODF content as DOM tree representation
      OdfFileDom odfContent = odfDoc.getContentDom();

      //// W3C XPath initialization ''(JDK5 functionality)''  - XPath is the path within the XML
      // file
      ...
      TextPElement para =
          (TextPElement) xpath2.evaluate("//text:p[1]", odfContent, XPathConstants.NODE);

      // adding an image - expecting the user to know that
      // an image consists always of a 'draw:image' and a 'draw:frame' parent

      // FUTURE USAGE: para.createDrawFrame().createDrawImage("/myweb.org/images/myHoliday.png",
      // "/Pictures/myHoliday.png");
      //             Child access methods are still not part of the v0.6.x releases
      // CURRENT USAGE:
      OdfDrawFrame odfFrame =
          (OdfDrawFrame) OdfXMLFactory.newOdfElement(odfContent, DrawFrameElement.ELEMENT_NAME);
      para.appendChild(odfFrame);
      OdfDrawImage odfImage =
          (OdfDrawImage) OdfXMLFactory.newOdfElement(odfContent, OdfDrawImage.ELEMENT_NAME);
      odfFrame.appendChild(odfImage);
      odfImage.newImage(ResourceUtilities.getURI("test-input" + File.separatorChar + TEST_PIC));

      OdfDrawImage odfImage2 =
          (OdfDrawImage) OdfXMLFactory.newOdfElement(odfContent, OdfDrawImage.ELEMENT_NAME);
  ...
  public void _2_createEmbeddedDocs() {
    try {
      ...
      odtDoc1.insertDocument(
          OdfDocument.loadDocument(
              ResourceUtilities.getAbsoluteOutputPath("TestEmpty_OdfGraphicsDocument.odg")),
          "Object3");
      ...
      Map<String, OdfDocument> embeddedTextDocs =
          odtDoc1.loadSubDocuments(OdfDocument.OdfMediaType.TEXT);
      LOG.log(Level.INFO, "Only Embedded Text Docs Size: {0}", embeddedTextDocs.size());
      Assert.assertTrue(embeddedTextDocs.size() == 2);

      Map<String, OdfDocument> embeddedChartDocs =
          odtDoc1.loadSubDocuments(OdfDocument.OdfMediaType.CHART);
      ...
      Assert.assertEquals(
          embeddedObject1.getMediaTypeString(), OdfDocument.OdfMediaType.TEXT.getMediaTypeString());
      ...
      Assert.assertEquals(
          embeddedObject3.getMediaTypeString(),
          OdfDocument.OdfMediaType.GRAPHICS.getMediaTypeString());
      ...
      Assert.assertEquals(
          embeddedObject6.getMediaTypeString(),
  ...
  public void _3_accessEmbeddedDocs() {
    try {
      ...
      OdfStyle documentStyle =
          embDoc.getDocumentStyles().getStyle("myStyle", OdfStyleFamily.Paragraph);
      ...
      OdfStyle documentStyle2 =
          embDoc2.getDocumentStyles().getStyle("myStyle", OdfStyleFamily.Paragraph);
      String prop2 = documentStyle2.getProperty(StyleTextPropertiesElement.FontWeight);
      Assert.assertEquals(prop2, "bold");

      TextSpanElement spanTest =
          (TextSpanElement)
              xpath.evaluate("//text:p[1]/text:span[1]", contentDom, XPathConstants.NODE);
  ...
  public void _4_accessEmbeddedWithinEmbeddedDocs() {
    ...
    try {
      ...
      OdfFileEntry fileEntry =
          embDoc.getPackage().getFileEntry(embDoc.getDocumentPath() + pathOfSecondInnerDoc);
      ...
      OdfDocument emb_embDoc =
          rootDocument.loadSubDocument(embDoc.getDocumentPath() + pathOfSecondInnerDoc);
      ...
      OdfDocument doubleEmbeddedDoc =
          docWithdoubleEmbeddedDoc.loadSubDocument("Object 1/Object in Object1");

      OdfContentDom dEDcontentDom = doubleEmbeddedDoc.getContentDom();
      TextSpanElement spanTest =
          (TextSpanElement)
              xpath.evaluate("//text:span[last()]", dEDcontentDom, XPathConstants.NODE);
      Assert.assertEquals(spanTest.getTextContent(), TEST_SPAN_TEXT);
  ...
  public void _6_testSaveEmbeddedDocuments() {
    ...
    try {
      ...
      for (String eDocPath : embDocs.keySet()) {
        ...
        OdfTextSpan span =
            (OdfTextSpan) xpath.evaluate("//text:span[last()]", contentDom2, XPathConstants.NODE);
        ...
        for (String eDocPath3 : embDocs3.keySet()) {
          ...
          TextPElement para2 =
              (TextPElement) xpath.evaluate("//text:p[1]", contentDom3, XPathConstants.NODE);
          addImageToDocument(contentDom3, para2);
          TextPElement para3 =
              (TextPElement) xpath.evaluate("//text:p[last()]", contentDom3, XPathConstants.NODE);
          ...
          NodeList linkNodes =
              (NodeList)
                  xpath.evaluate("//*[@xlink:href]", testLoad.getContentDom(), XPathConstants.NODE);
          for (int i = 0; i < linkNodes.getLength(); i++) {
            ...
            Assert.assertTrue(
                refObjPath.equals("Pictures/" + TEST_PIC) || refObjPath.equals("./NewEmbedded"));
          }
          ...
          OdfTextSpan span4 =
              (OdfTextSpan) xpath.evaluate("//text:span[last()]", contentDom4, XPathConstants.NODE);
  ...
  @Test
  /**
   * OdfDocument docA and docB are ODF subdocuments. docA containing docB, like
  ...
  /**
   * Test if saving OdfDocuments always results in a valid ODF file. This means that the mimetype is
   * written clearly at the first position in the zipped ODF package.
   */
  @Test
