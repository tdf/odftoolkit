...
public class ChartTemplateTest {
  ...
  public void testLoadingAChartTemplate() throws Exception {
    OdfDocument document =
        OdfDocument.loadDocument(ResourceUtilities.getAbsoluteInputPath(TEST_CHART_TEMPLATE));
    Assert.assertEquals(
        OdfDocument.OdfMediaType.CHART_TEMPLATE.getMediaTypeString(),
  ...
  public void testSavingAChartTemplate() throws Exception {
    OdfDocument document =
        OdfDocument.loadDocument(ResourceUtilities.getAbsoluteInputPath(TEST_CHART_TEMPLATE));
    File destination =
        File.createTempFile("odfdom-test", ".otc", ResourceUtilities.getTempTestDirectory());
  ...
  public void testSwitchingOdfChartDocument() throws Exception {
    ...
    Assert.assertEquals(
        OdfDocument.OdfMediaType.CHART.getMediaTypeString(),
        document.getPackage().getMediaTypeString());
  }
  ...
  public void testSwitchingOdfImageDocument() throws Exception {
    ...
    Assert.assertEquals(
        OdfDocument.OdfMediaType.IMAGE_TEMPLATE.getMediaTypeString(),
        document.getPackage().getMediaTypeString());

    document = OdfImageDocument.newImageTemplateDocument();
    document.changeMode(OdfImageDocument.OdfMediaType.IMAGE);
    Assert.assertEquals(
        OdfDocument.OdfMediaType.IMAGE.getMediaTypeString(),
        document.getPackage().getMediaTypeString());
  }
}
