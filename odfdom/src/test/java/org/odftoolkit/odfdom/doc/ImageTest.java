...
public class ImageTest {
                  ...
                  if (packagePath == null || !pkg.contains(packagePath)) {
                    ...
                    Assert.fail(
                        "Instead of '"
                            ...
                            + mImageName_ODFDOM
                            + "' the following image path was returned: '"
                            + packagePath
                            + "'");
                  }
                } catch (Exception ex) {
  ...
  public void testImageInTextDocument() {
    try {
      ...
      // load the file again
      OdfTextDocument doc1 =
          (OdfTextDocument)
              OdfDocument.loadDocument(ResourceUtilities.getAbsoluteOutputPath("addimages.odt"));
  ...
  public void testSVGInTextDocument() {
    try {
      ...
      OdfTextDocument doc1 =
          (OdfTextDocument)
              OdfDocument.loadDocument(ResourceUtilities.getAbsoluteOutputPath("svg-image.odt"));
  ...
  public void testRemoveAllImage() {
    try {
      // this test is dependent of the output of the following test, which is therefore being called
      // explicitly
