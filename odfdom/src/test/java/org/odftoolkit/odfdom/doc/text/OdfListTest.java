...
public class OdfListTest {
  ...
  public void testOdfList() {
    ...
    OdfTextList instance = new OdfTextList(dom, listContent, '>', styleName);
    Node node;
  ...
  private void checkNode(Node node) {
    while (node != null) {
      Assert.assertTrue("More nodes than expected", position < expected.length);
  ...
  private void checkParagraph(Node node, String value) {
    Node child;
    Assert.assertTrue("Not a paragraph", node instanceof TextPElement);
    Assert.assertTrue("Paragraph has no child", node.hasChildNodes());
