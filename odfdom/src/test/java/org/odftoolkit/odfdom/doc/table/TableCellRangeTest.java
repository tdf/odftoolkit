...
public class TableCellRangeTest {
      ...
      odsdoc =
          (OdfSpreadsheetDocument)
              OdfSpreadsheetDocument.loadDocument(
                  ResourceUtilities.getAbsolutePath(filename + ".ods"));
      odtdoc =
          (OdfTextDocument)
              OdfTextDocument.loadDocument(ResourceUtilities.getAbsolutePath(odtfilename + ".odt"));
    } catch (Exception e) {
  ...
  public void testTextCellMerge() {
    ...
    try {
      OdfTextDocument saveddoc =
          (OdfTextDocument)
              OdfTextDocument.loadDocument(
                  ResourceUtilities.getAbsolutePath(odtfilename + "MergeTwoCell.odt"));
  ...
  public void testTextTableMerge() {
    ...
    OdfTableCellRange cellRange =
        table1.getCellRangeByPosition(0, 0, table1.getColumnCount() - 1, table1.getRowCount() - 1);
    cellRange.merge();
  ...
  public void testTextColumnMerge() {
    // merge first column
    OdfTable table1 = odtdoc.getTableByName("Table1");
    OdfTableCellRange firstColumn =
        table1.getCellRangeByPosition(0, 0, 0, table1.getRowCount() - 1);
    firstColumn.merge();
    ...
    try {
      OdfTextDocument saveddoc =
          (OdfTextDocument)
              OdfTextDocument.loadDocument(
                  ResourceUtilities.getAbsolutePath(odtfilename + "MergeFirstColumn.odt"));
      OdfTable savedTable = saveddoc.getTableByName("Table1");
      OdfTableCellRange firstTwoColumn =
          savedTable.getCellRangeByPosition(0, 0, 1, savedTable.getRowCount() - 1);
  ...
  public void testTextRowMerge() {
    ...
    OdfTableCellRange firstTwoRow =
        table1.getCellRangeByPosition(0, 0, table1.getColumnCount() - 1, 1);
    firstTwoRow.merge();
  ...
  public void testSpreadSheetMerge() {
    ...
    try {
      OdfSpreadsheetDocument saveddos =
          (OdfSpreadsheetDocument)
              OdfSpreadsheetDocument.loadDocument(
                  ResourceUtilities.getAbsolutePath(filename + "CellRangeName.ods"));
