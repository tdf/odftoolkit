...
public class OdfDrawImageTest {
  ...
  public void testInsertImage_URI() throws Exception {
    ...
    assert (frame.getSvgWidthAttribute().startsWith("19.")
        && frame.getSvgWidthAttribute().endsWith("cm"));
    assert (frame.getSvgHeightAttribute().startsWith("6.")
        && frame.getSvgHeightAttribute().endsWith("cm"));
  ...
  public void testInsertImage_InputStream() throws Exception {
    ...
    image.newImage(
        new FileInputStream(ResourceUtilities.getAbsoluteInputPath("testA.jpg")),
        packagePath,
        mediaType);
    assertEquals(image.getXlinkTypeAttribute(), "simple");
    assert (frame.getSvgWidthAttribute().startsWith("19.")
