...
class RoundtripTestHelper {
  ...
  /**
   * @param testFileNameTrunc the name of the test file before the suffix (i.e. '.')
   * @param testMethodName the name of the test method, required for debug output
   */
  protected String roundtripRegressionTest(String testFileNameTrunc, String fileNameSuffix) {
    return roundtripOperationTest(
        testFileNameTrunc, fileNameSuffix, NO_METHOD_NAME, null, true, true, true, null, false);
  }
   ...
   * @param testMethodName the name of the test method, required for debug output
   */
  protected String roundtripOnlyToOriginalDocRegressionTextTest(
      String testFileNameTrunc, String testMethodName, String editOperations) {
    return roundtripOperationTest(
        testFileNameTrunc,
        ODT_SUFFIX,
        testMethodName,
   ...
   * @param testMethodName the name of the test method, required for debug output
   */
  protected String roundtripOnlyToEmptyDocRegressionTest(
      String testFileNameTrunc, String fileNameSuffix) {
    return roundtripOperationTest(
        testFileNameTrunc, fileNameSuffix, NO_METHOD_NAME, null, true, false, true, null, false);
  }

  /**
   * @param testFileNameTrunc the name of the test file before the suffix (i.e. '.')
   * @param testMethodName the name of the test method, required for debug output
   * @param editOperations a list of operations strings to edit the test document
   */
  protected String roundtripRegressionTest(
      String testFileNameTrunc, String testfileNameSuffix, String testMethodName) {
    return roundtripOperationTest(
        testFileNameTrunc, testfileNameSuffix, testMethodName, null, true, true, true, null, false);
  }
   ...
   * @param editOperations a list of operations strings to edit the test document
   */
  protected String importOnlyRegressionTest(
      String testFileNameTrunc,
      String testfileNameSuffix,
      String testMethodName,
      String editOperations) {
    return roundtripOperationTest(
        testFileNameTrunc,
        testfileNameSuffix,
        testMethodName,
   ...
   * @param testMethodName the name of the test method, required for debug output
   * @param editOperations a list of operations strings to edit the test document
   * @param resourceMap a map of resources given by byte arrays identified by an ID
   */
  protected String importOnlyRegressionWithResourcesTest(
      String testFileNameTrunc,
      String testfileNameSuffix,
      String testMethodName,
      Map<Long, byte[]> resourceMap,
      boolean debug) {
    return roundtripOperationTest(
        testFileNameTrunc,
        testfileNameSuffix,
   ...
   * @param testMethodName the name of the test method, required for debug output
   * @param editOperations a list of operations strings to edit the test document
   * @param resourceMap a map of resources given by byte arrays identified by an ID document
   */
  protected String roundtripRegressionWithResourcesTest(
      String testFileNameTrunc,
      String testMethodName,
      String editOperations,
      Map<Long, byte[]> resourceMap,
   ...
   */
  protected String roundtripRegressionTextTest(
      String testFileNameTrunc, String testMethodName, String editOperations) {
    return roundtripOperationTest(
        testFileNameTrunc,
        ODT_SUFFIX,
  ...
  /**
   * @param testFileNameTrunc the name of the test file before the suffix (i.e. '.')
   * @param testFileNameSuffix the suffix of the file name including the dot (e.g. ".odt")
   * @param editOperationList a list of operations strings to edit the test document
   * @param editOperationString a string of operations to edit the test document
   */
  protected String roundtripOperationTest(
      String testFileNameTrunc,
      String testfileNameSuffix,
      String testMethodName,
      String editOperationString,
      boolean _doReloadTest,
      boolean _applyEditOpsToOriginalDoc,
      boolean _applyOpsToEmptyDoc,
      Map<Long, byte[]> resourceMap,
      boolean operationDebugMode) {
    ...
    return roundtripOperationTest(
        ...
        editOperationString,
        _doReloadTest,
        _applyEditOpsToOriginalDoc,
        _applyOpsToEmptyDoc,
        resourceMap,
        operationDebugMode);
  }

  private boolean evaluateValidationProblems(CollabTextDocument doc, String documentName)
      throws Exception {
   ...
   */
  protected String roundtripOperationTest(
      CollabTextDocument doc,
      String testFileNameTrunc,
      String testfileNameSuffix,
      String testMethodName,
      String editOperationString,
      boolean doReloadTest,
      boolean applyEditOpsToOriginalDoc,
      boolean applyOpsToEmptyDoc,
      Map<Long, byte[]> resourceMap,
      boolean operationDebugMode) {
    ...
    try {
      ...
      LOG.log(
          Level.INFO,
          "\n\n******************************************\nDocument name: {0}{1}",
          new Object[] {testFileNameTrunc, testfileNameSuffix});
      ...
      String initialTestOpsFile =
          operationTestOutputPath
              + testFileNameTrunc
              + testfileNameSuffix
              + HYPEN
              + testMethodName
              + INITIAL_OPS_SUFFIX;
      String initialTestOps =
          JsonOperationNormalizer.asString(loadingOps, Boolean.TRUE)
              .replace(",{\"name\"", ",\n{\"name\"");
      ResourceUtilities.saveStringToFile(new File(initialTestOpsFile), initialTestOps);

      File initialRefOpsFile =
          new File(
              operationTestReferencePath
                  + testFileNameTrunc
                  + testfileNameSuffix
                  + HYPEN
                  + testMethodName
      ...
      if (applyEditOpsToOriginalDoc) {
        if (editOperationString != null && !editOperationString.isEmpty()) {
          ...
          if (editOperationString.length() > 0
              && !editOperationString.equals("[]")
              && opCount == 0) {
      ...
      if (applyOpsToEmptyDoc) {
        ...
        mHasOdfValidationProblem =
            evaluateValidationProblems(emptyDoc, testFileNameTrunc + testfileNameSuffix);
        emptyDoc.close();
      ...
      mHasOdfValidationProblem =
          evaluateValidationProblems(doc, testFileNameTrunc + testfileNameSuffix);
      doc.close();
      if (doReloadTest) {
        // **********RELOAD
        if (applyEditOpsToOriginalDoc && applyOpsToEmptyDoc) {
          String newOpsReloadedFilePath =
              operationTestOutputPath
                  + testFileNameTrunc
                  + testfileNameSuffix
                  + HYPEN
          ...
          String newOpsReloadedRefPath =
              operationTestReferencePath
                  ...
                  + "new"
                  + RELOADED_OPS_SUFFIX;
          boolean reloadedComparisonFailure_new =
              reloadDocument(odfOutputFile_new, newOpsReloadedFilePath, newOpsReloadedRefPath);

          String orgOpsReloadedFilePath =
              operationTestOutputPath
                  + testFileNameTrunc
                  + testfileNameSuffix
                  + HYPEN
          ...
          String orgOpsReloadedRefPath =
              operationTestReferencePath
                  ...
                  + "org"
                  + RELOADED_OPS_SUFFIX;
          boolean reloadedComparisonFailure_org =
              reloadDocument(odfOutputFile_org, orgOpsReloadedFilePath, orgOpsReloadedRefPath);
          Assert.assertTrue(
              "Verified initial ops: '"
                  + !initialComparisonFailure
                  + "'\n"
                  + "Verified reloaded ops on new File: '"
                  + !reloadedComparisonFailure_new
                  + "',\n"
                  + "Verified reloaded ops on original File: '"
                  + !reloadedComparisonFailure_org
                  + "',\n",
              !initialComparisonFailure
                  & !reloadedComparisonFailure_new
                  & !reloadedComparisonFailure_org);
          ...
          Assert.assertTrue(
              "Verified initial ops: '"
                  + !initialComparisonFailure
                  + "'\n"
                  + "Verified reloaded ops on org File: '"
                  + !reloadedComparisonFailure_org
                  + "',\n",
        ...
        Assert.assertTrue(
            "Verified initial ops: '" + !initialComparisonFailure + "'\n",
            !initialComparisonFailure);
      }
      if (mHasOdfValidationProblem) {
        Assert.fail(
            "Validation Problems with test document: '"
                + testFileNameTrunc
                + testfileNameSuffix
                + "'");
      }
  ...
  private boolean reloadDocument(
      String odfDocumentToBeReloaded, String testOpsTextFilePath, String referenceOpsTextFilePath) {
    boolean reloadedComparisonFailure = false;
    try {
      CollabTextDocument reloadedDoc =
          new CollabTextDocument(new FileInputStream(odfDocumentToBeReloaded));
      ...
      ResourceUtilities.saveStringToFile(
          new File(testOpsTextFilePath),
          JsonOperationNormalizer.asString(reloadedOps, Boolean.TRUE)
              .replace(",{\"name\"", ",\n{\"name\""));
      ...
      if (referenceReloadedOpsFile.exists()) {
        ...
        // Test the known read operations with the above one - using ASCII-compare, e.g. UTF-8
  ...
  /** Returns the test method name to differentiate multiple outputs from the same test file */
  ...
  protected String getEditingOperations(String testFileNameTrunc, String testMethodName) {
    return RoundtripTestHelper.this.getEditingOperations(
        ResourceUtilities.getTestInputFolder()
            + OPERATION_OUTPUT_DIR
            + testFileNameTrunc
            + ODT_SUFFIX
            + "-"
            + testMethodName
            + EDIT_OPS_FILE_SUFFIX);
  }
