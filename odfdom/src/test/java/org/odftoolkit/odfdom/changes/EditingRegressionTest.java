...
/**
 * Loads a document with tables and gathers its operations. Gathered operations will be applied to
 * an empty text document. The changed text document will be saved and reloaded. New gathered
...
public class EditingRegressionTest extends RoundtripTestHelper {
  ...
  /**
   * The document can be round-tripped in a single test, but not with all the other files on the
   * disc (memory problem of test or toolkit?)
   */
  public void BigDocTest() {
    final String SOURCE_FILE_NAME_TRUNC =
        "performance" + File.separator + "OpenDocument-v1.2-os-part1";
    // create the test directories for the "performance" subdirectory
    new File(ResourceUtilities.getTestInputFolder() + OPERATION_OUTPUT_DIR + "performance")
        .mkdirs();
    new File(ResourceUtilities.getTestOutputFolder() + OPERATION_OUTPUT_DIR + "performance")
        .mkdirs();
  ...
  @Test
  /**
   * Creating some JSON by string. Serializing the JSON as string and recreating it. The string
   * should keep the same!
   */
  public void roundtripJSON() {
    final String INPUT_STRING = "2Hello&#x0a; ää http://heise.de \" \t  c:\\path\\dummy";
    // NOTE: The sting is Java encoded/masked so a lot of backslashes appear in the String below,
    // which do not in the output
    ...
    ResourceUtilities.saveStringToFile(
        ResourceUtilities.getTestOutputFile("svanteRoundtrip"), out2);

    String newOps = JsonOperationNormalizer.asString(ro1, Boolean.TRUE);
    System.out.println("Serialized JSON3: norm:\n" + newOps);
    ResourceUtilities.saveStringToFile(
        ResourceUtilities.getTestOutputFile("svanteRoundtrip"), newOps);
  ...
  public void rowDeletionTest() {
    ...
    super.importOnlyRegressionTest(
        SOURCE_FILE_NAME_TRUNC,
        ODT_SUFFIX,
        getTestMethodName(),
        getEditingOperations(SOURCE_FILE_NAME_TRUNC, getTestMethodName()));
  }
  ...
  @Test
  /**
   * String firstEditOperations = "[" + "{\"text\":\"1\",\"start\":[0,0,2,0,0],\"name\":\"" +
   * OP_TEXT + "\"}," + "{\"start\":[0,0,2,0,1],\"name\":\"" + OP_PARAGRAPH_SPLIT + "\"}," +
   * "{\"text\":\"2\",\"start\":[0,0,2,1,0],\"name\":\"" + OP_TEXT + "\"}," +
   * "{\"start\":[0,0,2,1,1],\"name\":\"" + OP_PARAGRAPH_SPLIT + "\"}," /* <text:list
   * text:continue-numbering="true" text:style-name="L1"> <text:list-item> <text:p
   * text:style-name="aaaf1f7">1</text:p> </text:list-item> <text:list-item> <text:p
   * text:style-name="aaaf1f7">2</text:p> </text:list-item> <text:list-item> <text:p
   * text:style-name="aaaf1f7"/> </text:list-item> </text:list>
   *
   * <p>+
   * "{\"start\":[0,0,2,1],\"attrs\":{\"styleId\":null,\"paragraph\":{\"alignment\":null,\"lineHeight\":null,\"fillColor\":null,\"marginTop\":null,\"outlineLevel\":null,\"nextStyleId\":null,\"listStyleId\":null,\"borderBottom\":null,\"listLevel\":null,\"contextualSpacing\":null,\"borderLeft\":null,\"tabStops\":null,\"marginBottom\":null,\"listLabelHidden\":null,\"listStartValue\":null,\"indentRight\":null,\"indentLeft\":null,\"borderTop\":null,\"borderInside\":null,\"indentFirstLine\":null,\"borderRight\":null},\"character\":{\"vertAlign\":null,\"fontName\":null,\"bold\":null,\"strike\":null,\"fillColor\":null,\"color\":null,\"underline\":null,\"italic\":null,\"language\":null,\"fontSize\":null,\"url\":null}},\"name\":
   * \"" + OP_FORMAT + "\"},"
   *
   * <p><text:list text:continue-numbering="true" text:style-name="L1"> <text:list-item> <text:p
   * text:style-name="aaaf1f7">1</text:p> </text:list-item> </text:list> <text:p
   * text:style-name="a81c9de">2</text:p> <text:list text:continue-numbering="true"
   * text:style-name="L1"> <text:list-item> <text:p text:style-name="aaaf1f7"/> </text:list-item>
   * </text:list>
   *
   * <p>+ "{\"text\":\"a\",\"start\":[0,0,2,2,0],\"name\":\"" + OP_TEXT + "\"},"
   *
   * <p>WRONG: <text:list text:continue-numbering="true" text:style-name="L1"> <text:list-item>
   * <text:p text:style-name="aaaf1f7">1</text:p> </text:list-item> </text:list> <text:p
   * text:style-name="a81c9de">a2</text:p> <text:list text:continue-numbering="true"
   * text:style-name="L1"> <text:list-item> <text:p text:style-name="aaaf1f7"/> </text:list-item>
   * </text:list>
   *
   * <p>RIGHT: <text:list text:continue-numbering="true" text:style-name="L1"> <text:list-item>
   * <text:p text:style-name="aaaf1f7">1</text:p> </text:list-item> </text:list> <text:p
   * text:style-name="a81c9de">2</text:p> <text:list text:continue-numbering="true"
   * text:style-name="L1"> <text:list-item> <text:p text:style-name="aaaf1f7">a</text:p>
  ...
  @Test
  /**
   * Insert Row and Column should not show content (no longer cloning XML attribute
   *
   * @office:value)
   */
  ...
  @Test
  /**
   * Enhance Line Height by adding leading ("Bleistreifen" from former print) - the space between
   * lines
   */
  ...
  @Test
  /**
   * ODF space XML element that contains more than two spaces and was not completely deleted,
   * returned the removed spaces as remaining element
   */
  ...
  @Test
  /**
   * Foreign ODF elements could not be cloned. Throwable are now caught and rethrown as
   * filterexception to be found by Admin logging -- adding regression test references
   */
  ...
  @Test
  /**
   * Preceding nested paragraph in paragraph within unknown content parent was erroneously counted
   * as component
   */
  ...
  @Test
  /** ODT - mixed list (numbered/ bullet list): numbering is different in preview /editor */
  ...
  @Test
  /**
   * FIXED: 1) Changing an automatic styles, requires to rename it, if it is being used by others!
   * FIXME: 2) outlineLevel:0 check, copy all attributes and descendent from text:p to text:h and
   * vice versa..
   */
  ...
  @Test
  /** List items across the document with the same style do not influence each other in ODF */
  ...
  @Test
  /** ODT - Reedit odt with List entries: list entries are displayed on the wrong position */
  ...
  public void listMixedFirstLevelTest() {
    ...
    // getInputEditingOperations(SOURCE_FILE_NAME_TRUNC, getTestMethodName()));
  }
  ...
  @Test
  /** ODT - Numbered list: After saving the numbering list contains wrong labels */
  ...
  @Test
  /** ODT - splitParagraph does only split the paragraph and not create a new list item */
  ...
  @Test
  /**
   * ODT - bulletlist: placed text behind a bulletlist is lost, after closing the odt
   *
   * <p>The deletion of one of two paragraphs within a list, resulted into the deletion of both, as
   * the routine believed the second was empty after deletion within.
   */
   ...
   *
   * <p>The width of an image should be limited to its page width (page width - left&rigth margin)
   * QUESTION: Is the height adapted to keep the ratio?
   */
  ...
  public void insertLargeImageTest() {
    ...
    String savedDocumentPath =
        super.roundtripRegressionWithResourcesTest(
            SOURCE_FILE_NAME_TRUNC,
            getTestMethodName(),
            getEditingOperations(SOURCE_FILE_NAME_TRUNC, getTestMethodName()),
            resourceMap,
    ...
    if (pkg == null || !pkg.contains(INTERNAL_IMAGE_PATH)) {
      Assert.fail(
          "The image '"
              + INTERNAL_IMAGE_PATH
              + "' could not be found in the saved document '"
              + savedDocumentPath
              + "'!");
    }
  }
  ...
  @Test
  /**
   * When a listLevel is changed on a paragraph the ancestor elements <text:list> and
   * <text:list-item> have to be inserted/deleted.
   */
  ...
  @Test
  /**
   * Text will be inserted to a paragraph component/element, which has children, but none component
   * child
   */
  ...
  @Test
  /**
   * Split position marks the first component of the second half, therefore Character is Moving the
   * unknown content DANGER: These tests are unable to detect unknown content, like if it is in the
  ...
  @Test
  /** ODT - changing column width: after closing/re-edit the table is set to 100% */
  ...
  @Test
  /**
   * A text style change over style borders mixes all styles. Formating full text of a paragraph of
   * first part already formatted will mix forma over full selection.
   */
  ...
  @Test
  /** Changed bullet symbols are not exported Switching bullet symbols fails in different Level. */
  ...
  @Test
  /**
   * Split ODT paragraph with Page Break is loosing it TestDocument: A and B on two pages First:
   * Adding new paragraph with C behind B on second page Second: Adding page before C, so each
   * letter is on an own page.
   */
  ...
  @Test
  /**
   * Empty spans are being preserved to keep default character format in a paragraph (both
   * characters have to be bold)
   */
  ...
  @Test
  /**
   * Bug 35330 - Delete a numbered list/undo/closed/re-edit: => one entry is too much A
   * text:list-item becomes a text:list-header, when there are multiple paragraph in a list and the
   * first paragraph (the one with the list label) is being deleted and some of the following
   * sibling are remaining (without label). WYSIWYG.
   */
