...
public class XPathTest {
  ...
  /**
   * 1) The first test document "XPathTest-foreignPrefix.odp" uses the prefix "daisy" instead of
   * "office" for ODF XML elements. <daisy:document-content
   * xmlns:daisy="urn:oasis:names:tc:opendocument:xmlns:office:1.0" xmlns:style="ur...
   * <daisy:scripts/> <daisy:automatic-styles>
   *
   * <p>2) The test doc also uses multiple identical namespace prefixes with different URIs.
   * <text:p>Slide <draw:element1 xmlns:draw="urn://dummy-namespace-one"> <draw:element2
   * xmlns:draw="urn://dummy-namespace-two"
   * draw:attribute2="importantValue">dummy</draw:element2></draw:element1>One!!</text:p> It will be
   * tested, if they have been renamed correctly.
   *
   * <p>3) The test doc also uses multiple different namespace prefixes with the similar URIs.
   * Testing getPrefixes(String URI) from the NamespaceContext interface. <text:p>Slide
   * <prefixOne:element1 xmlns:prefixOne="urn://some-test-odfdom-namespace"> <prefixTwo:element2
   * xmlns:prefixTwo="urn://some-test-odfdom-namespace"
   * prefixTwo:attribute2="importantValue">dummy</prefixTwo:element2></prefixOne:element1>Two!!</text:p>
   *
   * <p>4) Having element and attribute without namespace
  ...
  public void testXPathwithAlienNodes() throws Exception {
    try {
      OdfPresentationDocument odpWithSlides =
          OdfPresentationDocument.loadDocument(ResourceUtilities.getAbsolutePath(SOURCE_FILE_1));
      OdfFileDom contentDom = odpWithSlides.getContentDom();

      XPath xpath = contentDom.getXPath();
      // Test scenario 1 - see comment above
      Iterator<String> prefixes =
          contentDom.getPrefixes("urn:oasis:names:tc:opendocument:xmlns:office:1.0");
      ...
      NodeList styleNameAttributes =
          (NodeList) xpath.evaluate(".//*[@style:name]", rootNode, XPathConstants.NODESET);
      ...
      // test if the identical namespace prefixes with different URI have been renamed correctly
      // (earlier draw:element1/draw:element2).
      String attributeWithDuplicatePrefix =
          (String)
              xpath.evaluate(
                  ".//draw__1:element1/draw__2:element2/@draw__2:attribute2",
                  rootNode,
                  XPathConstants.STRING);
      Assert.assertTrue(attributeWithDuplicatePrefix.equals("importantValue"));
      Assert.assertEquals(
          "urn:oasis:names:tc:opendocument:xmlns:office:1.0",
          xpath.getNamespaceContext().getNamespaceURI("office"));
      Assert.assertEquals(
          "http://www.w3.org/1999/xlink", xpath.getNamespaceContext().getNamespaceURI("xlink"));

      String alienAttributeValue =
          (String) xpath.evaluate(".//*/@foreign", rootNode, XPathConstants.STRING);
      LOG.log(
          Level.INFO,
          "The value of the alien attribute is {0}, expected is ''alien:valueOfAlienAttribute''!",
          alienAttributeValue);
      Assert.assertEquals("alien:valueOfAlienAttribute", alienAttributeValue);

      String alienElementValue =
          (String) xpath.evaluate("//text:p/test", rootNode, XPathConstants.STRING);
      LOG.log(
          Level.INFO,
          "The value of the alien element is {0}, expected is ''good''!",
          alienElementValue);
      Assert.assertEquals("good", alienElementValue);
      LOG.log(
          Level.INFO,
          "Amount of @alien:foreignAttribute and @style:name is {0}",
          ((NodeList)
                  xpath.evaluate(
                      ".//*[@alien:foreignAttribute or @style:name]",
                      rootNode,
                      XPathConstants.NODESET))
              .getLength());
      Assert.assertTrue(
          "Amount of @alien:foreignAttribute and @style:name is not 13!!",
          ((NodeList)
                      xpath.evaluate(
                          ".//*[@alien:foreignAttribute or @style:name]",
                          rootNode,
  ...
  public void testCopyForeignSlide() {
    try {
      OdfPresentationDocument targetodp =
          OdfPresentationDocument.loadDocument(ResourceUtilities.getAbsolutePath(SOURCE_FILE_1));
      OdfPresentationDocument sourceodp =
          OdfPresentationDocument.loadDocument(ResourceUtilities.getAbsolutePath(SOURCE_FILE_2));
  ...
  /**
   * This test SHOULD fail, but it isn't. Its source code is identical to the first test in this
   * unit test
   *
   * @throws Exception
   */
  @Test
  public void testXPathIsMissingXLinkButItWillPassBecauseItTheSecondTestInThisUnitTest()
      throws Exception {
    try {
      OdfPresentationDocument odpWithSlides =
          OdfPresentationDocument.loadDocument(ResourceUtilities.getAbsolutePath(SOURCE_FILE_1));
      ...
      NodeList linkNodes =
          (NodeList) xpath.evaluate(".//*[@xlink:href]", node, XPathConstants.NODESET);
  ...
  /**
   * This test checks if the XPath returned by OdfFileDom is correctly aware of namespacess. The
   * NamespaceContext implementation in OdfFileDom is aware of duplicate prefixes, but
   * getNamespaceURI(prefix) in OdfFileDom does not implement it correctly
   *
   * <p>With the patch applied in OdfFileDom these tests pass correctly
   *
   * <p>sample metadata rdf generate from Openoffice 3.3 looks like this : <?xml version="1.0"
   * encoding="utf-8"?> <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
   * <rdf:Description rdf:about="../content.xml#id1366098766"> <ns1:BungeniActionEvent
   * xmlns:ns1="http://editor.bungeni.org/1.0/anx/"></ns1:BungeniActionEvent> <ns2:BungeniSectionID
   * xmlns:ns2="http://editor.bungeni.org/1.0/anx/">Xda+5VC/SQKKG7Bk83a2JA</ns2:BungeniSectionID>
   * <ns3:BungeniSectionType
   * xmlns:ns3="http://editor.bungeni.org/1.0/anx/">Conclusion</ns3:BungeniSectionType>
   * <ns4:hiddenBungeniMetaEditable
   * xmlns:ns4="http://editor.bungeni.org/1.0/anx/">false</ns4:hiddenBungeniMetaEditable>
   * </rdf:Description>
   *
   * @throws Exception
   */
  ...
  public void testXPathDuplicatePrefixForForeignNamespace() throws Exception {
    try {
      ...
      OdfDocument odfDoc =
          OdfDocument.loadDocument(ResourceUtilities.getAbsolutePath(SOURCE_FILE_3));
      ...
      LOG.log(
          Level.INFO,
          "The value of the nsuri is {0} expected ns-uri is ''http://www.w3.org/1999/02/22-rdf-syntax-ns#''",
          rdfNsUri);
      ...
      LOG.log(
          Level.INFO,
          "The value of the nsuri is {0} expected ns-uri is ''http://editor.bungeni.org/1.0/anx/''",
          anxNsUri1);
      ...
      LOG.log(
          Level.INFO,
          "The value of the ns-uri is {0} expected ns-uri is ''http://editor.bungeni.org/1.0/anx/''",
          anxNsUri2);
