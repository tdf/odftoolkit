...
class StyleUtils {
  ...
  /**
   * Returns a property for a (text) node, as it would appear for a user in OpenOffice: First
   * examining the style (and the style's parent styles) and if the property is not found the search
   * continues in the enclosing node, and so on, until an enclosing node is found, whitc has a style
   * (or an inherited style) where the property is defined. Example:
   * findActualStylePropertyValueForNode(textNode, OdfTextProperties.FontName) will give the font
   * name
   *
