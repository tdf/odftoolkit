...
public class OfficeMetaTest {
  ...
  public void setUp() throws Exception {
    ...
    // Sometimes the file was changed (if this test ran earlier), sometimes not. Fixed by doing the
    // test result in a new ODT output file
    File newMetaFile = ResourceUtilities.newTestOutputFile(filenameOut);
    if (!newMetaFile.exists()) {
      doc =
          (OdfTextDocument)
              OdfTextDocument.loadDocument(ResourceUtilities.getAbsolutePath(filename));
    } else {
  ...
  public void updateTemplates() {
    ...
    // The following is only necessary, when the templates in the source repository should be
    // adapted, e.g. before a release
  ...
  private void loadSaveDirFiles(String targetDirectory) {
    try {
      ...
      Assert.assertTrue(
          "No meta:generator could be set as the System property 'odfdom.version' set by the Maven pom.xml was not found!",
          validTest);
  ...
  public void testSetAndGetUserdefinedData() throws Exception {
    ...
    names.add("late");
    // test set
    fMetadata.setUserDefinedData(names.get(0), Value.STRING.toString(), "windy");
    fMetadata.setUserDefinedData(names.get(1), Value.STRING.toString(), "happy");
    fMetadata.setUserDefinedData(names.get(2), Value.BOOLEAN.toString(), "false");
    tearDown();
    ...
    fMetadata.setUserDefinedData(names.get(2), Value.STRING.toString(), "no");
    tearDown();
    ...
    doc =
        (OdfTextDocument)
            OdfTextDocument.loadDocument(
                ResourceUtilities.getAbsolutePath("EmptyDocForMetaTest.odt"));
    metadom = doc.getMetaDom();
    fMetadata = new OdfOfficeMeta(metadom);
  ...
  public void testReadDocumentMeta() throws Exception {
    ...
    textDoc =
        (OdfTextDocument)
            OdfTextDocument.loadDocument(ResourceUtilities.getAbsolutePath("DocForMetaTest.odt"));
