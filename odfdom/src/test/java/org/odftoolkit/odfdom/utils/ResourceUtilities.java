...
public final class ResourceUtilities {
  ...
  public static URI getURI(String relativeFilePath) throws URISyntaxException {
    String filePath =
        "file:" + ResourceUtilities.class.getClassLoader().getResource(relativeFilePath).getPath();
    filePath = Util.toExternalForm(new URI(filePath));
    return new URI(filePath);
  }

  /**
   * The relative path of the test file will be used to determine an absolute path to a temporary
  ...
  /**
   * Relative to the test output directory a test file will be returned dependent on the
   * relativeFilePath provided.
   *
   * @param relativeFilePath Path of the test output resource relative to <code>target/test-classes/
   *     </code>.
