...
import static schema2template.grammar.PuzzlePiece.TYPE_VISITOR;
...
public class MSVExpressionInformation {
  ...
  public MSVExpressionInformation(Expression exp, String schemaFileName) {
    ...
    for (List<Expression> path : paths) {
      if (((MSVExpressionType) path.get(path.size() - 1).visit(TYPE_VISITOR))
    ...
    for (List<Expression> way : waysToChildren) {
      ...
      if (multiples.containsKey(childexp)) {
        ...
        if (!existingCardinality && !newCardinality) {
          ...
          if (commonChoice) {
            // Do nothing
          } // A case which we cannot handle. Both have 1:1 but do not share a common CHOICE
          // element: 1:2 ??? ... 1:3 ???
          else {
            System.err.println(
