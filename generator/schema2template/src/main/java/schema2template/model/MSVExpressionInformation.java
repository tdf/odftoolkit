...
public class MSVExpressionInformation {
  ...
  private static final MSVExpressionVisitorChildren childVisitor =
      new MSVExpressionVisitorChildren();
  ...
  public MSVExpressionInformation(Expression exp) {
    ...
    for (List<Expression> path : paths) {
      if (((MSVExpressionType) path.get(path.size() - 1).visit(typeVisitor))
          == MSVExpressionType.STRING) {
        mCanHaveText = true;
        break;
      }
    }
  ...
  private void registerChildrenMaxCardinalities(List<List<Expression>> waysToChildren) {
    ...
    for (List<Expression> way : waysToChildren) {
      ...
      // is this a multiple of an already existing parent -> ... -> child path? (i.e.
      // parent1==parent2 && child1==child2)
      if (multiples.containsKey(childexp)) {

        // so we have a parent -> ... -> child multiple. Read the max cardinality of the existing
        // path.
        Boolean existingCardinality = multiples.get(childexp);

        // so we have a parent -> ... -> child multiple. Is there a common CHOICE element on both
        // paths?
        ...
        // This restriction is needed because MSV does some optimization. (Example for no pattern
        // match):
        ...
        if (!existingCardinality.equals(newCardinality)) {
          // A case which we cannot handle. You have a choice between a definition which allows only
          // 1 occurence and another which allows N occurences
          if (commonChoice) {
            System.err.println(
        ...
        if (!existingCardinality && !newCardinality) {
          ...
          // A case which we cannot handle. Both have 1:1 but do not share a common CHOICE element:
          // 1:2 ??? ... 1:3 ???
          else {
            System.err.println(
  ...
  private static void buildPaths(
      MSVExpressionVisitorChildren visitor, List<List<Expression>> paths) {
  ...
  /**
   * Gets all paths leading from this.getExpression() to exp (but not necessarily ending in exp). A
   * path always starts with this.getExpression() and ends in someChildDefinition.getExpression().
   *
   * @param exp The MSV Expression. If you use this.getExpression() you get all paths starting from
   *     this.getExpression(). If you use someChildDefinition.getExpression() you get all paths from
  ...
  public boolean isMandatory(Collection<Expression> equallyNamedChildren) {
    ...
    for (Expression exp : equallyNamedChildren) {
      if (!(exp instanceof NameClassAndExpression)) {
        throw new RuntimeException(
            "ExpressionInformation: Cannot determine isMandatory for Expression other than ELEMENT and ATTRIBUTE");
      }
      // Twins == Same parent (first element in path), same child (last element in path)
      // Contract: For all paths containing an Element or Attribute child this child is always the
      // last Expression of the path
    ...
    for (List<Expression> path : twins) {
      for (int s = 0; s < path.size(); s++) {
        ...
        if (step instanceof ChoiceExp && !visitedChoices.contains(step)) {
          ...
          // we need to find two paths to CHILD ('path' and another one from the twins) _really_
          // (s.a.) sharing this choice. Otherwise child is optional.
          int sharingPaths = 0;

          for (List<Expression> otherPath : choiceInPaths) {
            if (otherPath.size() > s
                && path.subList(0, s + 1).equals(otherPath.subList(0, s + 1))) {
              sharingPaths++;
              if (sharingPaths == 2) {
                break;
              }
