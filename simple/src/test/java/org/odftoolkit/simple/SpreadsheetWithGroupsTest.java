...
public class SpreadsheetWithGroupsTest {
  ...
  private static final String TEST_FILE_NAME = "SpreadsheetWithGroupsTest.ods";
      ...
      for (int columnIndex = 0; columnIndex < sheet.getColumnCount(); columnIndex++) {
        for (int rowIndex = 0; rowIndex < sheet.getRowCount(); rowIndex++) {
          if (rowIndex == 0 && columnIndex == 2)
            Logger.getLogger(SpreadsheetWithGroupsTest.class.getName())
                .info(String.format("NEW-Value%d-%d", rowIndex, columnIndex));
  ...
  /**
   * Tests the usual use case of a row iterator, a call to {@code hasNext()} followed by exactly one
   * call of {@code next()}.
   *
   * <pre><code>
  ...
  /**
   * Tests if the iterator works even if multiple calls of {@code hasNext()} per row occur.
   *
   * <p>Possible use case:
   *
   * <pre><code>
  ...
  /**
   * Assure that the spreadsheet contains the expected data.
   *
  ...
  private void assertEquals(String message, List<List<String>> expectedData, Table sheet) {
    ...
    Assert.assertEquals(
        String.format("%sRow count must be equal", message),
        expectedData.size(),
        sheet.getRowCount());
  ...
  private void assertEquals(String message, int rowIndex, List<String> expectedRow, Row row) {
    Assert.assertNotNull(String.format("%sRow [%d] must exist", message, rowIndex), row);
    Assert.assertEquals(
  ...
  private void assertEquals(
      String message, int rowIndex, int columnIndex, String expectedCellValue, Cell cell) {
    Assert.assertNotNull(
        String.format("%sCell [%d] of row [%d] must exist", message, columnIndex, rowIndex), cell);

    Assert.assertEquals(
        String.format(
            "%sContent of cell [%d] of row [%d] must match", message, columnIndex, rowIndex),
        expectedCellValue,
        cell.getDisplayText());
  }
}
