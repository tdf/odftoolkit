...
public class SpreadsheetTemplateTest {
  ...
  public void testLoadingASpreadsheetTemplate() throws Exception {
    Document document =
        Document.loadDocument(this.getClass().getResourceAsStream(TEST_SPREADSHEET_TEMPLATE));
    Assert.assertEquals(
        Document.OdfMediaType.SPREADSHEET_TEMPLATE.getMediaTypeString(),
  ...
  public void testSavingASpreadsheetTemplate() throws Exception {
    Document document =
        Document.loadDocument(this.getClass().getResourceAsStream(TEST_SPREADSHEET_TEMPLATE));
    File destination =
        File.createTempFile("simple-test", ".ots", ResourceUtilities.getTempTestDirectory());
  ...
  public void testSwitchingOdfSpreadsheetDocument() throws Exception {
    ...
    Assert.assertEquals(
        Document.OdfMediaType.SPREADSHEET.getMediaTypeString(),
        document.getPackage().getMediaTypeString());
  }
}
