...
public class TextNavigationTest {
  ...
  private void matchLines(TextNavigation search, Node rootNode) {
    ...
    Assert.assertNotNull(match);
    Assert.assertEquals(
        "different span in one single word delete indeed", TextExtractor.getText(match));

    match = (OdfElement) search.getNextMatchElement(match);
    Assert.assertNotNull(match);
    Assert.assertEquals(
        "Hello delete this word delete true delete  indeed", TextExtractor.getText(match));
    ...
    try {
      PresentationDocument document =
          PresentationDocument.loadDocument(
              ResourceUtilities.getAbsolutePath(NAVIGATION_ODFELEMENT_FILE));
