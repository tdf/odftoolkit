...
public class GraphicsDocumentTest {
  ...
  public void testLoadingAChartTemplate() throws Exception {
    try {
      GraphicsDocument graphiDoc =
          GraphicsDocument.loadDocument(ResourceUtilities.getAbsolutePath(TEST_FILE));
  ...
  public void testLoadDocumentPath() throws Exception {
    try {
      ...
      Assert.assertEquals(
          "application/vnd.oasis.opendocument.graphics-template", gdoc.getMediaTypeString());
    } catch (Exception e) {
      LOG.log(Level.SEVERE, e.getMessage(), e);
      Assert.fail(e.getMessage());
    }
