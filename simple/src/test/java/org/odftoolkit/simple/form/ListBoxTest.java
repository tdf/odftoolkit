...
public class ListBoxTest {
  private static final FrameRectangle listBoxRtg =
      new FrameRectangle(0.5752, 0.1429, 2.3307, 0.8398, SupportedLinearMeasure.IN);
  ...
  public static void createForm() {
    try {
      ...
      // list box1
      FormControl listBox = form.createListBox(doc, listBoxRtg, "list1", true, false);
      ...
      listBox = form.createListBox(para, listBoxRtg, "list2", false, true);
      ...
      ((ListBox) listBox).setListSource("SELECT \"Publisher\", \"Identifier\" FROM \"biblio\"");
      ...
      doc.save(ResourceUtilities.newTestOutputFile("TestCreateListBox.odt"));

    } catch (Exception e) {
      Logger.getLogger(ListBoxTest.class.getName()).log(Level.SEVERE, null, e);
      Assert.fail();
    }
  }
    ...
    try {
      TextDocument textDoc =
          TextDocument.loadDocument(
              ResourceUtilities.getTestResourceAsStream("TestCreateListBox.odt"));
      ...
      Assert.assertEquals("list" + (++count), listBox.getName());
      Assert.assertEquals(
          "SELECT \"Publisher\", \"Identifier\" FROM \"biblio\"", listBox.getListSource());
      Assert.assertEquals(FormListSourceType.SQL, listBox.getListSourceType());
      ...
      Assert.assertEquals(
          textDoc
              ...
              .getCellByPosition("B1")
              .getParagraphByIndex(0, true)
              .getOdfElement(),
          listBox.getDrawControl().getContainerElement());
  ...
  public void testRemoveListBox() {
    try {
      ...
      textDoc.save(ResourceUtilities.newTestOutputFile("TestRemoveListBox.odt"));
    } catch (Exception e) {
      Logger.getLogger(ListBoxTest.class.getName()).log(Level.SEVERE, null, e);
      Assert.fail();
    }
  ...
  public void testSetListBoxRectangle() {
    try {
      ...
      textDoc.save(ResourceUtilities.newTestOutputFile("TestSetListBoxRectangle.odt"));
    } catch (Exception e) {
      Logger.getLogger(ListBoxTest.class.getName()).log(Level.SEVERE, null, e);
      Assert.fail();
    }
  ...
  public void testSetAnchorType() {
    try {
      ...
      Assert.assertEquals(
          VerticalRelative.BASELINE, graphicPropertiesForWrite.getVerticalRelative());
      Assert.assertEquals(
          FrameVerticalPosition.TOP, graphicPropertiesForWrite.getVerticalPosition());

      textDoc.save(ResourceUtilities.newTestOutputFile("TestSetListBoxAnchorType.odt"));
    } catch (Exception e) {
      Logger.getLogger(ListBoxTest.class.getName()).log(Level.SEVERE, null, e);
      Assert.fail();
    }
  ...
  public void testSetProperties() {
    try {
      ...
      textDoc.save(ResourceUtilities.newTestOutputFile("TestSetListBoxProperties.odt"));
    } catch (Exception e) {
      Logger.getLogger(ListBoxTest.class.getName()).log(Level.SEVERE, null, e);
      Assert.fail();
    }
