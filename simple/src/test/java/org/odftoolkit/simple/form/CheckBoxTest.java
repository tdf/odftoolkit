...
public class CheckBoxTest {
  private static final FrameRectangle CHECK_BOX_RTG =
  ...
  public void createForm(String testFileName) {
    try {
      ...
      para.applyHeading();
      CheckBox checkbox =
          (CheckBox) form.createCheckBox(para, CHECK_BOX_RTG, "CheckBox 2", "CheckBox 2", "2");

      // checkbox 3
      para = doc.addParagraph("Insert checkbox3 here, as_char.");
      checkbox =
          (CheckBox) form.createCheckBox(para, CHECK_BOX_RTG, "CheckBox 3", "CheckBox 3", "3");
      ...
      form.createCheckBox(para, CHECK_BOX_RTG, "CheckBox 4", "CheckBox 4", "4");
      doc.save(ResourceUtilities.newTestOutputFile(testFileName));

      LOG.log(Level.INFO, "Creation of ''{0}'' succeded!", testFileName);
    } catch (Exception e) {
      Logger.getLogger(CheckBoxTest.class.getName()).log(Level.SEVERE, null, e);
    ...
    try {
      ...
      TextDocument textDoc =
          TextDocument.loadDocument(
              ResourceUtilities.getTestResourceAsStream(testFileName + ".odt"));
  ...
  public void testRemoveCheckBox() {
    try {
      ...
      textDoc.save(ResourceUtilities.newTestOutputFile(testFileName + "_OUT.odt"));
    } catch (Exception e) {
      Logger.getLogger(CheckBoxTest.class.getName()).log(Level.SEVERE, null, e);
      Assert.fail("Failed with " + e.getClass().getName() + ": '" + e.getMessage() + "'");
    }
  ...
  public void testCheckedState() {
    try {
      ...
      Assert.assertNotNull(find);
      // validate
      Assert.assertEquals(FormCheckboxState.CHECKED, find.getCurrentState());

      find.setCurrentState(FormCheckboxState.UNKNOWN);
      Assert.assertEquals(FormCheckboxState.UNKNOWN, find.getCurrentState());
