...
public class ChartTemplateTest {
  ...
  public void testLoadingAChartTemplate() throws Exception {
    Document document =
        Document.loadDocument(this.getClass().getResourceAsStream(TEST_CHART_TEMPLATE));
    Assert.assertEquals(
        Document.OdfMediaType.CHART_TEMPLATE.getMediaTypeString(), document.getMediaTypeString());
  }
  ...
  public void testSavingAChartTemplate() throws Exception {
    Document document =
        Document.loadDocument(this.getClass().getResourceAsStream(TEST_CHART_TEMPLATE));
    File destination =
        File.createTempFile("simple-test", ".otc", ResourceUtilities.getTempTestDirectory());
    document.save(destination);

    // load again
    Document loadedDocument = Document.loadDocument(destination);
    Assert.assertEquals(
        Document.OdfMediaType.CHART_TEMPLATE.getMediaTypeString(),
  ...
  public void testSwitchingOdfChartDocument() throws Exception {
    ...
    Assert.assertEquals(
        Document.OdfMediaType.CHART.getMediaTypeString(),
        document.getPackage().getMediaTypeString());
  }
  ...
  public void testGetOdfMediaType() throws Exception {
    ...
    try {
      ...
      Assert.assertEquals(
          "application/vnd.oasis.opendocument.chart", chartType.getMediaTypeString());
  ...
  public void testGetMediaTypeString() throws Exception {
    try {
      ChartDocument.OdfMediaType odfMediaTypeChart =
          ChartDocument.OdfMediaType.valueOf(
              ChartDocument.OdfMediaType.class, ChartDocument.OdfMediaType.CHART.name());
