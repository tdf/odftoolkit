...
public class OfficeMetaTest {
    ...
    doc =
        (TextDocument)
            TextDocument.loadDocument(
                ResourceUtilities.getTestResourceAsStream("EmptyDocForMetaTest.odt"));
    metadom = doc.getMetaDom();
    fMetadata = new Meta(metadom);
  ...
  public void testReadDocumentMeta() throws Exception {
    ...
    textDoc =
        (TextDocument)
            TextDocument.loadDocument(
                ResourceUtilities.getTestResourceAsStream("DocForMetaTest.odt"));
  ...
  private void writeAndReload() throws Exception {
    File persistedDocument =
        File.createTempFile(getClass().getName(), ".odt", ResourceUtilities.getTempTestDirectory());
