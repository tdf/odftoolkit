...
public class SlideTest {
   ...
   * <b>Method</b> 1)By using PresentationDocument.getSlideCount() to Get the number of the slides
   * in this presentation. 2)According to the slide count, get a slide at the specific position of
   * the loaded presentation document using PresentationDocument.getSlideByIndex(int index), and the
   * param "index" should be limited with the slide count. Use OdfDrawPage.getSlideIndex() method to
   * verify the returned slide. 3)PresentationDocument.getSlideByName(String name) 4)Using
   * dfPresentationDocument.getSlides() to get the slide iterator <b>Covered Element</b>
   * &lt;office:presentation&gt;,&lt;draw:page&gt; <b>Note</b> PresentationDocument.getContentRoot
   * override Document.getContentRoot method will return OfficePresentationElement, rather than
   * OdfOfficePresentation because of the inheritance relationship broken up between dom and doc
   * element And because OdfOfficePresentation is useless, so I delete this class <b>Code Coverage
   * Result</b> 1)can not catch Exception because it is only used to catch the exception thrown by
   * getContentRoot()
   */
  @Test
    ...
    try {
      doc =
          PresentationDocument.loadDocument(
              ResourceUtilities.getTestResourceAsStream(TEST_PRESENTATION_FILE_MAIN));
  ...
  public void testSlideName() {
    ...
    try {
      doc.save(
          ResourceUtilities.newTestOutputFile(TEST_PRESENTATION_FILE_OUT_PREFIX + "SlideName.odp"));
    } catch (Exception e) {
      LOG.log(Level.SEVERE, e.getMessage(), e);
      Assert.fail("Failed with " + e.getClass().getName() + ": '" + e.getMessage() + "'");
    }
   ...
   *
   * <p><b>Precondition</b> Load Presentation File at test\resources\performance\Presentation1.odp
   * <b>Method</b> 1)By using PresentationDocument.deleteSlideByIndex(int index) to delete the slide
   * at the specified position which is present by param "index", then index of the slide which
   * after the delete slide will change. 2)Delete slide with the specified name by using
   * PresentationDocument.deleteSlideByName(String name) <b>PostCondition</b> Save the modified
  ...
  public void testDeleteSlide() {
    try {
      ...
      doc2 =
          PresentationDocument.loadDocument(
              ResourceUtilities.getTestResourceAsStream(TEST_PRESENTATION_FILE_ANOTHER));
    ...
    try {
      doc.save(
          ResourceUtilities.newTestOutputFile(TEST_PRESENTATION_FILE_OUT_PREFIX + "Delete.odp"));
      doc2.save(
          ResourceUtilities.newTestOutputFile(TEST_PRESENTATION_FILE_OUT_PREFIX + "Delete2.odp"));
    } catch (Exception e) {
      LOG.log(Level.SEVERE, e.getMessage(), e);
      Assert.fail("Failed with " + e.getClass().getName() + ": '" + e.getMessage() + "'");
    }
   ...
   *
   * <p><b>Precondition</b> Load Presentation File at test\resources\performance\Presentation1.odp
   * <b>Method</b> PresentationDocument.newSlide(int index, String name, OdfDrawPage.SlideLayout
  ...
  public void testNewSlide() {
    try {
      ...
      doc.newSlide(
          3,
          Slide.SlideLayout.TITLE_PLUS_TEXT.toString(),
          Slide.SlideLayout.enumValueOf("title_text"));
   ...
   * PresentationDocument.moveSlide(int current, int destination) to move a slide at the current
   * position to the destination position. 2)PresentationDocument.copySlide(int source, int dest,
   ...
   * to copy a slide which locates at "srcIndex" position of the "srcDoc" presentation document and
   * insert it to the current presentation document at "destIndex" position. <b>Postcondition</b>
   * Save the modified document at SlideResultCopyForeignSlide.odp <b>Covered Element</b> 1)
   * &lt;office:presentation&gt;,&lt;draw:page&gt; 2) All the element that contains "xlink:href"
   * attribute, such as &lt;draw:object&gt;, &lt;draw:image&gt;, etc. 3) Each style definition
   * element of &lt;draw:page&gt; and its all child elements such as layout style, master page
   * style, object style and text style the style definition element must contain "style:name" or
   * "draw:name" attribute, include &lt;style:mater-page
   * &gt;,&lt;style:page-layout&gt;,&lt;style:presentation-page-layout&gt;, &lt;style:style&gt;,
   * &lt;text:list-style&gt;, &lt;number:time-style&gt;, &lt;number:date-style&gt;,
   ...
   * <p><b>Precondition</b> Load Presentation File at test\resources\performance\Presentation1.odp &
   * test\resources\performance\Presentation2.odp <b>Method</b> Using
   * PresentationDocument.appendPresentation(PresentationDocument aDoc) to append the specified
   * presentation to the current presentation. <b>Postcondition</b> Save the modified document at
   * SlideResultMerge.odp <b>Covered Element</b> Same with <code>CopyForeignSlide</code> method,
  ...
  public void testAppendPresentation() {
    ...
    try {
      doc2.save(
          ResourceUtilities.newTestOutputFile(TEST_PRESENTATION_FILE_OUT_PREFIX + "Merge2.odp"));
    } catch (Exception e) {
      LOG.log(Level.SEVERE, e.getMessage(), e);
      Assert.fail("Failed with " + e.getClass().getName() + ": '" + e.getMessage() + "'");
    }
  ...
  public void testAnotherMergeDoc() {
    try {
      ...
      doc.save(
          ResourceUtilities.newTestOutputFile(TEST_PRESENTATION_FILE_OUT_PREFIX + "Merge1.odp"));
    } catch (Exception e) {
      LOG.log(Level.SEVERE, e.getMessage(), e);
      Assert.fail("Failed with " + e.getClass().getName() + ": '" + e.getMessage() + "'");
    }
  ...
  public void testCopyThreeDoc() {
    try {
      ...
      PresentationDocument testdoc1 =
          PresentationDocument.loadDocument(
              ResourceUtilities.getTestResourceAsStream(TEST_PRESENTATION_DOCUMENT1));
      // testdoc1 contain "dp1" for draw page style
      PresentationDocument testdoc2 =
          PresentationDocument.loadDocument(
              ResourceUtilities.getTestResourceAsStream(TEST_PRESENTATION_DOCUMENT2));
      // testdoc1 contain "dp1" for draw page style
      PresentationDocument testdoc3 =
          PresentationDocument.loadDocument(
              ResourceUtilities.getTestResourceAsStream(TEST_PRESENTATION_DOCUMENT3));
  ...
  public void testAddRemoveIterateSubList() {
    try {
      ...
      ListDecorator imageDecorator =
          new ImageDecorator(doc, ResourceUtilities.getURI("image_list_item.png"));
  ...
  public void testAddTable() {
    try {
      ...
      TableContainer container =
          doc.newSlide(2, "title_plus_text", Slide.SlideLayout.TITLE_PLUS_TEXT);
      ...
      doc =
          PresentationDocument.loadDocument(
              ResourceUtilities.getTestResourceAsStream("SlideTableOutput.odp"));
  ...
  public void testNewSlideTitle_subtitle() {
    try {
      ...
      doc =
          PresentationDocument.loadDocument(
              ResourceUtilities.getTestResourceAsStream("slidetitlelayout.odp"));
      ...
      if (slide.equals(slide1)) Assert.assertEquals(slide, slide1);
  ...
  public void testNewSlideTitle_left_chart_right_outline() {
    try {
      ...
      Slide slidepre =
          doc.newSlide(0, "testlayout", Slide.SlideLayout.TITLE_LEFT_CHART_RIGHT_OUTLINE);
  ...
  public void testNewSlideTitle_plus_3_object() {
    try {
      ...
      Node nodetitle3 =
          slide1.getOdfElement().getFirstChild().getNextSibling().getNextSibling().getNextSibling();
