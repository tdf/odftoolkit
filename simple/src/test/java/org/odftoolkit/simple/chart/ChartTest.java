...
public class ChartTest {
  ...
  // NEW OUTPUT DOCS
  private static final String TEST_SPREADSHEET_WITH_APPENDED_CHART =
      "SpreadsheetWithAppendedChart.ods";

  @Test
  public void testCopyChart() {
    try {
      SpreadsheetDocument emptySpreadsheetDocument = SpreadsheetDocument.newSpreadsheetDocument();
      SpreadsheetDocument chartSpreadSheetDocument =
          (SpreadsheetDocument)
              Document.loadDocument(
                  ResourceUtilities.getTestResourceAsStream(TEST_SPREADSHEET_WITH_CHART));
      ...
      // Make sure no duplicate manifest entries are after the insertion of the sheet with its
      // dependencies..
      checkManifestEntryDuplication(emptySpreadsheetDocument);

      emptySpreadsheetDocument.save(
          ResourceUtilities.newTestOutputFile(TEST_SPREADSHEET_WITH_APPENDED_CHART));
      SpreadsheetDocument reloadedSpreadsheetWithChart =
          (SpreadsheetDocument)
              Document.loadDocument(
                  ResourceUtilities.newTestOutputFile(TEST_SPREADSHEET_WITH_APPENDED_CHART));
  ...
  private static void checkManifestEntryDuplication(Document testDocument)
    ...
    NodeList manifestEntries =
        (NodeList)
            xpath.evaluate("./*/@manifest:full-path", manifestRootElement, XPathConstants.NODESET);
      ...
      Chart chart =
          new Chart(
              doc.getContentDom()
                  ...
                  .newOfficeChartElement()
                  .newChartChartElement(null),
              null);
  ...
  public void testCreateChartWithLocalData() {
    ...
    try {
      ...
      Assert.assertEquals(
          doc1.getMediaTypeString(), Document.OdfMediaType.SPREADSHEET.getMediaTypeString());
  ...
  public void testCreateChartWithCellRange() {
    try {
      ...
      rect.height = 12000;
      doc.createChart(
          barTitle, doc, CellRangeAddressList.valueOf("A.A1:A.E21"), true, true, true, rect);
      doc.createChart(
          barTitle, doc, CellRangeAddressList.valueOf("A.A1:A.E21"), true, true, false, rect);
      doc.createChart(
          barTitle, doc, CellRangeAddressList.valueOf("A.B1:A.E21"), true, false, true, rect);
      doc.createChart(
          barTitle, doc, CellRangeAddressList.valueOf("A.B1:A.E21"), true, false, false, rect);
      doc.createChart(
          barTitle, doc, CellRangeAddressList.valueOf("A.A2:A.E21"), false, true, true, rect);
      doc.createChart(
          barTitle, doc, CellRangeAddressList.valueOf("A.A2:A.E21"), false, true, false, rect);
      doc.createChart(
          barTitle, doc, CellRangeAddressList.valueOf("A.B2:A.E21"), false, false, true, rect);
      doc.createChart(
          barTitle, doc, CellRangeAddressList.valueOf("A.B2:A.E21"), false, false, false, rect);
      Chart chartID8 =
          doc.createChart(
              areaTitle,
              doc,
              CellRangeAddressList.valueOf("A.B2:A.E21"),
              false,
              false,
              false,
              rect);
  ...
  public void testRemoveChart() {
    ...
    try {
      SpreadsheetDocument doc =
          (SpreadsheetDocument)
              Document.loadDocument(ResourceUtilities.getTestResourceAsStream(CHART_FILE2));
