...
public class TableRowColumnTest {
      ...
      odsdoc =
          (SpreadsheetDocument)
              SpreadsheetDocument.loadDocument(
                  ResourceUtilities.getTestResourceAsStream(filename + ".ods"));
      odtdoc =
          (TextDocument)
              TextDocument.loadDocument(
                  ResourceUtilities.getTestResourceAsStream(odtfilename + ".odt"));
    } catch (Exception e) {
  ...
  public void testSetSize() {
    ...
    for (int i = 0; i < table3.getRowCount(); i++) {
      ...
      double inValue =
          Math.round(roundingFactor * oldHeight / Unit.INCH.unitInMillimiter()) / roundingFactor;
    ...
    for (int i = 0; i < table1.getColumnCount(); i++) {
      ...
      double roundingFactor = 10000.0;
      double inValue =
          Math.round(roundingFactor * oldWidth / Unit.INCH.unitInMillimiter()) / roundingFactor;
  ...
  /**
   * The DecimalFormat will create the depicted "000,8571" with the comma as separator if used in a
   * German Locale.
   *
   * <p>Within org.odftoolkit.simple.table.Column.setWith(long) there is a getWith() call which
   * ultimately calls org.odftoolkit.odfdom.type.Length.parseDouble(), where Double.valueOf(String
   * s) is used to transform the String into a Double. This method uses "." as separator. Hence the
   * Exception.
   */
  @Test
  ...
  public void testSplitRepeatedColumns() {
    ...
    String columnWidth =
        table5
            .getColumnByIndex(1)
            .maColumnElement
            .getProperty(OdfTableColumnProperties.ColumnWidth);
  ...
  public void testTableColumnWidth() {
    ...
    try {
      ...
      textDocument.addParagraph(
          "fifth table (space of -1 cm to the left and right space of -1.5cm)");
      ...
      textDocument.addParagraph(
          "Above, other table with top space of 0.25cm and bottom spacin of 0.5cm");

      textDocument.addParagraph(null);
      textDocument.addParagraph(null);
      textDocument.addParagraph(
          "below, table in footer Standard (bug with getWidth, but the widths are good)");
  ...
  private void setColumnsWidth(Table table, long[] columnsWidth) {
    ...
    for (int i = 0; i < columnsWidth.length; i++) {
      ...
      table
          .getCellByPosition(i, 0)
          .setStringValue(columnsWidth[i] + "/" + vListColumns.get(i).getWidth());
    }
  }
