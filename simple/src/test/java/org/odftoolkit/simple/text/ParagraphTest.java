...
public class ParagraphTest {
  ...
  private static final String[][] ELEMENT_RESULT = {
    {"nospace"},
    {"one space"},
    {"two ", "*s1", "spaces"},
    {"three ", "*s2", "spaces"},
  ...
  private TextDocument readParagraphDocument() throws Exception {
    return (TextDocument)
        TextDocument.loadDocument(ResourceUtilities.getTestResourceAsStream("Paragraph.odt"));
  }
    ...
    try {
      ...
      TextDocument textdoc =
          TextDocument.loadDocument(
              ResourceUtilities.getTestResourceAsStream("TestParagraphHyperlink.odt"));
      Iterator<TextHyperlink> linkList =
          textdoc.getParagraphByReverseIndex(1, true).getHyperlinkIterator();
      if (linkList.hasNext()) {
  ...
  public void testGetTextContentWithLibreOfficeFile() throws FileNotFoundException, Exception {
    TextDocument document =
        TextDocument.loadDocument(ResourceUtilities.getAbsolutePath("Bell.odt"));
