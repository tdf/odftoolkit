...
public class SectionTest {
  ...
  public void testCopyPasteResource() {
    try {
      ...
      DrawImageElement oldImage =
          (DrawImageElement)
              xpath.evaluate(".//draw:image", theSec.getOdfElement(), XPathConstants.NODE);
      DrawImageElement newImage =
          (DrawImageElement)
              xpath.evaluate(".//draw:image", newSec.getOdfElement(), XPathConstants.NODE);
      ...
      while (filePaths.hasNext()) {
        ...
        if (path.startsWith(imagePathPrefix) && path.length() > imagePathPrefix.length()) count++;
        if (count > 2) break;
      }
      ...
      newImage =
          (DrawImageElement)
              xpath.evaluate(".//draw:image", newSec.getOdfElement(), XPathConstants.NODE);
      if (oldImage.getXlinkHrefAttribute().equals(newImage.getXlinkHrefAttribute())) Assert.fail();
    ...
    try {
      ...
      newDoc =
          TextDocument.loadDocument(
              ResourceUtilities.getTestResourceAsStream("NewNewSections.odt"));
  ...
  public void testAppendNewSection() {
    try {
      ...
      Assert.assertEquals(
          section.getParagraphByIndex(0, true).getTextContent(), "Here's a section.");
  ...
  public void testSetProtectionKeyDigestProvider() {
    try {
      ...
      Assert.assertEquals(
          "http://www.w3.org/2000/09/#md5", section.getProtectionKeyDigestAlgorithm());
      ...
      Assert.assertEquals(
          "http://www.w3.org/2000/09/xmldsig#sha1", section.getProtectionKeyDigestAlgorithm());
  ...
  public void testGetParagraphIterator() {
    try {
      ...
      while (iter.hasNext()) {
        ...
        if ("paragraph".equals(parai.getTextContent())) flag = true;
      }
  ...
  public void testAddTable() {
    try {
      ...
      Assert.assertEquals(
          "A1",
          Table.getInstance((TableTableElement) odfEle.getLastChild())
              .getCellByPosition("A1")
              .getDisplayText());
  ...
  /**
   * tests the Function 'updateXMLIds' indirectly by calling TextDocument.appendSection() The
   * function updateXMLIds has to change the xml:id of a list element and its reference
   * text:continue-list in the same way.
   *
   * <p>The SectionContinuedList.odt includes one section. This section includes three lists, the
   * third list continues the first one.
   *
   * <p>In this test we copy the section and remove the source section. Then we investigate the
   * xml:id and text:continue-list attribute of the copied section. The attribute values must be
  ...
  public void testUpdateXMLIds() {
    try {
      TextDocument doc =
          TextDocument.loadDocument(
              ResourceUtilities.getTestResourceAsStream("SectionContinuedList.odt"));
      Iterator<Section> sections = doc.getSectionIterator();
