...
public class ListTest {
  ...
  private static final String SAMPLE_LIST_DOCUMENT = "testList.odt";
      ...
      TextDocument odtdoc =
          (TextDocument)
              TextDocument.loadDocument(
                  ResourceUtilities.getTestResourceAsStream(SAMPLE_LIST_DOCUMENT));
  ...
  private void addOutLineItem(
      List list, String itemPrefix, int listlevel, ListDecorator decorator) {
    if (listlevel == 0) {
      return;
    } else {
  ...
  public void testInsertListBefore() {
    ...
    try {
      ...
      List firstPara =
          List.appendListBefore(
              doc, brotherParagraph, "Header of first List", new SquareDecorator(doc));
  ...
  private TextDocument readListTestDocument() throws Exception {
    return (TextDocument)
        TextDocument.loadDocument(ResourceUtilities.getTestResourceAsStream("ListTest.odt"));
  }
}
