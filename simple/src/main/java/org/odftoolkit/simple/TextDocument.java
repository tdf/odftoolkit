...
public class TextDocument extends Document
  ...
  public static TextDocument newTextDocument() throws Exception {
    return (TextDocument)
        Document.loadTemplate(EMPTY_TEXT_DOCUMENT_RESOURCE, Document.OdfMediaType.TEXT);
  }
  ...
  public static TextDocument newTextTemplateDocument() throws Exception {
    return (TextDocument)
        Document.loadTemplate(EMPTY_TEXT_DOCUMENT_RESOURCE, Document.OdfMediaType.TEXT_TEMPLATE);
  }
  ...
  public static TextDocument newTextMasterDocument() throws Exception {
    TextDocument doc =
        (TextDocument)
            Document.loadTemplate(EMPTY_TEXT_DOCUMENT_RESOURCE, Document.OdfMediaType.TEXT_MASTER);
  ...
  public static TextDocument newTextWebDocument() throws Exception {
    TextDocument doc =
        (TextDocument)
            Document.loadTemplate(EMPTY_TEXT_DOCUMENT_RESOURCE, Document.OdfMediaType.TEXT_WEB);
   ...
   * TextDocument, the InputStream is cached. This usually takes more time compared to the other
   ...
   *
   * <p>TextDocument relies on the file being available for read access over the whole lifecycle of
   * TextDocument.
   *
   * <p>If the resource stream is not a ODF text document, ClassCastException might be thrown.
   *
  ...
  protected TextDocument(
      OdfPackage pkg, String internalPath, TextDocument.OdfMediaType odfMediaType) {
   ...
   * @throws Exception if the file DOM could not be created.
   * @deprecated As of Simple version 0.5, replaced by <code>addParagraph(String text)</code>
   ...
   * @return The new paragraph
   * @throws Exception if the file DOM could not be created.
   * @deprecated As of Simple version 0.5, replaced by <code>
   *     Paragraph.newParagraph(ParagraphContainer)</code>
   ...
   * @throws Exception if the file DOM could not be created.
   * @deprecated As of Simple version 0.5, replaced by <code>
   *     Paragraph.appendTextContent(String content)</code>
   ...
   *
   * <p>The IDs and names in this section would be changed to ensure unique.
   *
   * <p>If the section contains a linked resource, <code>isResourceCopied</code> would specify
   * whether the linked resource would be copied or not, when the copy and append happens within a
   * same document.
   *
   * @param section - the section object
   * @param isResourceCopied - whether the linked resource is copied or not.
   */
  public Section appendSection(Section section, boolean isResourceCopied) {
    boolean isForeignNode = false;
    try {
      if (section.getOdfElement().getOwnerDocument() != getContentDom()) isForeignNode = true;
      ...
      if (isForeignNode) copyForeignStyleRef(newSectionEle, section.getOwnerDocument());
      if (isForeignNode) // not in a same document
      newSectionEle =
            (TextSectionElement) cloneForeignElement(newSectionEle, getContentDom(), true);
   ...
   *
   * @param name - specify the section name
   * @return an instance of the section
   * @throws RuntimeException if content DOM could not be initialized
   */
  public Section appendSection(String name) {
    ...
    try {
      ...
      newSectionEle = getContentRoot().newTextSectionElement("true", name);
   ...
   *
   * @param isFirstPage if <code>isFirstPage</code> is true, return the First Page header, otherwise
  ...
  public Header getHeader(boolean isFirstPage) {
    ...
    if (tmpHeader == null) {
      try {
        ...
        StyleHeaderElement headerElement =
            OdfElement.findFirstChildNode(StyleHeaderElement.class, masterPageElement);
   ...
   *
   * @param isFirstPage if <code>isFirstPage</code> is true, return the First Page footer, otherwise
  ...
  public Footer getFooter(boolean isFirstPage) {
    ...
    if (tmpFooter == null) {
      try {
        StyleMasterPageElement masterPageElement = getMasterPage(isFirstPage);
        StyleFooterElement footerElement =
            OdfElement.findFirstChildNode(StyleFooterElement.class, masterPageElement);
   ...
   *
   * @param refParagraph the reference paragraph after where the page break inserted.
   * @since 0.6.5
   */
   ...
   *
   * @param columnsNumber the number of columns (are of width identical)
   * @param spacing column spacing in cm (ex. 2.40 for 2,4 cm)
   * @since 0.6.6
   */
  public void setPageColumns(int columnsNumber, double spacing) {
    String vSpacingColumn =
        (new DecimalFormat("#0.###").format(spacing) + Unit.CENTIMETER.abbr()).replace(",", ".");
    ...
    try {
      ...
      if (list.getLength() > 0) {
        ...
        for (int i = 0; i < officeMasterStyles.getLength(); i++) {
          StyleMasterPageElement syleMasterPage =
              (StyleMasterPageElement) officeMasterStyles.item(i);
      ...
      if (pageLayout != null) {
        // Clone the OdfStylePageLayout if another master style possesses the same name before
            ...
            for (int i = 0; i < masterpage.getLength(); i++) {
              ...
              if (vSyleMasterPage.getStyleNameAttribute().equals("Standard")) {
                if (vSyleMasterPage.getStylePageLayoutNameAttribute().equals(oldPageLayoutName)) {
                  vSyleMasterPage.setStylePageLayoutNameAttribute(
                      pageLayout.getStyleNameAttribute());
                }
              }
            }
          }
        ...
        NodeList vListStlePageLprop =
            pageLayout.getElementsByTagName("style:page-layout-properties");
        StylePageLayoutPropertiesElement vStlePageLprop =
            (StylePageLayoutPropertiesElement) vListStlePageLprop.item(0);
        StyleColumnsElement vStyleColumnsElement =
            vStlePageLprop.newStyleColumnsElement(columnsNumber);
   ...
   *
   * @param refParagraph the reference paragraph after where the column break inserted.
   * @since 0.6.6
   */
   ...
   *
   * @param refParagraph the reference paragraph after where the column break inserted.
   * @param breakAttribute the attribute name (page or column)
   */
   ...
   *
   * @param refParagraph the reference paragraph after where the page break inserted.
   * @param master the master page style applied to the new page.
   * @since 0.8
   */
  public void addPageBreak(Paragraph refParagraph, MasterPage master) {
    ...
    try {
      ...
      throw new RuntimeException(
          "PageBreak with mater page - " + master.getName() + " - appends failed.", e);
    }
  }
   ...
   *
   * @param para the instance of paragraph
   * @return true if the paragraph is removed successfully, false if errors happen.
   */
  ...
  public Paragraph getParagraphByReverseIndex(int reverseIndex, boolean isEmptyParagraphSkipped) {
    return getParagraphContainerImpl()
        .getParagraphByReverseIndex(reverseIndex, isEmptyParagraphSkipped);
  }
  ...
  public Chart createChart(
      String title, String[] labels, String[] legends, double[][] data, Rectangle rect) {
  ...
  private StyleMasterPageElement getMasterPage(boolean pFirstPage) throws Exception {
    ...
    OfficeMasterStylesElement masterStyles =
        OdfElement.findFirstChildNode(OfficeMasterStylesElement.class, rootElement);
    if (masterStyles == null) {
      masterStyles = rootElement.newOfficeMasterStylesElement();
    }
    StyleMasterPageElement masterPageEle = null;
    NodeList lastMasterPages =
        masterStyles.getElementsByTagNameNS(OdfDocumentNamespace.STYLE.getUri(), "master-page");
    ...
    if (masterPageEle == null) {
      OdfStylePageLayout layout =
          OdfElement.findFirstChildNode(
              OdfStylePageLayout.class, getStylesDom().getAutomaticStyles());
      masterPageEle =
          masterStyles.newStyleMasterPageElement(pageStyleName, layout.getStyleNameAttribute());
    }
  ...
  private ParagraphContainerImpl getParagraphContainerImpl() {
    if (paragraphContainerImpl == null) paragraphContainerImpl = new ParagraphContainerImpl();
  ...
  /**
   * Create a new Table Of Content to this document before the reference paragraph. Because until
   * you do a full layout of the document, taking into account font metrics,line breaking
   * algorithms, hyphenation, image positioning, "orphan and widow" rules, etc., you don't know what
   * content is on which page. So all the page numbers in TOC are be set value "1". Please update
   * the page numbers from AOO Menus: Tools->Update->All Indexes and Tables
   *
   * @param refParagraph the reference paragraph where the TOC be inserted.
   * @param before true:insert TOC before the reference paragraph. false:insert TOC after the
   *     reference paragraph.
   * @since 0.8.6
   */
  public TextTableOfContentElement createDefaultTOC(Paragraph refParagraph, boolean before) {
    ...
    try {
      ...
      textTableOfContent = content.newOdfElement(TextTableOfContentElement.class);
      textTableOfContent.setTextNameAttribute("Table of Contents");
      textTableOfContent.setTextProtectedAttribute(true);
      TextTableOfContentSourceElement textTableOfContentSource =
          textTableOfContent.newTextTableOfContentSourceElement();
      ...
      TextIndexTitleTemplateElement textIndexTitleTemplate =
          textTableOfContentSource.newTextIndexTitleTemplateElement();
      ...
      for (int i = 1; i <= 10; i++) {
        TextTableOfContentEntryTemplateElement textTableOfContentEntryTemplate =
            textTableOfContentSource.newTextTableOfContentEntryTemplateElement(
                i, "Contents_20_" + i);
        TextIndexEntryLinkStartElement TextIndexEntryLinkStart =
            textTableOfContentEntryTemplate.newTextIndexEntryLinkStartElement();
        ...
        TextIndexEntryTabStopElement TextIndexEntryTabStop =
            textTableOfContentEntryTemplate.newTextIndexEntryTabStopElement("right");
      ...
      TextIndexTitleElement TextIndexTitle =
          textIndexBody.newTextIndexTitleElement("Table of Contents_Head");
      ...
      Logger.getLogger(TextDocument.class.getName())
          .log(Level.SEVERE, "Failed to create Default TOC", e);
      throw new RuntimeException("Failed to create Default TOC", e);
    }
  }

  private void ceateIndexBodyEntry(
      TextIndexBodyElement textIndexBody, String stylename, String text) {
  ...
  /**
   * Create a new Table Of Content to this document before the reference paragraph. The additional
   * paragraph styles list will be included in the TOC. Because until you do a full layout of the
   * document, taking into account font metrics,line breaking algorithms, hyphenation, image
   * positioning, "orphan and widow" rules, etc., you don't know what content is on which page. So
   * all the page numbers in TOC are be set value "1". Please update the page numbers from AOO
   * Menus: Tools->Update->All Indexes and Tables. If additionalStyle is null then call
   * createDefaultTOC().
   *
   * @param refParagraph the reference paragraph before where the TOC inserted.
   * @param additionalStyle the additional paragraph styles that you want to include in the TOC
   * @param before true:insert TOC before the reference paragraph. false:insert TOC after the
   *     reference paragraph.
   * @since 0.8.6
   */
  public TextTableOfContentElement createTOCwithStyle(
      Paragraph refParagraph, TOCStyle additionalStyle, boolean before) {
    if (refParagraph == null) {
      Logger.getLogger(TextDocument.class.getName())
          .log(
              Level.SEVERE,
    ...
    Iterable<OdfStyle> paragraphStyles = docstyles.getStylesForFamily(OdfStyleFamily.Paragraph);
      ...
      if (!tocstyleList.isEmpty()) {
        ...
        while (it.hasNext()) {
          ...
          TextIndexSourceStylesElement textIndexSourceStyles =
              textTableOfContentSource.newTextIndexSourceStylesElement(textOutlineLevelValue);
   ...
   *
   * @param referenceParagraph - where the Paragraph be inserted
   * @param sourceParagraph - the Paragraph which will be copied
   * @param before true:insert Paragraph before the reference paragraph. false:insert Paragraph
   *     after the reference paragraph.
   */
  public Paragraph insertParagraph(
      Paragraph referenceParagraph, Paragraph sourceParagraph, boolean before) {
    boolean isForeignNode = false;
    try {
      ...
      TextParagraphElementBase newParagraphEle =
          (TextParagraphElementBase) oldParagraphEle.cloneNode(true);

      if (isForeignNode)
        copyForeignStyleRef(sourceParagraph.getOdfElement(), sourceParagraph.getOwnerDocument());
      if (isForeignNode) // not in a same document
      newParagraphEle =
            (TextParagraphElementBase) cloneForeignElement(newParagraphEle, getContentDom(), true);
  ...
  public Table insertTable(Paragraph referenceParagraph, Table sourceTable, boolean before) {
    ...
    TableTableElement newTEle =
        (TableTableElement)
            insertOdfElement(
                referenceParagraph.getOdfElement(),
                ownDocument,
                sourceTable.getOdfElement(),
                before);
   ...
   *
   * @param referenceOdfElement - where the OdfElement be inserted
   * @param sourceDocument - the source Document which contain the sourceOdfElement
   * @param sourceOdfElement - the OdfElement which will be copied
   * @param before true:insert OdfElement before the reference OdfElement. false:insert OdfElement
   *     after the reference OdfElement.
   */
  public OdfElement insertOdfElement(
      OdfElement referenceOdfElement,
    ...
    try {
      ...
      if (sourceOdfElement.getOwnerDocument() != getContentDom()) isForeignNode = true;
  ...
  private OdfElement insertOdfElementwithoutstyle(
      OdfElement referenceOdfElement,
      Document sourceDocument,
      OdfElement sourceOdfElement,
      boolean before) {
   ...
   *
   * @param sourceDocument the source TextDocument
   * @param referenceParagraph where the text content of the source TextDocument be inserted
   * @param isCopyStyle true:copy the styles in source document to current TextDocment. false:don't
   *     copy the styles in source document to current TextDocment.
   */
  public void insertContentFromDocumentAfter(
      TextDocument sourceDocument, Paragraph referenceParagraph, boolean isCopyStyle) {
    try {
      ...
      for (int i = (clist.getLength() - 1); i >= 0; i--) {
        ...
        if (isCopyStyle) {
          ...
          insertOdfElementwithoutstyle(
              referenceParagraph.getOdfElement(), sourceDocument, node, false);
        }
      }
   ...
   * the reference Paragraph, with Styles or without Styles.
   *
   * @param srcDoc the source TextDocument
   * @param referenceParagraph where the text content of the source TextDocument be inserted
   * @param isCopyStyle true:copy the styles in source document to current TextDocment. false:don't
   *     copy the styles in source document to current TextDocment.
   */
  public void insertContentFromDocumentBefore(
      TextDocument sourceDocument, Paragraph referenceParagraph, boolean isCopyStyle) {
    try {
      OfficeTextElement sroot = sourceDocument.getContentRoot();
      NodeList clist = sroot.getChildNodes();
      for (int i = 0; i < clist.getLength(); i++) {
        ...
        if (isCopyStyle) {
          ...
          insertOdfElementwithoutstyle(
              referenceParagraph.getOdfElement(), sourceDocument, node, true);
        }
      }
    } catch (Exception e) {
      Logger.getLogger(TextDocument.class.getName()).log(Level.SEVERE, null, e);
  ...
  private class FormContainerImpl extends AbstractFormContainer {
    ...
    public OfficeFormsElement getFormContainerElement() {
      ...
      try {
        ...
        forms = OdfElement.findFirstChildNode(OfficeFormsElement.class, root);
        if (forms == null) {
          Node firstChild = root.getFirstChild();
          OfficeFormsElement officeForms =
              ((OdfFileDom) getContentDom()).newOdfElement(OfficeFormsElement.class);
          forms = (OfficeFormsElement) root.insertBefore(officeForms, firstChild);
        }
        return forms;
      } catch (Exception e) {
        Logger.getLogger(TextDocument.class.getName()).log(Level.SEVERE, null, e);
  ...
  public Control createDrawControl() {
    ...
    Node refChild = OdfElement.findFirstChildNode(TextSequenceDeclsElement.class, parent);
