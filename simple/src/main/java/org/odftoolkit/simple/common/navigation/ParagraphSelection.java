...
public class ParagraphSelection extends Selection {
   ...
   * different Document.
   *
   * @param paragraph the reference paragraph to replace.
   */
  public Paragraph replaceWithParagraph(Paragraph paragraph) {
    ...
    if (search instanceof TextNavigation) {
      ...
      if (textSearch != null
          ...
          && textSearch.getReplacedItem().getElement() == this.textSelection.getElement()) {
        continued = true;
      } else {
      ...
      if (textSearch != null) {
        ...
        OdfElement containerElement = paragraphContainer.getOdfElement();
        String content = TextExtractor.getText(containerElement);
        TextSelection selected =
            TextSelection.newTextSelection(
                textSearch, this.textSelection.getText(), containerElement, content.length() - 1);
  ...
  /**
   * Construct a ParagraphSelection with TextSelection. Then user can replace text content with
   * paragraph.
   *
   * @param selection the TextSelection to be decorated.
   */
  ...
  private ParagraphProperties getParagraphPropertiesForWrite() {
    OdfStyleBase style = paragraphContainer.getStyleHandler().getStyleElementForRead();
    if (style == null || style.getLocalName().equals("default-style")) {
      OdfStyle element = paragraphContainer.getStyleHandler().getStyleElementForWrite();
    ...
    ParagraphProperties properties =
        paragraphContainer.getStyleHandler().getParagraphPropertiesForWrite();
    return properties;
  }
  ...
  private void handlePageBreak(Paragraph origParagraph, String pos, boolean continued) {
    if (continued && this.textSelection.getTextNavigation().isHandlePageBreak()) return;
    ParagraphProperties orgParaPty =
        origParagraph.getStyleHandler().getParagraphPropertiesForRead();
    ...
    if (orgParaPty != null) {
      ...
      if (breakAttribute != null) {
        if (posInPara.equals("head") || posInPara.equals("whole")) {
          getParagraphPropertiesForWrite().setBreak("before", breakAttribute);
          handleBreak = true;
        }
      }
      ...
      if (breakAttribute != null) {
        if (posInPara.equals("end") || posInPara.equals("whole")) {
          getParagraphPropertiesForWrite().setBreak("after", breakAttribute);
          handleBreak = true;
        }
      }
    }
    String masterStyle =
        origParagraph
            ...
            .getOdfAttributeValue(OdfName.newName(OdfDocumentNamespace.STYLE, "master-page-name"));
    if (masterStyle != null && !masterStyle.isEmpty()) {
      if (posInPara.equals("head") || posInPara.equals("whole")) {
        getParagraphStyleElementForWrite()
            .setOdfAttributeValue(
                OdfName.newName(OdfDocumentNamespace.STYLE, "master-page-name"), masterStyle);
        handleBreak = true;
        try {
          ...
          Logger.getLogger(ParagraphSelection.class.getName())
              .log(Level.SEVERE, e.getMessage(), "NumberFormatException");
        }
      }
    }
    if (handleBreak && !posInPara.equals("whole")) cleanBreakProperty(origParagraph);
  }
      ...
      if (index == 0) {
        if (leftLength == nodeLength) {
          ...
          Paragraph orgparagraph =
              Paragraph.getInstanceof((TextParagraphElementBase) rightparentElement);
        ...
        // at the middle of original Paragraph, split original Paragraph, insert before the second
        // Paragraph.
        delete(index, leftLength, rightparentElement);
        ...
        TextDocument document = (TextDocument) orgparagraph.getOwnerDocument();
        paragraphContainer = document.insertParagraph(orgparagraph, sourceParagraph, true);
        if (!continued) cleanBreakProperty(orgparagraph);
      }
    } else {
