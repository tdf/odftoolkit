...
/**
 * ConditionField specifies a condition for display of one text string or another. If the condition
 * is true, one of the text strings is displayed. If the condition is false, the other text string
 * is displayed.
 *
 * @since 0.5
 */
public class ConditionField extends Field {
  ...
  ConditionField(
      ...
      String trueText,
      String falseText,
      boolean isHiddenTextField) {
    if (odfElement instanceof TextPElement) {
      conditionalTextElement =
          ((TextPElement) odfElement)
              .newTextConditionalTextElement("ooow:" + condition, falseText, trueText);
    } else if (odfElement instanceof TextSpanElement) {
      conditionalTextElement =
          ((TextSpanElement) odfElement)
              .newTextConditionalTextElement("ooow:" + condition, falseText, trueText);
    } else {
      TextPElement pElement =
          ((OdfFileDom) odfElement.getOwnerDocument()).newOdfElement(TextPElement.class);
      odfElement.appendChild(pElement);
      conditionalTextElement =
          pElement.newTextConditionalTextElement("ooow:" + condition, falseText, trueText);
    }
   ...
   *
   * @param condition the new condition of this condition field.
   */
