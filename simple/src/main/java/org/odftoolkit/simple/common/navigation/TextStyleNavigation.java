 ...
 * document and find matched style properties and return <code>TextSelection</code> instance(s).
 */
public class TextStyleNavigation extends Navigation {
  ...
  /**
   * Construct <code>TextStyleNavigation</code> with style properties condition and navigation
   * scope.
   *
  ...
  /**
   * Check if the element has specified style properties, which are stated when the <code>
   * TextStyleNavigation</code> created.
   *
  ...
  private TextSelection findNext(TextSelection selected) {
    ...
    if (mNode != null) {
      ...
      TextSelection item = new TextSelection(this, mText, element, mCurrentIndex);
      return item;
    }
    return null;
  }
  ...
  private Set<String> getMatchStyleNames() {
    ...
    try {
      NodeList defStyleList =
          mDocument.getDocumentStyles().getElementsByTagName("style:default-style");
      ...
      Iterator<OdfStyle> cStyles =
          mDocument.getContentDom().getAutomaticStyles().getAllStyles().iterator();
