...
/**
 * <code>TextSelection</code> describes a sub element in a paragraph element or a heading element.
 * It is recognized by the container element, which type should be {@link
 * org.odftoolkit.odfdom.incubator.doc.text.OdfTextParagraph OdfTextParagraph} or {@link
 * org.odftoolkit.odfdom.incubator.doc.text.OdfTextHeading OdfTextHeading}, the start index of text
 * content in container element and the text content of this <code>Selection</code>.
 */
public class TextSelection extends Selection {
   ...
   *
   * @param text the text content of this <code>TextSelection</code>
   * @param containerElement the paragraph element or heading element that contains this <code>
   *     TextSelection</code>
   * @param index the start index of the text content in container element
   */
   ...
   * @since 0.5.5
   */
  public static TextSelection newTextSelection(
      Navigation search, String text, OdfElement containerElement, int index) {
  ...
  /**
   * Get the paragraph element or heading element that contains this <code>TextSelection</code>.
   *
  ...
  public void replaceWith(String newText) throws InvalidNavigationException {
    ...
    search
        .mDocument
        .getSelectionManager()
        .refresh(getContainerElement(), offset, index + getText().length());
   ...
   *
   * @param newTable the replace Table
   * @return
   * @throws InvalidNavigationException if the selection is unavailable.
   ...
   *
   * @param newImage the replace Image
   * @return
   * @throws InvalidNavigationException if the selection is unavailable.
   * @return the new Image in the TextDocument,the image name is set to "replace" +
   *     System.currentTimeMillis(), please update the name to others by yourself.
   */
   ...
   * Replace the text content of selection with a new Image.
   *
   * @param imageUri the replace Image URI
   * @throws InvalidNavigationException if the selection is unavailable.
   * @return the new Image in the TextDocument,the image name is set to "replace" +
   *     System.currentTimeMillis(), please update the name to others by yourself.
   */
   ...
   *
   * @param orgField the reference Field to replace.
   * @throws InvalidNavigationException if the selection is unavailable.
    ...
    switch (fieldType) {
      ...
      case USER_VARIABLE_FIELD:
        ...
        TextUserFieldDeclElement textUserFieldDeclElement =
            (TextUserFieldDeclElement) userVariableField.getOdfElement();
      ...
      case CONDITION_FIELD:
        ...
        TextConditionalTextElement textConditionalTextElement =
        ...
        ConditionField newdConditionField =
            nextFieldSelection.replaceWithConditionField(
                StringCondition, StringValueIfTrue, StringValueIfFalse);
        TextConditionalTextElement newTextConditionalTextElement =
            (TextConditionalTextElement) newdConditionField.getOdfElement();
      ...
      case HIDDEN_TEXT_FIELD:
        ...
        TextConditionalTextElement textConditionalTextElementHIDDEN =
            (TextConditionalTextElement) conditionFieldHIDDEN.getOdfElement();
        String StringValueIfFalseHIDDEN =
            textConditionalTextElementHIDDEN.getTextStringValueIfFalseAttribute();
        String StringConditionHIDDEN = textConditionalTextElementHIDDEN.getTextConditionAttribute();
        boolean CurrentValueHIDDEN =
            textConditionalTextElementHIDDEN.getTextCurrentValueAttribute();
        ConditionField newdConditionFieldHIDDEN =
            nextFieldSelection.replaceWithHiddenTextField(
                StringConditionHIDDEN, StringValueIfFalseHIDDEN);
        TextConditionalTextElement newTextConditionalTextElementHIDDEN =
            (TextConditionalTextElement) newdConditionFieldHIDDEN.getOdfElement();
      ...
      default:
        throw new IllegalArgumentException(
            "Simple Java API for ODF doesn't support this type now.");
    }
    return newfield;
  }
   ...
   * different Document.
   *
   * @param newParagraph the reference paragraph to replace.
   * @throws InvalidNavigationException if the selection is unavailable.
  ...
  /**
   * Replace the content with a TextDocument with Styles. Note: You need cache the
   * TextNavigation.nextSelection item because after you replace currtenTextSelection with
   * TextDocument, TextNavigation.nextSelection will search from the inserted Content, it will make
   * you into a loop if the Search keyword also can be found in the new inserted Content. The right
   * way to use this replaceWithTextDocument(TextDocument textDocument) method should like this:
   * <Code>
   ...
   * </Code>
   *
   * @param newTextDocument the reference TextDocument to replace.
  ...
  public void pasteAtEndOf(Selection positionItem) throws InvalidNavigationException {
    ...
    if (positionItem instanceof TextSelection) {
      indexOfNew =
          ((TextSelection) positionItem).getIndex()
              + ((TextSelection) positionItem).getText().length();
      newElement = ((TextSelection) positionItem).getContainerElement();
    }
    OdfTextSpan textSpan = getSpan((OdfFileDom) positionItem.getElement().getOwnerDocument());
    mIsInserted = false;
   ...
   *
   * @param url the URL of this hypertext reference
   * @throws InvalidNavigationException if the selection is unavailable.
   */
   ...
   * @param creator the creator of this comment, if <code>creator</code> is null, the value of
   *     <code>System.getProperty("user.name")</code> will be used.
   * @throws InvalidNavigationException if the selection is unavailable.
   * @since 0.6.5
   */
  public void addComment(String content, String creator) throws InvalidNavigationException {
    ...
    StyleTextPropertiesElement styleTextPropertiesElement =
        textStyle.newStyleTextPropertiesElement(null);
   ...
   * provided.
   *
   * @return a String representation of the value of this <code>TextSelection</code>
   */
  @Override
  ...
  void cleanBreakProperty(Paragraph paragraph) {
    ...
    if (search == null) throw new IllegalStateException("Navigation is null");
    ...
    if (modifiedStyleElement == null) {
      ...
      NodeList paragraphProperties =
          modifiedStyleElement.getElementsByTagName("style:paragraph-properties");
      if (paragraphProperties != null && paragraphProperties.getLength() > 0) {
        StyleParagraphPropertiesElement property =
            (StyleParagraphPropertiesElement) paragraphProperties.item(0);
  ...
  private OdfTextSpan getSpan(OdfFileDom ownerDoc) {
    ...
    if (parentElement != null) {
      ...
      if (copyParentNode instanceof OdfStylableElement) {
        applyTextStyleProperties(
            getTextStylePropertiesDeep((OdfStylableElement) copyParentNode), textSpan);
      }
      return textSpan;
    }
    return null;
      ...
      while (node != null) {
        if (node.getNodeType() == Node.TEXT_NODE) {
          ...
          if (!node.equals(textSpan)) {
            ...
            Map<OdfStyleProperty, String> styles1 =
                getTextStylePropertiesDeep((OdfStylableElement) node);
            if (styles == null) {
              styles = styles1;
  ...
  private Map<OdfStyleProperty, String> getTextStyleProperties(OdfStylableElement element) {
    ...
    OdfStyleBase styleElement =
        element.getAutomaticStyles().getStyle(styleName, element.getStyleFamily());
    ...
    if (styleElement != null) {
      ...
      if ((styleElement.getPropertiesElement(OdfStylePropertiesSet.ParagraphProperties) == null)
        ...
        styleElement =
            ((Document) ((OdfFileDom) styleElement.getOwnerDocument()).getDocument())
                .getDocumentStyles()
