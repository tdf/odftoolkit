 ...
 *
 * <p>The advantage of entering a reference as a field is that you do not have to adjust the
 * references manually every time you change the document. Just update the fields and the references
 * in the document are updated too.
 *
...
public class ReferenceField extends Field {
  ...
  /**
   * A <tt>DisplayType</tt> specifies the information that a reference field should display.
   *
   * <ul>
   *   <li>CHAPTER: displays the number of the chapter in which the referenced item appears.
   *   <li>DIRECTION: displays whether the referenced item is above or below the reference field.
   *   <li>PAGE: displays the number of the page on which the referenced item appears.
  ...
  ReferenceField(OdfElement odfElement, String referenceName) {
    ...
    if (parentEle instanceof TextAElement
        || parentEle instanceof TextHElement
        || parentEle instanceof TextMetaElement
        || parentEle instanceof TextMetaFieldElement
        || parentEle instanceof TextPElement
        || parentEle instanceof TextSpanElement) {
      referencedElement = odfElement;
      name = referenceName;
      // insert start target element
      referenceMarkStartElement =
          ((OdfFileDom) odfElement.getOwnerDocument())
   ...
   *
   * @param referenceName the name of this content.
   */
   ...
   *
   * @param odfEle the reference of odf element.
  ...
  public void appendReferenceTo(OdfElement odfEle, DisplayType type) {
    ...
    TextSpanElement spanElement =
        ((OdfFileDom) odfEle.getOwnerDocument()).newOdfElement(TextSpanElement.class);
    ...
    if (odfEle instanceof TextPElement) {
      ...
      TextPElement pElement =
          ((OdfFileDom) odfEle.getOwnerDocument()).newOdfElement(TextPElement.class);
