...
/**
 * This class presents frame object. It provides method to get/set frame properties, content, and
 * and styles. A frame is a container for enhanced content like text boxes, images or objects. A
 * frame may contain multiple renditions of content.
 *
 * @since 0.5
 */
public class Frame extends Component {
   ...
   * Attributes:
   *
   * <p>draw:caption-id 19.115, draw:class-names 19.120, draw:copy-of 19.126, draw:id 19.187.3,
   * draw:layer 19.189, draw:name 19.197.10, draw:style-name 19.219.13, draw:text-style-name 19.227,
   * draw:transform 19.228, draw:z-index 19.231, presentation:class 19.389, presentation:class-names
   * 19.390, presentation:placeholder 19.407, presentation:style-name 19.422,
   * presentation:user-transformed 19.427, style:rel-height 19.509, style:rel-width 19.510.2,
   * svg:height 19.539.8, svg:width 19.571.10, svg:x 19.573.5, svg:y 19.577.5,
   * table:end-cell-address 19.627, table:end-x 19.632, table:end-y 19.633, table:table-background
   * 19.728, text:anchor-page-number 19.753, text:anchor-type 19.754 xml:id 19.914.
   */
   ...
   *
   * @param element - the instance of DrawFrameElement
   ...
   *
   * <p>The frame will be added at the end of this container.
   *
   * @param container - the frame container that contains this frame.
   */
  ...
  /**
   * Get the instance of <code>DrawFrameElement</code> which represents this frame.
   *
   ...
   *
   * @param rectangle - the rectangle used by this frame
   */
  public void setRectangle(FrameRectangle rectangle) {
    String linemeasure = rectangle.getLinearMeasure().toString();
    if (rectangle.getWidth() > 0) mElement.setSvgWidthAttribute(rectangle.getWidth() + linemeasure);
    if (rectangle.getHeight() > 0)
      mElement.setSvgHeightAttribute(rectangle.getHeight() + linemeasure);
    if (rectangle.getX() > 0) mElement.setSvgXAttribute(rectangle.getX() + linemeasure);
    if (rectangle.getY() > 0) mElement.setSvgYAttribute(rectangle.getY() + linemeasure);
  }
   ...
   *
   * @param title - the title of this text box
   */
  public void setTitle(String title) {
    SvgTitleElement titleElement = OdfElement.findFirstChildNode(SvgTitleElement.class, mElement);
    if (titleElement == null) titleElement = mElement.newSvgTitleElement();
  ...
  public String getTitle() {
    ...
    else return titleElement.getTextContent();
  }
  ...
  public String getDesciption() {
    ...
    else return descElement.getTextContent();
  }

  /**
   * Set the description of this text box.
   *
   * @param description - the description of this text box
   */
  public void setDescription(String description) {
    SvgDescElement descElement = OdfElement.findFirstChildNode(SvgDescElement.class, mElement);
    if (descElement == null) descElement = mElement.newSvgDescElement();
  ...
  public void setHyperlink(URI linkto) {
    ...
    if (parent instanceof OfficeImageElement) return;
    try {
      DrawAElement aElement = mOwnerDocument.getContentDom().newOdfElement(DrawAElement.class);
      ...
      if (brother == null) parent.appendChild(aElement);
  ...
  public FrameStyleHandler getStyleHandler() {
    if (mStyleHandler == null) mStyleHandler = new FrameStyleHandler(this);
   ...
   *
   * <p>If the parameter is null, there will be no background color defined for this frame. The old
   * setting of background color will be removed.
   *
  ...
  private void removeContent() {
    ...
    for (i = 0; i < nodeList.getLength(); i++) {
      ...
      if (node.getNodeType() == Node.TEXT_NODE) mElement.removeChild(node);
