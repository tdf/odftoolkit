...
/**
 * AbstractTextboxContainer is an abstract implementation of the TextboxContainer interface, with a
 * default implementation for every method defined in TextboxContainer, except
 * getFrameContainerElement(). A subclass must implement the abstract method
 * getFrameContainerElement().
 *
 * @since 0.5
 */
public abstract class AbstractTextboxContainer extends AbstractFrameContainer
   ...
   * Remove the text box.
   *
   * @param box - the text box to be removed
   * @return true if the text box is removed successfully, false if errors happen.
   */
  public boolean removeTextbox(Textbox box) {
    ...
    try {
      ...
      Logger.getLogger(AbstractParagraphContainer.class.getName())
  ...
  public Textbox getTextboxByName(String name) {
    ...
    if (frame != null) {
      DrawTextBoxElement boxElement =
          OdfElement.findFirstChildNode(DrawTextBoxElement.class, frame.getDrawFrameElement());
      if (boxElement != null) {
   ...
   *
   * <p>This method will return a list of text boxs by the usage defined in presentation slides.
   *
   * @param usage - the usage description
   * @return a list of text box Null will be returned if the owner document is not a presentation
  ...
  private class SimpleTextboxIterator implements Iterator<Textbox> {
    ...
    private Textbox findNext(Textbox thisBox) {
      ...
      if (thisBox == null) {
        ...
        nextFrame =
            OdfElement.findNextChildNode(DrawFrameElement.class, thisBox.getDrawFrameElement());
      }

      if (nextFrame != null) {
        DrawTextBoxElement nextbox =
            OdfElement.findFirstChildNode(DrawTextBoxElement.class, nextFrame);
