...
public class Image extends Component {
   ...
   *
   * @param imageElement - the instance of DrawImageElement
  ...
  public FrameStyleHandler getStyleHandler() {
    if (mStyleHandler == null) mStyleHandler = new FrameStyleHandler(mFrame);
  ...
  public static Image newImage(FrameContainer container, URI uri) {
    ...
    try {
      ...
      String packagePath =
          insertImageResourceIntoPackage((OdfSchemaDocument) ownerDom.getDocument(), uri);
      packagePath = packagePath.replaceFirst(ownerDom.getDocument().getDocumentPath(), "");
      URI newURI =
          configureInsertedImage(
   ...
   *
   * @param frame - the frame which contains this image
   * @param uri - the URI of the image
   * @return the instance of Image
   */
  public static Image newImage(Frame frame, URI uri) {
    ...
    try {
      ...
      URI newURI =
          configureInsertedImage(
              (OdfSchemaDocument) ownerDom.getDocument(), imageElement, packagePath, true);
  ...
  private static String insertImageResourceIntoPackage(
      OdfSchemaDocument mOdfSchemaDoc, URI imageUri) throws Exception {
  ...
  /* Helper method */
  public static URI configureInsertedImage(
   ...
   *
   * @return true if the image object is successfully removed; false if otherwise.
   */
  public boolean remove() {
    OdfElement containerElement = (OdfElement) mElement.getParentNode();
    try {
      mElement.removeChild(getOdfElement());
      if (mElement.hasChildNodes() == false) containerElement.removeChild(mElement);
   ...
   *
   * @param imageUri - the URI of the new image resource
   */
  ...
  public String getInternalPath() {
    try {
      if (mImageURI == null) mImageURI = new URI(mImageElement.getXlinkHrefAttribute());
   ...
   *
   * @param rectangle - the rectangle used by this image
   */
