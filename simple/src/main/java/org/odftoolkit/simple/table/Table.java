 ...
 *
 * <p>Table provides methods to get/add/delete/modify table column/row/cell.
 */
public class Table extends Component {
   ...
   *
   * <p>The end user isn't allowed to create it directly, otherwise an <code>IllegalStateException
   * </code> will be thrown.
   *
   * @since 0.3.5
   */
  public static class TableBuilder {
    ...
    private final IdentityHashMap<TableTableElement, Table> mTableRepository =
        new IdentityHashMap<TableTableElement, Table>();

    /**
     * TableBuilder constructor. This constructor should only be use in owner {@link
     * org.odftoolkit.simple.table.TableContainer TableContainer} constructor. The end user isn't
     * allowed to call it directly, otherwise an <code>IllegalStateException</code> will be thrown.
     *
     * @param container the owner <code>TableContainer</code>.
     * @throws IllegalStateException if new TableBuilder out of owner Document constructor, this
    ...
    public TableBuilder(TableContainer container) {
      if (container.getTableBuilder() == null) {
        ...
        throw new IllegalStateException(
            "TableBuilder only can be created in table containter constructor.");
      }
    }
    ...
    /**
     * Construct the <code>Table</code> feature. The default column count is 5. The default row
     * count is 2.
     *
     * <p>The table will be inserted at the end of the table container. An unique table name will be
     * given, you may set a custom table name using the <code>setTableName</code> method.
     *
     * <p>If the container is a text document, cell borders will be created by default.
     *
    ...
    /**
     * Construct the <code>Table</code> feature with a specified row number, column number, header
     * row number, header column number.
     *
     * <p>The table will be inserted at the end of the container. An unique table name will be
     * given, you may set a custom table name using the <code>setTableName</code> method.
     *
     * <p>If the container is a text document, cell borders will be created by default.
     *
    ...
    public Table newTable(int numRows, int numCols, int headerRowNumber, int headerColumnNumber) {
      try {
        TableTableElement newTEle =
            createTable(ownerContainer, numRows, numCols, headerRowNumber, headerColumnNumber);
     ...
     * @param headerRowNumber the header row number
     * @param headerColumnNumber the header column number
     * @param marginLeft the left table margin in centimeter(cm), between the left margin of table
     *     container and the table
     * @param marginRight the right table margin in centimeter(cm), between the right margin of
     *     table container and the table
     * @return a new instance of <code>Table</code>
     * @since 0.5.5
     */
    public Table newTable(
    ...
    /**
     * Construct the <code>Table</code> feature with a specified row number and column number.
     *
     * <p>The table will be inserted at the end of the container. An unique table name will be
     * given, you may set a custom table name using the <code>setTableName</code> method.
     *
    ...
    /**
     * Construct the Table feature with a specified 2 dimension array as the data of this table. The
     * value type of each cell is float.
     *
     * <p>The table will be inserted at the end of the container. An unique table name will be
     * given, you may set a custom table name using the <code>setTableName</code> method.
     *
    ...
    /**
     * Construct the Table feature with a specified 2 dimension array as the data of this table. The
     * value type of each cell is string.
     *
     * <p>The table will be inserted at the end of the container. An unique table name will be
     * given, you may set a custom table name using the <code>setTableName</code> method.
     *
   ...
   * Get a table feature instance by an instance of <code>TableTableElement</code>.
   *
   * @param element an instance of <code>TableTableElement</code>
   * @return an instance of <code>Table</code> that can represent <code>element</code>
   */
  ...
  /**
   * Construct the <code>Table</code> feature. The default column count is 5. The default row count
   * is 2.
   *
   * <p>The table will be inserted at the end of the container. An unique table name will be given,
   * you may set a custom table name using the <code>setTableName</code> method.
   *
   * <p>If the <code>tableContainer</code> is a text document, cell borders will be created by
   * default.
   *
   * @param tableContainer the table container that contains this table
   * @return the created <code>Table</code> feature instance
   */
   ...
   * Construct the <code>Table</code> feature with a specified row number and column number.
   *
   * <p>The table will be inserted at the end of the tableContainer. An unique table name will be
   * given, you may set a custom table name using the <code>setTableName</code> method.
   *
   * <p>If the <code>tableContainer</code> is a text document, cell borders will be created by
   * default.
   ...
   * @param numRows the row number
   * @param numCols the column number
   * @param marginLeft double <I>the left table margin in cm (between the left margin of document
   *     and the table)</I>
   * @param marginRight double <I>the right table margin in cm (between the right margin of document
   *     and the table)</I>
   * @return a new instance of <code>Table</code>
   */
  public static Table newTable(
    ...
    return tableContainer
        .getTableBuilder()
        .newTable(numRows, numCols, 0, 0, marginLeft, marginRight);
  }

  /**
   * Construct the <code>Table</code> feature with a specified row number, column number, header row
   ...
   * default.
   *
   * @param tableContainer the ODF document that contains this feature
   * @param numRows the row number
   * @param numCols the column number
   * @param headerRowNumber the header row number
   * @param headerColumnNumber the header column number
  ...
  public static Table newTable(
    ...
    return tableContainer
        .getTableBuilder()
        .newTable(numRows, numCols, headerRowNumber, headerColumnNumber);
  }
   ...
   */
  public static Table newTable(
      TableContainer tableContainer, String[] rowLabel, String[] columnLabel, double[][] data) {
   ...
   */
  public static Table newTable(
      TableContainer tableContainer, String[] rowLabel, String[] columnLabel, String[][] data) {
    return tableContainer.getTableBuilder().newTable(rowLabel, columnLabel, data);
  }
  ...
  public void setWidth(double width) {
    if (!mIsSpreadsheet) {
      ...
      double inValue =
          Math.round(roundingFactor * width / Unit.INCH.unitInMillimiter()) / roundingFactor;
      ...
      if (alineStyle == null
          || alineStyle.length() == 0
          || TableAlignAttribute.Value.MARGINS.toString().equals(alineStyle)) {
  ...
  private static TableTableElement createTable(
    ...
    if (marginLeft != 0) {
      tableStyle.setProperty(
          StyleTablePropertiesElement.MarginLeft,
          (new DecimalFormat("#0.##").format(marginLeft) + Unit.CENTIMETER.abbr())
              .replace(",", "."));
    }
    if (marginRight != 0) {
      tableStyle.setProperty(
          StyleTablePropertiesElement.MarginRight,
          (new DecimalFormat("#0.##").format(marginRight) + Unit.CENTIMETER.abbr())
              .replace(",", "."));
    }
    ...
    if (isTextDocument) {
      columnStyle.setProperty(
          ...
          IN_FORMAT.format(tableWidth / numCols) + "in");
      columnStyle.setProperty(
    ...
    if (headerColumnNumber > 0) {
      ...
      if (headerColumnNumber > 1) {
        ...
        headercolumn.removeAttributeNS(
   ...
   *
   * <p>A table can only be formatted as one type of styles: even-odd-rows or even-odd-columns. The
   * rule is to check the style of odd rows and even rows in the template, only if they have one
   * different properties, table: style-name or table:paragraph-style-name, the table template will
   * be treated as a even-odd-columns styled table.
   *
   * <p>If one style in the template is null, the style of corresponding cells will be removed. An
   * empty template can be used to remove all the styles in a table.
   *
   * @param template
   * @throws IllegalArgumentException if the given template is null
  ...
  public void applyStyle(TableTemplate template) throws Exception {
    ...
    OdfStyle evenRowsTableStyle =
        styles.getStyle(template.getTableEvenRowsTableStyle(), OdfStyleFamily.TableCell);
    OdfStyle oddRowsTableStyle =
        styles.getStyle(template.getTableOddRowsTableStyle(), OdfStyleFamily.TableCell);
    OdfStyle evenRowsParagraphStyle =
        styles.getStyle(template.getTableEvenRowsParagraphStyle(), OdfStyleFamily.Paragraph);
    OdfStyle oddRowsParagraphStyle =
        styles.getStyle(template.getTableOddRowsParagraphStyle(), OdfStyleFamily.Paragraph);
    ...
    if (evenRowsParagraphStyle != null || oddRowsParagraphStyle != null)
      isEqualParaStyle = evenRowsParagraphStyle.compareTo(oddRowsParagraphStyle) == 0;

    Iterator<Row> rowIterator = this.getRowIterator();

    if (rowIterator.hasNext()) { // first row
      Row currentRow = rowIterator.next();
      String firstCellTableStyle =
          template.getExtendedTableStyleByType(TableTemplate.ExtendedStyleType.FIRSTROWSTARTCOLUM);
      String firstCellParagraphStyle =
          template.getExtendedParagraphStyleByType(
              TableTemplate.ExtendedStyleType.FIRSTROWSTARTCOLUM);
      String lastCellTableStyle =
          template.getExtendedTableStyleByType(TableTemplate.ExtendedStyleType.FIRSTROWENDCOLUMN);
      String lastCellParagraphStyle =
          template.getExtendedParagraphStyleByType(
              TableTemplate.ExtendedStyleType.FIRSTROWENDCOLUMN);
      String evenCellTableStyle = template.getTableFirstRowTableStyle();
      String evenCellParagraphStyle = template.getTableFirstRowParagraphStyle();
      ...
      applyStyleToRow(
          ...
          evenCellTableStyle,
          lastCellTableStyle,
          firstCellParagraphStyle,
          oddCellParagraphStyle,
          evenCellParagraphStyle,
      ...
      while (rowIterator.hasNext()) {
        ...
        if (!rowIterator.hasNext()) { // last row
          firstCellTableStyle =
              template.getExtendedTableStyleByType(
                  TableTemplate.ExtendedStyleType.LASTROWSTARTCOLUMN);
          firstCellParagraphStyle =
              template.getExtendedParagraphStyleByType(
                  TableTemplate.ExtendedStyleType.LASTROWSTARTCOLUMN);
          lastCellTableStyle =
              template.getExtendedTableStyleByType(
                  TableTemplate.ExtendedStyleType.LASTROWENDCOLUMN);
          lastCellParagraphStyle =
              template.getExtendedParagraphStyleByType(
                  TableTemplate.ExtendedStyleType.LASTROWENDCOLUMN);
          oddCellTableStyle = evenCellTableStyle = template.getTableLastRowTableStyle();
          oddCellParagraphStyle = evenCellParagraphStyle = template.getTableLastRowParagraphStyle();
          ...
          firstCellTableStyle = template.getTableFirstColumnTableStyle();
          firstCellParagraphStyle = template.getTableFirstColumnParagraphStyle();
          lastCellTableStyle = template.getTableLastColumnTableStyle();
          lastCellParagraphStyle = template.getTableLastColumnParagraphStyle();

          if (line % 2 != 0) { // odd row

            oddCellTableStyle = evenCellTableStyle = template.getTableOddRowsTableStyle();
            oddCellParagraphStyle =
                evenCellParagraphStyle = template.getTableOddRowsParagraphStyle();
            applyStyleToRow(
                template,
                currentRow,
                firstCellTableStyle,
            ...
            oddCellTableStyle = evenCellTableStyle = template.getTableEvenRowsTableStyle();
            oddCellParagraphStyle =
                evenCellParagraphStyle = template.getTableEvenRowsParagraphStyle();
          ...
          lastCellTableStyle = template.getTableLastColumnTableStyle();
          lastCellParagraphStyle = template.getTableLastColumnParagraphStyle();
          evenCellTableStyle = template.getTableEvenColumnsTableStyle();
          evenCellParagraphStyle = template.getTableEvenColumnsParagraphStyle();
          oddCellTableStyle = template.getTableOddColumnsTableStyle();
          oddCellParagraphStyle = template.getTableOddColumnsParagraphStyle();
          applyStyleToRow(
              template,
              currentRow,
              firstCellTableStyle,
  ...
  private void applyStyleToRow(
      TableTemplate template,
      Row row,
      String firstCellTableStyle,
      String oddCellTableStyle,
      String evenCellTableStyle,
      String lastCellTableStyle,
      String firstCellParagraphStyle,
      String oddCellParagraphStyle,
      String evenCellParagraphStyle,
      String lastCellParagraphStyle) {
    int cellIndex = 0;
    ...
    for (Node n : new DomNodeList(row.getOdfElement().getChildNodes())) {
      if (n instanceof TableTableCellElementBase) {
        cell = this.getCellInstance((TableTableCellElementBase) n, 0, mnRepeatedIndex);
        if (cell.getColumnsRepeatedNumber() > 1) lastIndex -= cell.getColumnsRepeatedNumber() - 1;
        if (cellIndex == 0) {
  ...
  public int getColumnCount() {
    ...
    for (Node n : new DomNodeList(mTableElement.getChildNodes())) {
      ...
      if (n instanceof TableTableHeaderRowsElement
          || n instanceof TableTableRowsElement
          || n instanceof TableTableRowGroupElement
          || n instanceof TableTableRowElement) {
        break;
      }
    }
    return result;
   ...
   * table.
   *
   * <p>Since SIMPLE supports automatic table expansion. Whenever a cell outside the current table
   * is addressed the table is instantly expanded. Method <code>getCellByPosition</code> can
  ...
  public Row appendRow() {
    ...
    if (refRowElement == null) {
      ...
      TableTableCellElementBase cellElement =
          (TableTableCellElementBase) refRowElement.getFirstChild();
      int i = 1;
      while (cellElement != null && i <= columnCount) {
        ...
        if (cellElement instanceof TableCoveredTableCellElement) {
          ...
          TableTableRowElement aRowEle =
              (TableTableRowElement) (coveredCellEle.getParentNode().getPreviousSibling());
          while (aRowEle != null) {
            // the cover cell and the first covered cell must have
            // the same column index.
            TableTableCellElementBase coverCellEle =
                (TableTableCellElementBase) (aRowEle.getFirstChild());
            ...
            while (j < i) {
              ...
              if (coverCellEle instanceof TableTableCellElement) {
                j +=
                    (coverCellEle.getTableNumberColumnsRepeatedAttribute()
            ...
            if (coverCellEle instanceof TableTableCellElement) {
              TableTableCellElement newCellEle =
                  (TableTableCellElement) (coverCellEle.cloneNode(true));
              ...
              if (mIsSpreadsheet) {
                ...
                int newColumnRepeatedNumber =
                    newCellEle.getTableNumberColumnsRepeatedAttribute() * columnsSpannedNumber;
                ...
                while (tempi > 0) {
                  ...
                  if (iColumnRepeatedNumber > tempi) {
                    // split covered cell
                    if (cellElement instanceof TableCoveredTableCellElement) {
                      cellElement.setTableNumberColumnsRepeatedAttribute(
                          iColumnRepeatedNumber - tempi);
                      TableTableCellElementBase newCoveredCellEle =
                          (TableTableCellElementBase) cellElement.cloneNode(true);
                  ...
                  if (!(cellElement instanceof TableCoveredTableCellElement) && (tempi > 0)) {
                    Logger.getLogger(Table.class.getName())
                        .log(Level.FINE, "Not covered cell was ignored");
                  }
                ...
                while ((cellElement != null)
                    && (cellElement instanceof TableCoveredTableCellElement)
                    && (newColumnSpanNumber > 1)) {
                  TableCoveredTableCellElement newCoveredCellElement =
                      (TableCoveredTableCellElement) cellElement.cloneNode(true);
            ...
            if (mIsSpreadsheet) {
              ...
              while ((j < tableNumberColumnsSpanned) && (cellElement != null)) {
                ...
                if (iColumnRepeatedNumber > tableNumberColumnsSpanned - j) {
                  ...
                  if (cellElement instanceof TableCoveredTableCellElement) {
                    cellElement.setTableNumberColumnsRepeatedAttribute(
                        iColumnRepeatedNumber - tableNumberColumnsSpanned + j);
                    TableTableCellElementBase newCoveredCellEle =
                        (TableTableCellElementBase) cellElement.cloneNode(true);
                    cleanCell(newCoveredCellEle);
                    newCoveredCellEle.setTableNumberColumnsRepeatedAttribute(
  ...
  public Column appendColumn() {
    ...
    if (columnList.size() == 0) // no column, create a new column
    {
      OdfStyle columnStyle =
          mTableElement.getAutomaticStyles().newStyle(OdfStyleFamily.TableColumn);
      String columnStylename = columnStyle.getStyleNameAttribute();
      columnStyle.setProperty(
    ...
    for (int i = 0; i < columnCount; ) {
      ...
      if (!refCell.isCoveredElement()) // not cover element
      {
        ...
        if (coveredLength >= 1) {
          ...
          aRow.appendChild(newCellEle);
          coveredLength =
              coveredCell.getTableNumberColumnsSpannedAttribute() - columnsRepeatedNumber;
        }
        i = i + columnsRepeatedNumber;
      }
    }
  ...
  public List<Column> insertColumnsBefore(int index, int columnCount) {
    ...
    if (refColumn.getOdfElement() == positionCol.getOdfElement()) {
      ...
      if (isUseRepeat()) {
        ...
        TableTableColumnElement newBeforeColumnEle =
            (TableTableColumnElement) refColumn.getOdfElement().cloneNode(true);
        if (index > 1) {
  ...
  /**
   * Calculates the width between the left and right margins of the table container.
   *
   * @param container TableContainer
   * @param marginLeft space between left margin and the table
   * @param marginRight space between right margin and the table
   * @return width that can be attributed at the table (in)
   */
  private static double getTableWidth(
      TableContainer container, double marginLeft, double marginRight) {
    ...
    if (pageLayout != null) {
      ...
      tableWidth =
          (pageWidth - (dLeftPageMargin + dRightPageMargin + marginLeft + marginRight)) / 2.5399;
      if (tableWidth <= 0) {
        tableWidth = DEFAULT_TABLE_WIDTH;
      }
    }
    return Double.valueOf(new DecimalFormat("#0.###").format(tableWidth).replace(",", "."))
        .doubleValue();
  }
      ...
      if (n instanceof TableTableRowsElement) {
        for (Node nn : new DomNodeList(n.getChildNodes())) {
          if (nn instanceof TableTableRowElement) {
            ...
            result += row.getRowsRepeatedNumber();
            if (result > index) {
              return getRowInstance(
                  row.getOdfElement(), index - (result - row.getRowsRepeatedNumber()));
            }
          }
        }
      }
   ...
   * when it's possible.
   *
   * <p>The default setting is <code>true</code>, which helps to decrease the document size. If
   * setting is <code>false</code>, each column/row/cell will be described by its owned single
   * element.
   *
   * <p>This feature setting will influence <code>appendRows()</code>, <code>appendColumns()</code>,
   * <code>insertRowsBefore()</code>, <code>insertColumnsBefore()</code>, <code>getCellByPosition()
   * </code>, <code>getCellRangeByPosition()</code>, <code>getCellRangeByName()</code>, <code>
   * getRowByIndex()</code> and <code>getColumnByIndex()</code>.
   *
   * @return true if the new created columns/rows/cells are described by a single element when it's
   *     possible.
   ...
   * described by a single element. The repeated number attribute, for row is
   * table:number-rows-repeated, while for column and cell are table:number-columns-repeated,
   * specifies the number of columns/rows/cells to which a column/row/cell element applies.
   *
   * <p>This method allows users to set whether the new created columns/rows/cells are described by
   * a single element. Of course, the default setting is <code>true</code>, which helps to decrease
   * the document size. If setting is <code>false</code>, each column/row/cell will be described by
   ...
   * getRowByIndex()</code> and <code>getColumnByIndex()</code>.
   *
   * @param isSingle if<code>isSingle</code> is true, the new created columns/rows/cells are
   *     described by a single element, if possible.
   * @see #isUseRepeat()
   * @see #appendColumns(int)
   * @see #appendRows(int)
   * @see #insertColumnsBefore(int, int)
   ...
   *
   * @param spaceTop space above the table in centimeter(cm), ex. 1.25 cm
   * @param spaceBottom spacing below the table in centimeter(cm), ex. 0.7 cm
   * @since 0.5.5
   */
  public void setVerticalMargin(double spaceTop, double spaceBottom) {
    ...
    if (tableStyle != null) {
      ...
      tableStyle.setProperty(
          ...
          (new DecimalFormat("#0.##").format(spaceBottom) + Unit.CENTIMETER.abbr())
              .replace(",", "."));
    }
  }
   ...
   * @param keepWithNext
   *     <ul>
   *       <li>true sets the property to always
   *       <li>false sets the property to auto
   *     </ul>
   */
  public void setKeepWithNext(boolean keepWithNext) {
    mTableElement.setProperty(
        ...
        keepWithNext
            ? FoKeepWithNextAttribute.Value.ALWAYS.toString()
            : FoKeepWithNextAttribute.Value.AUTO.toString());
  }
  ...
  public DefaultStyleHandler getStyleHandler() {
    if (mStyleHandler == null) mStyleHandler = new DefaultStyleHandler(this.getOdfElement());
