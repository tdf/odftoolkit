...
public class GraphicsDocument extends Document {
  ...
  public static GraphicsDocument newGraphicsDocument() throws Exception {
    return (GraphicsDocument)
        Document.loadTemplate(EMPTY_GRAPHICS_DOCUMENT_RESOURCE, Document.OdfMediaType.GRAPHICS);
  }
    ...
    GraphicsDocument doc =
        (GraphicsDocument)
            Document.loadTemplate(
                EMPTY_GRAPHICS_DOCUMENT_RESOURCE, Document.OdfMediaType.GRAPHICS_TEMPLATE);
   ...
   */
  protected GraphicsDocument(
      OdfPackage pkg, String internalPath, GraphicsDocument.OdfMediaType odfMediaType) {
   ...
   * GraphicsDocument, the InputStream is cached. This usually takes more time compared to the other
   ...
   *
   * <p>GraphicsDocument relies on the file being available for read access over the whole lifecycle
   * of GraphicsDocument.
   *
   * <p>If the resource stream is not a ODF graphics document, ClassCastException might be thrown.
   *
  ...
  public OdfElement getTableContainerElement() {
    throw new UnsupportedOperationException(
        "Graphics document is not supported to hold table now.");
  }
}
