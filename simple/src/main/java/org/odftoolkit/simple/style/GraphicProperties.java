...
/**
 * This class represents the graphic style settings. It provides methods to access borders and
 * background styles. More functions will be added later.
 *
 * <p>This class is a corresponded high level class for element "style:graphic-properties". It
 * provides methods to access the attributes and children of "style:graphic-properties".
 *
 * @since 0.5
 */
public class GraphicProperties {
  ...
  public static GraphicProperties getGraphicProperties(OdfStyleBase style) {
    OdfStylePropertiesBase properties =
        style.getPropertiesElement(OdfStylePropertiesSet.GraphicProperties);
    if (properties != null)
      return new GraphicProperties((StyleGraphicPropertiesElement) properties);
    else return null;
  }
   ...
   *
   * <p>There are three types of stroke: none, solid and dash.
   *
   * <p>If the stroke is NONE, there is no stroke around the frame.
   *
   * <p>If the stroke is SOLID, there is solid line around the frame. color and width need to be
   * specified.
   *
   * <p>If the stroke is DASH, there is dash line around the frame. color, width and the style name
   * of dash line need to be specified.
   *
   * @param stroke the stroke type
   * @param color the color of the stroke
   * @param widthDesc the width description of the stroke, e.g. "0.01in"
   * @param dashStyleName the dash style name
  ...
  public void setStroke(OdfDrawStroke stroke, Color color, String widthDesc, String dashStyleName) {
    switch (stroke) {
      ...
      case SOLID:
        ...
        if (color != null) mElement.setSvgStrokeColorAttribute(color.toString());
   ...
   *
   * <p>This method supports 2 types of fill: none and solid.
   *
   * <p>If the fill type is NONE, the object is no filled at all.
   *
   * <p>If the fill type is SOLID, the drawing object is filled with the color specified by the
   * second parameter.
   *
   * @param fillType the fill style type
  ...
  /**
   * Set whether the content of a graphic object is displayed in the background or foreground. If
   * it's displayed in the background, the content wouldn't be selected or moved.
   *
   * @param isBackgroundObject If <code>true</code>, the graphic object is displayed in the
   *     background.
   * @since 0.5.5
   */
   ...
   *
   * @param horizontalPos the horizontal position
   * @since 0.5.5
   */
   ...
   *
   * @param relative the horizontal relative
   * @since 0.5.5
   */
   ...
   *
   * @param relative the vertical relative
   * @since 0.5.5
   */
  ...
  private boolean verifyWidthDesc(String widthDesc) {
    ...
    if (!(char1 >= 'a' && char1 <= 'z') && !(char1 >= 'A' && (char1 <= 'Z'))) return false;
    if (!(char2 >= 'a' && char2 <= 'z') && !(char2 >= 'A' && (char2 <= 'Z'))) return false;
