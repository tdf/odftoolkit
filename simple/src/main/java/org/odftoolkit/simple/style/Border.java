...
/**
 * This class represents border style settings, including line style, color, width, inner line
 * width, outer line width and the distance.
 *
 * @since 0.3
 */
public class Border {
   ...
   * @param linearMeasure - the linear measurement of the border width
   */
  public Border(
      Color aColor, double width, StyleTypeDefinitions.SupportedLinearMeasure linearMeasure) {
    color = aColor;
    this.width = width;
   ...
   * @param aColor - the color of the border
   * @param width - the line width of the border
   * @param innerLineWidth - the inner line width of the border
   * @param outerLineWidth - the outer line width of the border
   * @param linearMeasure - the linear measurement of the border width
   * @throws IllegalArgumentException if the width is not bigger than the sum of inner line width
   ...
   *
   * <p>The possible return value are "SINGLE","DOUBLE", and "NONE";
   *
   * @return the line style of the border.
   */
   ...
   *
   * <p>The valid parameter can be "SINGLE","DOUBLE", and "NONE";
   *
  ...
  /**
   * Set border style by a description string. The description string includes border width, line
   * style and color. For example: "0.0362in double #ff3333" is a valid description string.
   *
   * @param borderDesc - the description of border style
   */
  protected void setBorderByDescription(String borderDesc) {
    ...
    if (borderStyle.equals("solid")) borderStyle = StyleTypeDefinitions.LineType.SINGLE.toString();

    StyleTypeDefinitions.LineType borderStyleType =
        StyleTypeDefinitions.LineType.enumValueOf(borderStyle);
  ...
  /**
   * Return the description string of border style. The description string includes border width,
   * line style and color. For example, "0.0362in double #ff3333".
   *
  ...
  /**
   * Return a description string for border widths with double lines. The description string
   * includes inner line width, distance, and outline width. For example, "0.0008in 0.0346in
   * 0.0346in".
   *
   * @return - the description for border widths with double lines
   */
  protected String getDoubleLineWidthDescription() {
    if (lineStyle == StyleTypeDefinitions.LineType.SINGLE) return null;
    DecimalFormat formater =
        new DecimalFormat(WidthFormatInInch, new DecimalFormatSymbols(Locale.US));
    return formater.format(innerLineWidth)
        + linearMeasure.toString()
        + " "
        + formater.format(distance)
        + linearMeasure
        + " "
        + formater.format(outerLineWidth)
        + linearMeasure;
  }

  /**
   * Set the border widths with double lines by a description string. The description string
   * includes inner line width, distance, and outline width. For example, "0.0008in 0.0346in
   * 0.0346in".
   *
   * @param widthDesc - the description string
   */
  ...
  public double getInnerLineWidth() {
    ...
    if (lineStyle == StyleTypeDefinitions.LineType.SINGLE) return getWidth();
    else return innerLineWidth;
  }
   ...
   * will happen.
   *
   * @param innerWidth - the inner line width
   */
  public void setInnerLineWidth(double innerWidth) {
    if (lineStyle == StyleTypeDefinitions.LineType.SINGLE) return;
  ...
  /**
   * Set the distance between inner line and outer line of border with double lines. If the line
   * style is not double, nothing will happen.
   *
   * @param distance - the distance between inner line and outer line.
   */
   ...
   * will happen.
   *
   * @param lineWidth - the outer line width
   */
  public void setOuterLineWidth(double lineWidth) {
    if (lineStyle == StyleTypeDefinitions.LineType.SINGLE) return;
    else this.outerLineWidth = lineWidth;
  }
  ...
  private boolean doubleEqual(double d1, double d2) {
    if (this.linearMeasure == SupportedLinearMeasure.IN && Math.abs(d1 - d2) > 0.0002) return false;
    if (this.linearMeasure == SupportedLinearMeasure.PT && Math.abs(d1 - d2) > 0.02) return false;
    return true;
  }

  @Override
  public boolean equals(Object o) {
    if (o == Border.NONE && this == Border.NONE) return true;
    if (o instanceof Border) {
      Border aBorder = (Border) o;
      if (aBorder.getLineStyle() != this.getLineStyle()) return false;
      if (!aBorder.color.toString().equals(this.color.toString())) return false;
      ...
      if (!doubleEqual(aBorder.width, this.width)) return false;
      if (lineStyle == StyleTypeDefinitions.LineType.DOUBLE) {
        if (!doubleEqual(aBorder.distance, this.distance)) return false;
        if (!doubleEqual(aBorder.innerLineWidth, this.innerLineWidth)) return false;
        if (!doubleEqual(aBorder.outerLineWidth, this.outerLineWidth)) return false;
      }
      return true;
    }
    return false;
   ...
   *
   * @param linearMeasure the linear measurement
   */
