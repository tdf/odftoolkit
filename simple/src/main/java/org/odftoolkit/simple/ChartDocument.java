...
public class ChartDocument extends Document {
  ...
  public static ChartDocument newChartDocument() throws Exception {
    return (ChartDocument)
        Document.loadTemplate(EMPTY_CHART_DOCUMENT_RESOURCE, Document.OdfMediaType.CHART);
  }
    ...
    ChartDocument doc =
        (ChartDocument)
            Document.loadTemplate(
                EMPTY_CHART_DOCUMENT_RESOURCE, Document.OdfMediaType.CHART_TEMPLATE);
   ...
   * ChartDocument, the InputStream is cached. This usually takes more time compared to the other
   ...
   *
   * <p>ChartDocument relies on the file being available for read access over the whole lifecycle of
   * ChartDocument.
   *
   * <p>If the resource stream is not a ODF chart document, ClassCastException might be thrown.
   *
  ...
  protected ChartDocument(
      OdfPackage pkg, String internalPath, ChartDocument.OdfMediaType odfMediaType) {
