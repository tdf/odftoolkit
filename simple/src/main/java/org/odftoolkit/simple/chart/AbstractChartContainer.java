...
public abstract class AbstractChartContainer implements ChartContainer {
  ...
  protected AbstractChartContainer(Document doc) {
    try {
      ...
      for (Document chartDoc : charts) {
        ...
        ChartChartElement chartEle =
            (ChartChartElement)
                contentDom.getXPath().evaluate("//chart:chart[1]", contentDom, XPathConstants.NODE);
  ...
  public Chart createChart(String title, DataSet dataset, Rectangle rect) {
    try {
      ...
      DrawObjectElement drawObject =
          OdfElement.findFirstChildNode(DrawObjectElement.class, drawFrame);
      if (drawObject == null) {
  ...
  public Chart createChart(
      ...
      Rectangle rect) {
    DataSet dataset =
        new DataSet(cellRangeAddr, document, firstRowAsLabel, firstColumnAsLabel, rowAsDataSeries);
  ...
  public void deleteChartById(String chartId) {
    try {
      ...
      frameObjects =
          mDocument
              .getContentDom()
              .getElementsByTagNameNS(OdfDocumentNamespace.DRAW.getUri(), "object");
