...
/**
 * <code>Chart</code> represents the chart feature of the ODF document. <code>Chart</code> provides
 * methods to get/set chart title, get/set chart data, etc.
 *
 * @since 0.6
 */
public class Chart {
  ...
  private void setPlotArea() {
    // chart:plotarea
    NodeList plotAreas =
        chartElement.getElementsByTagName(ChartPlotAreaElement.ELEMENT_NAME.getQName());
    ...
    // chart:axis
    ChartAxisElement axisX =
        plotArea.newChartAxisElement(ChartDimensionAttribute.Value.x.toString());
    if (getTableCellRange() != null) {
      ChartCategoriesElement categories =
          (ChartCategoriesElement) axisX.newChartCategoriesElement();
    ...
    ChartAxisElement axisY =
        (ChartAxisElement) plotArea.newChartAxisElement(ChartDimensionAttribute.Value.y.toString());
   ...
   *
   * @param dimType the chart axis dimension, x, y or z
   * @return return axis title according to the given dimension,null if the specific dimensional
   *     axis has no title
   */
  public String getAxisTitle(String dimType) {
    NodeList chartAxises =
        (NodeList) plotArea.getElementsByTagName(ChartAxisElement.ELEMENT_NAME.getQName());
    for (int i = 0; i < chartAxises.getLength(); i++) {
      ...
      if (dimension.equals(dimType)) {
        ...
        NodeList paras =
            (NodeList) axisTitle.getElementsByTagName(TextPElement.ELEMENT_NAME.getQName());
    ...
    if (dataSet == null) {
      ...
      if (table == null) {
        table =
            (TableTableElement)
                chartElement
                    .getElementsByTagName(TableTableElement.ELEMENT_NAME.getQName())
                    .item(0);
        headerRows =
            (TableTableHeaderRowsElement)
                table
                    .getElementsByTagName(TableTableHeaderRowsElement.ELEMENT_NAME.getQName())
                    .item(0);
        headerRow =
            (TableTableRowElement)
                table.getElementsByTagName(TableTableRowElement.ELEMENT_NAME.getQName()).item(0);
        NodeList headerCells =
            headerRow.getElementsByTagName(TableTableCellElement.ELEMENT_NAME.getQName());
        ...
        for (int i = 1; i < legendcount + 1; i++) {
          ...
          legends[i - 1] =
              ((OdfTextParagraph)
                      headerCell.getElementsByTagName(TextPElement.ELEMENT_NAME.getQName()).item(0))
                  .getTextContent();
        }

        rows =
            (TableTableRowsElement)
                table.getElementsByTagName(TableTableRowsElement.ELEMENT_NAME.getQName()).item(0);
        ...
        for (int i = 0; i < labelcount; i++) {
          NodeList cells =
              ((TableTableRowElement) row.item(i))
                  .getElementsByTagNameNS(OdfDocumentNamespace.TABLE.getUri(), "table-cell");
          labels[i] =
              ((OdfTextParagraph)
                      ((TableTableCellElement) cells.item(0))
                          .getElementsByTagNameNS(OdfDocumentNamespace.TEXT.getUri(), "p")
                          .item(0))
                  .getTextContent();
          for (int j = 0; j < legendcount; j++) {
            String aValue =
                ((TableTableCellElement) cells.item(j + 1))
                    .getOdfAttributeValue(
                        OdfName.newName(
                            OdfNamespace.newNamespace(OdfDocumentNamespace.OFFICE), "value"));
  ...
  public String getChartTitle() {
    if (chartTitle == null) {
      chartTitle =
          (ChartTitleElement)
              chartElement.getElementsByTagName(ChartTitleElement.ELEMENT_NAME.getQName()).item(0);
    }
    NodeList paras =
        (NodeList) chartTitle.getElementsByTagName(TextPElement.ELEMENT_NAME.getQName());
   ...
   *
   * @param _3deffect a flag specifying whether or not apply a 3D effect
   */
   ...
   *
   * @param dataset the data set for the chart, which is a 2 dimensional data container
   */
  public void setChartData(DataSet dataset) {
    ...
    setPlotArea();
    NodeList tables =
        chartElement.getElementsByTagNameNS(OdfDocumentNamespace.CHART.getUri(), "table");
  ...
  public boolean setChartTitle(String title) {
    ...
    // chart:title
    NodeList titles =
        chartElement.getElementsByTagNameNS(OdfDocumentNamespace.CHART.getUri(), "title");
    if (titles.getLength() == 0) {
   ...
   *
   * @param useLegend a flag specifying whether or not use legend
   */
  public void setUseLegend(boolean useLegend) {
    this.isUseLegend = useLegend;
    NodeList legends =
        chartElement.getElementsByTagNameNS(OdfDocumentNamespace.CHART.getUri(), "legend");
    if (useLegend) {
      if (legends.getLength() == 0) {
        legend =
            chartElement.newChartLegendElement(
                ChartLegendPositionAttribute.Value.TOP_END.toString(),
  ...
  private Object[] getValueCellRange() {
    ...
    if (dataSet.isLocalTable()) {
      dataSet.getLocalTableCellRanges(
          dataSet.getDataSeriesCount(),
          dataSet.getLabels().length,
          seriesCellRange,
          legendCellAddr);
    } else {
      dataSet.getCellRanges(
          dataSet.getCellRangeAddress().toString(),
          dataSet.isFirstRowAsLabel(),
          dataSet.isFirstColumnAsLabel(),
          dataSet.isRowAsDataSeries(),
          seriesCellRange,
          legendCellAddr);
    }
  ...
  private String getCellRanges(
      String tablecellrange,
    ...
    if (seriesCellRange == null) seriesCellRange = new Vector<String>();
    else seriesCellRange.removeAllElements();
    if (legendCellAddr == null) legendCellAddr = new Vector<String>();
    else legendCellAddr.removeAllElements();
    ...
    if (rowAsDataSeries) {
      ...
      if (!bFirstRowAsLabel) {
        ...
        labelCellRange =
            createCellRange(sheettable, beginRow, (char) (beginColumn + 1), beginRow, endColumn);
      else labelCellRange = createCellRange(sheettable, beginRow, beginColumn, beginRow, endColumn);
      for (int i = starti; i < endRow + 1; i++) {
        if (bFirstColumnAsLabel) {
          seriesCellRange.add(
              createCellRange(sheettable, i, (char) (beginColumn + 1), i, endColumn));
        } else {
      ...
      if (!bFirstColumnAsLabel) {
        ...
        labelCellRange =
            createCellRange(sheettable, beginRow + 1, beginColumn, endRow, beginColumn);
      else labelCellRange = createCellRange(sheettable, beginRow, beginColumn, endRow, beginColumn);

      for (char ch = startch; ch <= endColumn; ch++) {
        if (bFirstRowAsLabel)
          seriesCellRange.add(createCellRange(sheettable, beginRow + 1, ch, endRow, ch));
        else seriesCellRange.add(createCellRange(sheettable, beginRow, ch, endRow, ch));

        if (bFirstRowAsLabel) legendCellAddr.add(sheettable + "." + ch + beginRow);
        else legendCellAddr.add(null);
      }
    }
  ...
  private String getLocalTableCellRanges(
      int seriesCount,
      int labelLength,
      Vector<String> seriesCellRange,
      Vector<String> legendCellAddr) {
    ...
    String tablecellrange =
        localtable + "." + "A1:" + (char) ('A' + seriesCount) + (1 + labelLength);
    return dataSet.getCellRanges(
        tablecellrange, true, true, false, seriesCellRange, legendCellAddr);
  }

  private String createCellRange(
      String table, int beginRow, char beginColumn, int endRow, char endColumn) {
