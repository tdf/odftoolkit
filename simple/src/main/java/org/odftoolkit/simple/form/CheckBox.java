...
public class CheckBox extends FormControl {
  ...
  /**
   * Get an instance of check box by an instance of FormCheckboxElement, while searching the
   * document content to make a bind with the DrawControl which already reference to this check box.
   *
   * @param element - an instance of FormCheckboxElement
  ...
  public static CheckBox getInstanceOf(FormCheckboxElement element) {
    ...
    try {
      checkbox.loadDrawControl(
          ((Document) ((OdfFileDom) element.getOwnerDocument()).getDocument()).getContentRoot());
    } catch (Exception e) {
      Logger.getLogger(CheckBox.class.getName())
          .log(Level.WARNING, "Cannot load the drawing shape of this check box.");
    }
    return checkbox;
  }
  ...
  public void setControlImplementation(String controlImpl) {
    ((FormCheckboxElement) mElement).setFormControlImplementationAttribute(controlImpl);
  }
  ...
  FormPropertiesElement getFormPropertiesElementForWrite() {
    if (mFormProperties == null)
      mFormProperties = ((FormCheckboxElement) mElement).newFormPropertiesElement();
   ...
   *
   * @param label - the label of this check box
   */
   ...
   *
   * @param state - default state of this check box
   */
  public void setDefaultState(FormCheckboxState state) {
    ((FormCheckboxElement) mElement).setFormStateAttribute(state.toString());
  }
   ...
   *
   * @param state - current state of this check box
   */
  public void setCurrentState(FormCheckboxState state) {
    ((FormCheckboxElement) mElement).setFormCurrentStateAttribute(state.toString());
  }
  ...
  public FormCheckboxState getCurrentState() {
    String value = ((FormCheckboxElement) mElement).getFormCurrentStateAttribute();
    return FormCheckboxState.enumValueOf(value);
  }
   ...
   *
   * @param container - an instance of form where to traverse the check boxes
   */
  ...
  private static class SimpleCheckBoxIterator implements Iterator<FormControl> {
    ...
    private CheckBox findNext(CheckBox thisCheckBox) {
      ...
      if (thisCheckBox == null) {
        nextCheckBox = OdfElement.findFirstChildNode(FormCheckboxElement.class, containerElement);
      } else {
        nextCheckBox =
            OdfElement.findNextChildNode(FormCheckboxElement.class, thisCheckBox.getOdfElement());
      }
