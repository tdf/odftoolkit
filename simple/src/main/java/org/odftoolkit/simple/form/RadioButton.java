...
public class RadioButton extends FormControl {
  ...
  /**
   * Get an instance of radio button by an instance of FormRadioElement, while searching the
   * document content to make a bind with the DrawControl which already reference to this radio
   * button.
   *
   * @param element - an instance of FormRadioElement
  ...
  public static RadioButton getInstanceOf(FormRadioElement element) {
    ...
    try {
      radio.loadDrawControl(
          ((Document) ((OdfFileDom) element.getOwnerDocument()).getDocument()).getContentRoot());
    } catch (Exception e) {
      Logger.getLogger(RadioButton.class.getName())
          .log(Level.WARNING, "Cannot load the drawing shape of this radio button.");
    }
    return radio;
  }
  ...
  public void setControlImplementation(String controlImpl) {
    ((FormRadioElement) mElement).setFormControlImplementationAttribute(controlImpl);
  }
  ...
  FormPropertiesElement getFormPropertiesElementForWrite() {
    if (mFormProperties == null)
      mFormProperties = ((FormRadioElement) mElement).newFormPropertiesElement();
   ...
   *
   * @param label - the label content of this radio button
   */
   ...
   *
   * @param isSelected - true means selected; false means unselected
   */
  ...
  public void setCurrentSelected(boolean isSelected) {
    ((FormRadioElement) mElement).setFormCurrentSelectedAttribute(isSelected);
  }
   ...
   *
   * @param container - an instance of form where to traverse the radio buttons
   */
  public static Iterator<FormControl> getSimpleIterator(Form container) {
    return new SimpleRadioButtonIterator(container);
  }

  private static class SimpleRadioButtonIterator implements Iterator<FormControl> {
    ...
    private RadioButton findNext(RadioButton thisRadioButton) {
      ...
      if (thisRadioButton == null) {
        nextRadioButton = OdfElement.findFirstChildNode(FormRadioElement.class, containerElement);
      } else {
        nextRadioButton =
            OdfElement.findNextChildNode(FormRadioElement.class, thisRadioButton.getOdfElement());
      }
