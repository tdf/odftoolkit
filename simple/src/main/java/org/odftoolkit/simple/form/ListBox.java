...
public class ListBox extends FormControl {
  ...
  /**
   * Get an instance of list box by an instance of FormListboxElement, while searching the document
   * content to make a bind with the DrawControl which already reference to this list box.
   *
  ...
  public static ListBox getInstanceOf(FormListboxElement element) {
    ...
    try {
      listbox.loadDrawControl(
          ((Document) ((OdfFileDom) element.getOwnerDocument()).getDocument()).getContentRoot());
    } catch (Exception e) {
      Logger.getLogger(ListBox.class.getName())
          .log(Level.WARNING, "Cannot load the drawing shape of this listbox.");
    }
    return listbox;
  }
  ...
  FormPropertiesElement getFormPropertiesElementForWrite() {
    if (mFormProperties == null)
      mFormProperties = ((FormListboxElement) mElement).newFormPropertiesElement();
    return mFormProperties;
  }

  @Override
  public void setControlImplementation(String controlImpl) {
    ((FormListboxElement) mElement).setFormControlImplementationAttribute(controlImpl);
  }
   ...
   *
   * @param isMultiSelection - specify if the list box supports multi-selection
   */
  public void setFormMultiSelection(boolean isMultiSelection) {
    ((FormListboxElement) mElement).setFormMultipleAttribute(isMultiSelection);
  }
  ...
  public void addItem(String item) {
    ...
    ((FormListboxElement) mElement).newFormOptionElement().setFormLabelAttribute(item);
    entries.add(item);
  }
    ...
    if (entries == null || entries.size() == 0) {
      ...
      if (items != null && items.getLength() > 0) {
        for (int i = 0; i < items.getLength(); i++) {
          if (entries == null) entries = new ArrayList<String>();
          entries.add(((FormOptionElement) items.item(i)).getFormLabelAttribute());
        }
        return entries;
      }
    }
  ...
  public void setListSourceType(FormListSourceType type) {
    ((FormListboxElement) mElement).setFormListSourceTypeAttribute(type.toString());
  }
  ...
  public FormListSourceType getListSourceType() {
    String aValue = ((FormListboxElement) mElement).getFormListSourceTypeAttribute();
   ...
   *
   * @param container - an instance of form where to traverse the list boxes
   */
  ...
  private static class SimpleListBoxIterator implements Iterator<FormControl> {
    ...
    private ListBox findNext(ListBox thisListBox) {
      ...
      if (thisListBox == null) {
        nextListBox = OdfElement.findFirstChildNode(FormListboxElement.class, containerElement);
      } else {
        nextListBox =
            OdfElement.findNextChildNode(FormListboxElement.class, thisListBox.getOdfElement());
      }
