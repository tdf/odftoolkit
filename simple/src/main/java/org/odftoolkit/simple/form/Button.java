...
public class Button extends FormControl {
   ...
   *
   * @param element - an instance of FormButtonElement
  ...
  public static Button getInstanceOf(FormButtonElement element) {
    ...
    try {
      ...
      Logger.getLogger(Button.class.getName())
          .log(Level.WARNING, "Cannot load the drawing shape of this button.");
    }
    return btn;
  }
  ...
  public void setControlImplementation(String controlImpl) {
    ((FormButtonElement) mElement).setFormControlImplementationAttribute(controlImpl);
  }

  FormPropertiesElement getFormPropertiesElementForWrite() {
    if (mFormProperties == null)
      mFormProperties = ((FormButtonElement) mElement).newFormPropertiesElement();
   ...
   *
   * @param container - an instance of form where to traverse the buttons
   */
  ...
  private static class SimpleButtonIterator implements Iterator<FormControl> {
    ...
    private Button findNext(Button thisButton) {
      ...
      if (thisButton == null) {
        nextForm = OdfElement.findFirstChildNode(FormButtonElement.class, containerElement);
      } else {
        nextForm =
            OdfElement.findNextChildNode(FormButtonElement.class, thisButton.getOdfElement());
      }
