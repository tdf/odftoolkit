 ...
 * special number list with outline. User can extend this class and realize their own list and list
 * item style. For example, set a specifies list item with red color.
 *
 * <p>An OutLineDecorator can be reused in the same Document.
 *
 * @since 0.4
 */
public class OutLineDecorator implements ListDecorator {
  ...
  private static String[] DEFAULT_TEXT_SPACE_BEFORE_ATTRIBUTES = {
    null, "0.501cm", "1cm", "1.501cm", "2cm", "2.501cm", "3.001cm", "3.502cm", "4.001cm", "4.502cm"
  };
   ...
   *
   * @param doc the Document which this OutLineDecorator will be used on.
   */
  public OutLineDecorator(Document doc) {
    ...
    try {
      ...
      if (listStyle == null) {
        ...
        for (int i = 0; i < 10; i++) {
          TextListLevelStyleNumberElement listLevelElement =
              listStyle.newTextListLevelStyleNumberElement(DEFAULT_NUM_FORMAT, i + 1);
          ...
          if (DEFAULT_TEXT_SPACE_BEFORE_ATTRIBUTES[i] != null) {
            styleListLevelPropertiesElement.setTextSpaceBeforeAttribute(
                DEFAULT_TEXT_SPACE_BEFORE_ATTRIBUTES[i]);
          }
          styleListLevelPropertiesElement.setTextMinLabelWidthAttribute(
              DEFAULT_TEXT_MIN_LABEL_WIDTH);
          StyleTextPropertiesElement styleTextPropertiesElement =
              listLevelElement.newStyleTextPropertiesElement("true");
