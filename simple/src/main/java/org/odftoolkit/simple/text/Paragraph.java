...
/**
 * This class presents paragraph element in ODF document. It provides methods to manipulate text
 * content, and other child component under the paragraph. Headings and body text paragraphs are
 * collectively referred to as paragraph elements.
 *
 * @since 0.5
 */
public class Paragraph extends Component
    implements TextboxContainer, TextHyperlinkContainer, ControlContainer {
  ...
  /**
   * Gets a paragraph instance by an instance of <code>TextParagraphElementBase</code>.
   *
   * @param paragraphElement the instance of TextParagraphElementBase.
   * @return an instance of paragraph.
   */
  public static Paragraph getInstanceof(TextParagraphElementBase paragraphElement) {
    if (paragraphElement == null) return null;
   ...
   *
   * <p>The paragrah will be added at the end of this container.
   *
   ...
   * Create an instance of paragraph
   *
   * <p>The paragraph will be added before the odfelement 'brother'.
   *
   * @param container the paragraph container that contains this paragraph.
   * @param brother the the brother odfelement before that the new paragraph will be inserted.
   */
   ...
   *
   * <p>All the existing text content of this paragraph would be removed, and then new text content
   * would be set. The style of the last character will be inherited.
   *
   * <p>The white space characters in the content would be collapsed by default. For example, tab
   * character would be replaced with <text:tab>, break line character would be replaced with
   * <text:line-break>.
   *
  ...
  public void setTextContent(String content) {
    ...
    if (lastNode != null
        && lastNode.getNodeName() != null
        && (lastNode.getNodeName().equals("text:a")
  ...
  static void removeEmptyHyperlink(OdfElement element) {
    ...
    for (int i = 0; i < nodeList.getLength(); i++) {
      ...
      if (node.getNodeType() == Node.ELEMENT_NODE) {
        ...
        if (nodename.equals("text:a") && node.hasChildNodes() == false) element.removeChild(node);
      }
    }
  }
   ...
   *
   * <p>The other child elements except text content will not be removed.
   */
    ...
    for (int i = 0; i < nodeList.getLength(); i++) {
      ...
      if (node.getNodeType() == Node.TEXT_NODE) {
        ...
        if (nodename.equals("text:s")
            || nodename.equals("text:tab")
            || nodename.equals("text:line-break")) {
          ownerElement.removeChild(node);
          // element removed need reset index.
          i--;
        } else if (nodename.equals("text:a")) removeTextContentImpl((OdfElement) node);
      }
    }
  }
   ...
   *
   * <p>The other child elements except text content will not be returned.
   *
   * @return - the text content of this paragraph
   */
   ...
   * would be set.
   *
   * <p>The white space characters in the content would not be collapsed.
   *
   * @param content - the text content
  ...
  public void setTextContentNotCollapsed(String content) {
    removeTextContent();
    if (content != null && !content.equals("")) appendTextElements(getOdfElement(), content, false);
  }

  /**
   * Append the text content at the end of this paragraph. The appended text would follow the style
  ...
  /**
   * Append the text content at the end of this paragraph. The appended text would follow the style
   * of the last character if the second parameter is set to true; Or else, the appended text would
   * follow the default style of this paragraph.
   *
   * <p>The white space characters in the content would be collapsed by default. For example, tab
   * character would be replaced with <text:tab>, break line character would be replaced with
   * <text:line-break>.
   *
   * @param content - the text content
   * @param isStyleInherited - whether the style would be inherited by the appended text
   * @see #appendTextContentNotCollapsed(String content)
   */
   ...
   *
   * <p>The style handler is an instance of ParagraphStyleHandler
   *
  ...
  /**
   * Return the <code>TextParagraphElementBase</code> of this paragraph. Headings and body text
   * paragraphs are collectively referred to as paragraph elements, so the <code>
   * TextParagraphElementBase</code> can be <code>TextHElement</code> element or <code>TextPElement
   * </code> element.
   *
  ...
  public void addComment(String content, String creator) {
    ...
    OfficeAnnotationElement annotationElement =
   ...
   *
   * @return the paragraph type, if this paragraph is heading, returns <code>true</code>, otherwise
   ...
   *
   * @return outline level, if this paragraph is a body text paragraph, 0 will be returned.
   * @since 0.6.5
   */
   ...
   *
   * @param isHeading if <code>true</code>, this paragraph would be formatted as heading, otherwise
   *     as a body text paragraph.
   * @param level the heading outline level of this paragraph, if <code>isHeading</code> is <code>
   *     true</code>.
   * @since 0.6.5
   */
   ...
   *
   * @return font if there is no style definition for this paragraph, <code>null</code> will be
   *     returned.
   * @since 0.6.5
   */
  public Font getFont() {
   ...
   *
   * @param font the font definition of this paragraph
   * @since 0.6.5
   */
   ...
   *
   * <p>Null will returned if there is no explicit style definition for this paragraph.
   *
  ...
  /**
   * Set the horizontal alignment setting of this paragraph. If the alignment is set as Default, the
  ...
  private TextboxContainerImpl getTextboxContainerImpl() {
    if (mTextboxContainerImpl == null) mTextboxContainerImpl = new TextboxContainerImpl();
    return mTextboxContainerImpl;
  }

  static void appendTextElements(
      OdfElement ownerElement, String content, boolean isWhitespaceCollapsed) {
    ...
    if (isWhitespaceCollapsed) {
      ...
      while (i < length) {
        ...
        if (ch == ' ') {
          ...
          TextLineBreakElement lineBreakElement =
              ownerDocument.newOdfElement(TextLineBreakElement.class);
