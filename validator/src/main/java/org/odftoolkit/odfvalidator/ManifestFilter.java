...
class ManifestFilter extends NamespaceFilter {
  ...
  private static final String MANIFEST_NAMESPACE_URI =
  ...
  ManifestFilter(
      Logger aLogger,
      ManifestListener aManifestListener,
  ...
  public InputSource resolveEntity(String aPublicId, String aSystemId)
    ...
    if (aPublicId.equals(OOO_MANIFEST_PUBLIC_ID)) {
      String aMsg =
          "Ignoring doctype definition '"
              + OOO_MANIFEST_PUBLIC_ID
              + "' (has been stored by old OOo versions)";
      m_aLogger.logInfo(aMsg, false);
      return new InputSource(new StringReader(""));
    } else return super.resolveEntity(aPublicId, aSystemId);
  }

  String adaptNamespaceUri(String aUri, String aPrefix) {
    String aNewUri = null;
    if (aUri.equals(OOO_MANIFEST_NAMESPACE_URI)) aNewUri = MANIFEST_NAMESPACE_URI;
  ...
  void namespaceUriAdapted(String aUri, String aNewUri) {
    String aMsg =
        "Adapting OpenOffice.org namspace'" + aUri + "' (has been stored by old OOo versions)";
    m_aLogger.logInfo(aMsg, false);
  }
  ...
  public void startElement(String aUri, String aLocalName, String aQName, Attributes aAtts)
    ...
    if ((aUri.equals(MANIFEST_NAMESPACE_URI) || aUri.equals(OOO_MANIFEST_NAMESPACE_URI))
        && aLocalName.equals(FILE_ENTRY)) {
      String aFullPath = aAtts.getValue(aUri, FULL_PATH);
      String aMediaType = aAtts.getValue(aUri, MEDIA_TYPE);
      if (aFullPath != null) {
        if (aFullPath.equals("/")) {
          if (m_aManifestListener != null) m_aManifestListener.setMediaType(aMediaType);
        } else {
