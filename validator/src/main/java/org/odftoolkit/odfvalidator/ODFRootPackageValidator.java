...
abstract class ODFRootPackageValidator extends ODFPackageValidator implements ManifestEntryListener {
 ...
 outer: for (int i = haystack.length - needle.length; 0 <= i; --i) {
    ...
    class WishJavaHadTuples
    {
        ZipEntry entry;
        int pos;
        WishJavaHadTuples(ZipEntry a, int b) { entry = a; pos = b; }
    }
    ...
    private WishJavaHadTuples getFileHeader(ByteBuffer buf, int pos) throws IOException
    {
        ...
        return new WishJavaHadTuples(ret,
    ...
    private InputStream readEntry(byte[] file, ZipEntry entry) throws IOException
    {
        ...
        ByteArrayInputStream stream = new ByteArrayInputStream(file,
            entry.offset + 30 + fileNameLength + extraFieldLength, entry.size);
        if (entry.compressed) {
        ...
        try {
            ...
            for (ZipEntry entry : entries) {
                if (entry.name.equals(OdfPackage.OdfFile.MANIFEST.getPath())) {
                    ...
                    Validator manifestValidator = m_aValidatorProvider.getManifestValidator(logger.getOutputStream(), version);
