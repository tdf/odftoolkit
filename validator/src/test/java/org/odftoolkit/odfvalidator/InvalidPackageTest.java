...
public class InvalidPackageTest extends OdfValidatorTestBase {
  ...
  public void validateEncryptedODT() {
    ...
    Assert.assertTrue(
        output.contains(
            "Fatal: ZIP entry 'mimetype': only DEFLATED entries can have EXT descriptor"));
    Assert.assertTrue(
        output.contains(
  ...
  public void validateEncryptedODF13WithInvalidOpenPGP() {
    ...
    Assert.assertTrue(
        output.contains(
            "META-INF/manifest.xml[3,20]:  Error: tag name \"manifest:keyinfo\" is not allowed. Possible tag names are: <encrypted-key>,<file-entry>"));
    Assert.assertTrue(
        output.contains(
            "META-INF/manifest.xml[10,25]:  Error: element \"manifest:CipherData\" was found where no element may occur"));
