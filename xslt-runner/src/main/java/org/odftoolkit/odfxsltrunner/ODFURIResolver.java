...
class ODFURIResolver implements URIResolver {
   ...
   * @param aFileEntryPath the path of the file within the package in which the to be resolved URIs
   *     are included
   * @param aLogger the logger for error messages
   */
  public ODFURIResolver(
      OdfPackage aPackage, String aPackageBase, String aFileEntryPath, Logger aLogger) {
    m_aPackage = aPackage;
  ...
  public Source resolve(String aHRef, String aBase) throws TransformerException {
    if (aBase.startsWith(m_aPackageBase) && !aHRef.contains(":") && !aHRef.startsWith("/")) {
      if (aHRef.length() == 0) {
        try {
          return new StreamSource(
              m_aPackage.getInputStream(m_aFileEntryPath), m_aPackageBase + '/' + m_aFileEntryPath);
        } catch (Exception ex) {
      ...
      do {
        if (aHRefBuffer.substring(0, 2).equals("./")) {
          ...
          try {
            ...
            return new StreamSource(
                m_aPackage.getInputStream(aFileEntryPath), m_aPackageBase + '/' + aFileEntryPath);
          } catch (Exception ex) {
            m_aLogger.logError(ex.getMessage());
            return null;
          }
